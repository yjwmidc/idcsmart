<?php /*a:1:{s:90:"/mnt/sdc/Webs/zjmf-v10-console/public/plugins/server/mf_cloud/template/admin/mf_cloud.html";i:1755671708;}*/ ?>
...
<div class="common_config">
  <com-config>
      ...
      <t-tabs v-model="tabs" @change="changeTab">
        ...
        <t-tab-panel value="duration" :label="lang.mf_duration">
          <div class="box limit-table">
            <div class="com-top small-gap">
              <p class="tit first-text-color">
                {{lang.mf_duration}}
                <span class="cycle-tip" v-if="isShowCycleTip">({{lang.empty_cycle_tip}})</span>
              </p>
              <div class="right">
                <t-button class="add-btn" @click="addCycle" :disabled="isAgent">
                  {{lang.add_cycle}}
                </t-button>
                <t-button class="add-btn com-gray-btn" theme="default" @click="changeRadio">
                  {{lang.cycle_ratio}}
                </t-button>
              </div>
            </div>
            <p class="s-tip third-text-color">{{lang.mf_tip30}}</p>
            <t-table row-key="id" :data="cycleData" size="medium" :columns="cycleColumns" :hover="hover"
              ...
              <template #price_factor="{row}">{{row.price_factor.toFixed(2)}}</template>
              <template #unit="{row}">
        ...
        <!-- 按需付费 -->
        <t-tab-panel value="demand" :label="lang.mf_demand_pay" v-if="calcShowDemand">
          <div class="box demand-box">
            <t-form :data="demandForm" ref="otherConfig" @submit="submitDemand" :rules="demandRules" label-align="left">
              <t-form-item :label="lang.mf_pay_cycle" class="required">
                <t-form-item name="billing_cycle_unit">
                  <t-select v-model="demandForm.billing_cycle_unit" :disabled="isAgent"
                    :placeholder="lang.mf_pay_cycle">
                    <t-option :value="item.value" :label="item.label" v-for="(item,index) in cycleUnitList"
                      :key="index"></t-option>
                  </t-select>
                </t-form-item>
                <t-form-item name="billing_cycle_day" v-if="demandForm.billing_cycle_unit === 'month'"
                  class="cycle-short">
                  <t-select v-model="demandForm.billing_cycle_day" :disabled="isAgent">
                    <t-option :value="item.value" :label="item.label" v-for="item in dayArr"
                      :key="item.value"></t-option>
                  </t-select>
                </t-form-item>
                <t-form-item name="billing_cycle_point"
                  v-if="demandForm.billing_cycle_unit && demandForm.billing_cycle_unit !== 'hour'" class="cycle-short"
                  :class="{half: demandForm.billing_cycle_unit === 'day'}">
                  <t-time-picker format="HH:mm" v-model="demandForm.billing_cycle_point"
                    :disabled="isAgent"></t-time-picker>
                </t-form-item>
              </t-form-item>
              <t-form-item :label="lang.cycle_ratio">
                <t-form-item>
                  <t-select v-model="demandForm.duration_id" :disabled="isAgent" clearable>
                    <t-option :value="item.id" :label="item.name" v-for="item in cycleData" :key="item.id"></t-option>
                  </t-select>
                </t-form-item>
                <span>{{lang.mf_demand_tip1}}</span>
                <t-input-number theme="normal" :min="0" :decimal-places="2" :suffix="lang.mf_multiple"
                  v-model="demandForm.duration_ratio" class="short"
                  @change="resetMinNum($event, 'duration_ratio')"></t-input-number>
              </t-form-item>
              <!-- <t-form-item :label="lang.mf_demand_grain">
                <t-select value="minute" disabled>
                  <t-option value="minute" :label="lang.mf_per_minute" key="minute"></t-option>
                </t-select>
              </t-form-item> -->
              <t-form-item :label="lang.mf_demand_tip2" name="min_credit">
                <t-input-number theme="normal" :min="0" :decimal-places="2" :placeholder="lang.mf_demand_tip15"
                  @change="resetMinNum($event,'min_credit')" :label="currency_prefix"
                  v-model="demandForm.min_credit"></t-input-number>
              </t-form-item>
              <t-form-item :label="lang.mf_demand_tip3" class="required">
                <t-form-item name="min_usage_time">
                  <t-input-number theme="normal" :min="0" :decimal-places="0" :placeholder="lang.mf_demand_tip15"
                    v-model="demandForm.min_usage_time" @change="resetMinNum($event,'min_usage_time')"></t-input-number>
                </t-form-item>
                <t-form-item name="min_usage_time_unit" class="time-select">
                  <t-select v-model="demandForm.min_usage_time_unit" :disabled="isAgent">
                    <t-option :value="item.value" :label="item.label" v-for="item in timeArr"
                      :key="item.value"></t-option>
                  </t-select>
                </t-form-item>
              </t-form-item>
              <t-form-item :label="lang.mf_demand_tip13" class="required">
                <t-form-item name="upgrade_min_billing_time">
                  <t-input-number theme="normal" :min="0" :decimal-places="0" :placeholder="lang.mf_demand_tip15"
                    v-model="demandForm.upgrade_min_billing_time"
                    @change="resetMinNum($event, 'upgrade_min_billing_time')"></t-input-number>
                </t-form-item>
                <t-form-item name="upgrade_min_billing_time_unit" class="time-select">
                  <t-select v-model="demandForm.upgrade_min_billing_time_unit" :disabled="isAgent">
                    <t-option :value="item.value" :label="item.label" v-for="item in timeArr"
                      :key="item.value"></t-option>
                  </t-select>
                </t-form-item>
              </t-form-item>
              <t-form-item :label="lang.mf_grace_time" class="required">
                <t-form-item name="grace_time">
                  <t-input-number theme="normal" :min="0" :decimal-places="0"
                    @change="resetMinNum($event, 'grace_time')" :placeholder="lang.mf_demand_tip15"
                    v-model="demandForm.grace_time"></t-input-number>
                </t-form-item>
                <t-form-item name="grace_time_unit" class="time-select">
                  <t-select v-model="demandForm.grace_time_unit" :disabled="isAgent">
                    <t-option :value="item.value" :label="item.label" v-for="item in keepArr"
                      :key="item.value"></t-option>
                  </t-select>
                </t-form-item>
                <t-tooltip :content="lang.mf_demand_tip11" :show-arrow="false" theme="light" placement="top-left">
                  <t-icon name="help-circle"></t-icon>
                </t-tooltip>
              </t-form-item>
              <t-form-item :label="lang.mf_keep_time" class="required">
                <t-form-item name="keep_time">
                  <t-input-number theme="normal" :min="0" :decimal-places="0" @change="resetMinNum($event, 'keep_time')"
                    v-model="demandForm.keep_time" :placeholder="lang.mf_demand_tip15"></t-input-number>
                </t-form-item>
                <t-form-item name="keep_time_unit" class="time-select">
                  <t-select v-model="demandForm.keep_time_unit" :disabled="isAgent">
                    <t-option :value="item.value" :label="item.label" v-for="item in keepArr"
                      :key="item.value"></t-option>
                  </t-select>
                </t-form-item>
                <t-tooltip :content="lang.mf_demand_tip12" :show-arrow="false" theme="light" placement="top-left">
                  <t-icon name="help-circle"></t-icon>
                </t-tooltip>
              </t-form-item>
              <t-form-item :label="lang.mf_demand_tip4" name="keep_time_billing_item">
                <t-select v-model="demandForm.keep_time_billing_item" multiple :min-collapsed-num="1"
                  :disabled="isAgent">
                  <t-option :value="item.name" :label="item.value" v-for="item in demandItem"
                    :key="item.name"></t-option>
                </t-select>
              </t-form-item>
              <t-form-item :label="lang.mf_initial_fee" name="initial_fee">
                <t-input-number theme="normal" :min="0" :decimal-places="2" :label="currency_prefix"
                  @change="resetMinNum($event, 'initial_fee')" v-model="demandForm.initial_fee"
                  :placeholder="lang.mf_demand_tip15"></t-input-number>
              </t-form-item>
              <t-form-item :label="lang.mf_demand_tip5" class="radio-item">
                <t-radio-group v-model="demandForm.client_auto_delete" :options="radioOption"></t-radio-group>
              </t-form-item>
              <t-form-item class="radio-item">
                <template slot="label">
                  <t-tooltip :content="lang.mf_demand_tip7" :show-arrow="false" theme="light" placement="top-left">
                    <span>{{lang.mf_demand_tip6}}</span>
                    <t-icon name="help-circle"></t-icon>
                  </t-tooltip>
                </template>
                <t-radio-group v-model="demandForm.on_demand_to_duration" :options="radioOption"></t-radio-group>
              </t-form-item>
              <t-form-item class="radio-item">
                <template slot="label">
                  <t-tooltip :content="lang.mf_demand_tip9" :show-arrow="false" theme="light" placement="top-left">
                    <span>{{lang.mf_demand_tip8}}</span>
                    <t-icon name="help-circle"></t-icon>
                  </t-tooltip>
                </template>
                <t-radio-group v-model="demandForm.duration_to_on_demand" :options="radioOption"></t-radio-group>
              </t-form-item>
              <t-form-item :label="lang.mf_demand_tip10" class="radio-item">
                <t-radio-group v-model="demandForm.credit_limit_pay" :options="radioOption"></t-radio-group>
              </t-form-item>
        ...
        <t-tab-panel value="calc" :label="lang.mf_calc">
          <div class="box limit-table" v-if="lang">
            <div class="com-top">
              <p class="tit first-text-color">{{lang.cpu_config}}</p>
              <t-button class="add-btn" @click="addCalc('cpu')" :disabled="isAgent">
                {{lang.order_text53}}CPU
              </t-button>
            </div>
            <t-table row-key="id" :data="cpuList" size="medium" :columns="cpuColumns" :hover="hover"
              ...
              <template #price="{row}">
                <p v-if="payType !== 'on_demand'">
                  <span>{{currency_prefix}}{{(row.price*1).toFixed(2)}}</span>
                  <template v-if="row.duration">/{{row.duration}}</template>
                  &nbsp;{{lang.rise}}
                </p>
                <p v-if="calcShowDemand">
                  {{lang.mf_demand}}：{{currency_prefix}}{{(row.on_demand_price*1).toFixed(4)}}/{{lang.mf_hour}}
                </p>
              </template>
              <template #op="{row}">
                <div class="com-opt">
            ...
            <div class="com-top" style="margin-top: 30px">
              <p class="tit first-text-color">
                {{lang.memory_config}}
                <template v-if="memoryType">-{{memoryType}}{{lang.mf_way}}</template>
              </p>
              <t-button class="add-btn" @click="addCalc('memory')" :disabled="isAgent">
                {{lang.order_text53}}{{lang.memory}}
              </t-button>
            </div>
            <t-table row-key="id" :data="memoryList" size="medium" :columns="calcMemeryColumns" :hover="hover"
              ...
              <template #value="{row}">
                <template v-if="row.type === 'radio'">{{row.value}}</template>
                <template v-else>{{row.min_value}}-{{row.max_value}}</template>
              </template>
              <template #price="{row}">
                <p v-if="payType !== 'on_demand'">
                  <span>{{currency_prefix}}{{(row.price*1).toFixed(2)}}</span>
                ...
                <p v-if="calcShowDemand">
                  {{lang.mf_demand}}：{{currency_prefix}}{{(row.on_demand_price*1).toFixed(4)}}/{{lang.mf_hour}}
                  <template v-if="row.type !== 'radio'">&nbsp;{{lang.rise}}</template>
                </p>
              </template>
              <template #op="{row}">
                <div class="com-opt">
        ...
        <t-tab-panel value="data_center" :label="lang.data_center_config">
          <div class="box data-box">
            <div class="com-top">
              <p class="tit first-text-color">{{lang.data_center}}</p>
              <div class="r-btn">
                <t-button class="add-btn" @click="addData" :disabled="isAgent">
                  {{lang.invoice_btn6}}{{lang.data_center}}
                </t-button>
                <t-button class="add-btn" @click="addLine" :disabled="isAgent">
                  {{lang.invoice_btn6}}{{lang.line}}
                </t-button>
                <t-button theme="default" class="add-btn com-gray-btn" @click="handleGlobal" :disabled="isAgent"
                  v-if="hasFirewall && isFirewallProduct">
                  {{lang.mf_tip49}}
                </t-button>
              </div>
            </div>
                ...
                <template v-if="row.line.length > 0">
                  <div class="item line-item" v-for="(item,index) in row.line">
                    ...
                    <t-switch :custom-value="[0,1]" v-model="item.hidden" :label="[lang.mf_show, lang.mf_hide]"
                      @change="changeShowLine($event,item)"
                      :disabled="isAgent && item.upstream_hidden !== 0"></t-switch>
                  </div>
                </template>
            ...
            <div class="config-tip third-text-color">
        ...
        <t-tab-panel value="store" :label="lang.mf_store">
          <div class="box store-box limit-table1">
            <p class="t-top third-text-color">
              {{lang.mf_tip10}}
              <br />
              &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{{lang.mf_tip11}}
              <br />
            ...
            <div class="com-top">
              <p class="tit first-text-color">
                ...
                <template v-if="systemDisk.length >0">
                  -{{lang['mf_'+systemDisk[0].type]}}{{lang.mf_way}}
                </template>
              </p>
            ...
            <t-table row-key="id" :data="systemDisk" size="medium" :columns="systemDiskColumns" :hover="hover"
              ...
              <template #type="{row}">{{row.other_config.disk_type || lang.default}}</template>
              <template #op="{row}">
                <div class="com-opt">
            ...
            <div class="com-top" style="margin-top: 30px">
              <p class="tit first-text-color">
                ...
                <template v-if="dataDisk.length > 0">
                  -{{lang['mf_'+dataDisk[0].type]}}{{lang.mf_way}}
                </template>
              </p>
              <t-button class="add-btn" @click="addCalc('data_disk')" :disabled="isAgent">
                {{lang.order_text53}}{{lang.data_disk}}
              </t-button>
            </div>
            ...
            <div class="disk-limit-box">
              <p class="item free-disk-box">
                ...
                <t-switch :custom-value="[1,0]" v-model="otherForm.free_disk_switch" size="large"
                  @change="changeFreeDiskLimit($event,'switch')" :disabled="isAgent"></t-switch>
                <t-select v-model="otherForm.free_disk_type" :disabled="isAgent" @change="changeFreeDiskLimit"
                  ...
                  <t-option :key="item.value" :value="item.value" :label="item.name"
                    v-for="item in dataDiskType"></t-option>
                </t-select>
              ...
              <p class="item">
                ...
                <t-switch :custom-value="[1,0]" v-model="otherForm.disk_limit_switch" size="large"
            ...
            <p class="limit">
              ...
              <t-switch :custom-value="[1,0]" :value="store_limit" size="large" @change="changeLimit"
            ...
            <div v-show="store_limit">
              <div class="com-top">
                <p class="tit first-text-color">
                  {{lang.system_disk_nature}}
                  <template v-if="systemType">-{{systemType}}{{lang.mf_way}}</template>
                </p>
              ...
              <t-table row-key="id" :data="systemLimitList" size="medium" :columns="natureColumns" :hover="hover"
                ...
                <template #capacity_size="{row}">{{row.min_value}}-{{row.max_value}}</template>
                <template #price="{row}">
                  <span>{{currency_prefix}}{{row.price}}</span>
                  &nbsp;{{lang.rise}}
                </template>
                <template #type="{row}">{{row.other_config.disk_type}}</template>
                <template #op="{row}">
                  <div class="com-opt">
        ...
        <t-tab-panel value="limit" :label="lang.advanced_rules">
          <div class="box limit-box">
            <div class="com-top">
              <t-button class="add-btn" @click="addLimit" :disabled="isAgent">
                {{lang.order_text53}}{{lang.mf_rule}}
              </t-button>
              <span class="limit-tip third-text-color">
                    ...
                    <span v-if="item[0] === 'memory'">
                      <span v-for="(sub,sInd) in item[1]" :key="sInd">
                        <template v-if="memoryList[0]?.type !== 'radio'">
                          {{sub.min}}{{memory_unit}}~{{sub.max ? sub.max + memory_unit :
                          lang.mf_limitless}}；
                        </template>
                        <template v-else>
                          <span v-for="(el, val) in sub.value" :key="val">
        ...
        <t-tab-panel value="system" :label="lang.opt_system">
          <div class="box">
            <div class="top-opt">
              <div class="item">
                <t-button @click="classManage" :disabled="isAgent">
                  {{lang.news_classific}}{{lang.manage}}
                </t-button>
                <t-button theme="danger" class="ml_10" @click="deleteBatch" :disabled="isAgent">
                  {{lang.batch_delete}}
                </t-button>
                <t-button @click="handelPullOs('net')" theme="default" class="ml_10 com-gray-btn" :disabled="isAgent">
                  {{lang.get_system}}
                </t-button>
                <t-button @click="handelPullOs('local')" theme="default" class="ml_10 com-gray-btn" :disabled="isAgent">
                  {{lang.get_local_system}}
                </t-button>
                <t-button theme="default" class="ml_10 com-gray-btn" @click="createNewSys"
              ...
              <div class="item">
                <t-select v-model="systemParams.image_group_id" class="select"
                  ...
                  <t-option :key="item.id" :value="item.id" :label="item.name" v-for="item in systemGroup"></t-option>
                </t-select>
            ...
            <t-table row-key="id" :data="systemList" size="medium" :columns="systemColumns" :hover="hover"
              :loading="loading" table-layout="fixed" display-type="fixed-width" :hide-sort-tips="true"
              ...
              <template #charge="{row}">
                <t-switch :custom-value="[1,0]" v-model="row.charge" @change="changeSystemStatus(row)"
                  :disabled="isAgent"></t-switch>
              </template>
              <template #price="{row}">{{currency_prefix}}&nbsp;{{row.price}}</template>
              <template #enable="{row}">
                <t-switch :custom-value="[1,0]" v-model="row.enable" :disabled="isAgent"
                  @change="changeSystemStatus(row)"></t-switch>
              </template>
              <template #op="{row}">
                <div class="com-opt">
        ...
        <t-tab-panel value="recommend" :label="lang.package_config">
          <div class="box">
            <div class="com-top">
              <p class="tit first-text-color">{{lang.package_config}}</p>
              <div class="right">
                <t-button class="add-btn" @click="addRecommend" :disabled="isAgent">
                  {{lang.order_text53}}
                </t-button>
                <t-button class="add-btn com-gray-btn" theme="default" @click="handlerRange">
                  {{lang.upgrade_range}}
                </t-button>
              </div>
            </div>
            <t-table row-key="id" :data="recommendList" size="medium" :columns="calcModelColumns" :hover="hover"
              ...
              <template #price="{row}">
                ...
                <template v-if="calcShowDemand">
                  <p>
                    {{lang.mf_demand}}：{{currency_prefix}}{{(row.on_demand_price*1).toFixed(4)}}/{{lang.mf_hour}}
                  </p>
                  <p v-if="row.bill_type === 'flow'">
                    {{lang.mf_flow_demand}}：{{currency_prefix}}{{(row.on_demand_flow_price*1).toFixed(4)}}/GB
                  </p>
                </template>
              </template>
              <template #hidden="{row}">
              ...
              <template #ontrial_price="{row}">
                <span v-if="row.ontrial === 1">
                  {{currency_prefix}}{{(row.ontrial_price*1).toFixed(2)}}
                </span>
                <span v-else>--</span>
              </template>
              <template #ontrial_qty="{row}">
                <span v-if="row.ontrial === 1 && row.ontrial_stock_control === 1">
                  {{row.ontrial_qty}}
                </span>
                <span v-else>--</span>
              </template>
            ...
            <div class="bot-tip">
              <p>
                ...
                <t-switch :custom-value="[1,0]" v-model="otherForm.no_upgrade_tip_show" size="large"
                  @change="changeUpgradeTip" :disabled="isAgent"></t-switch>
              </p>
              <p>
                {{lang.mf_only_package}}
                <t-switch :custom-value="[1,0]" v-model="otherForm.only_sale_recommend_config" size="large"
                  @change="changeOnlyPackage" :disabled="isAgent"></t-switch>
                <span class="s-tip third-text-color">{{lang.mf_package_tip1}}</span>
              </p>
              <p class="s-tip third-text-color">{{lang.mf_package_tip2}}</p>
            </div>
          </div>
        </t-tab-panel>
        <!-- 试用设置 -->
        <t-tab-panel value="ontrial" :label="lang.ontrial_text1" v-if="!isAgent && payType !== 'on_demand'">
            ...
            <t-form label-width="160px" label-align="left" :data="payOntrialForm" ref="payOntrialRef"
              ...
              <div class="ontrial-open">
                ...
                <t-switch :custom-value="[1,0]" v-model="payOntrialForm.status"
                  :label="[lang.ontrial_text2,lang.ontrial_text3]"></t-switch>
              </div>
              <template v-if="payOntrialForm.status === 1">
                <t-form-item :label="lang.ontrial_text5">
                  <t-form-item label=" " label-width="0px" style="margin-right: 0">
                    <t-input-number style="width: 120px" v-model="payOntrialForm.cycle_num" name="cycle_num"
                      :placeholder="lang.ontrial_text5" theme="normal" :min="0" :decimal-places="0"></t-input-number>
                  </t-form-item>
                ...
                <t-form-item :label="lang.ontrial_text6">
                  <t-radio-group v-model="payOntrialForm.client_limit">
                    <t-radio :value="item.value" v-for="item in clientLimitOptions" :key="item.value">
                      {{item.label}}
                    </t-radio>
                  </t-radio-group>
                </t-form-item>
                <t-form-item :label="lang.ontrial_text13" :help="lang.ontrial_text14">
                  <t-checkbox-group v-model="payOntrialForm.account_limit">
                    <t-checkbox :value="item.value" v-for="item in accountLimitOptions" :key="item.value">
                      {{item.label}}
                    </t-checkbox>
                  </t-checkbox-group>
                </t-form-item>
                <t-form-item :label="lang.ontrial_text15" :help="lang.ontrial_text16">
                  <com-tree-select @choosepro="chooseDelPro" :multiple="true" style="width: 360px"
                    :value="payOntrialForm.old_client_exclusive"></com-tree-select>
                </t-form-item>
                <t-form-item :label="lang.ontrial_text17">
                  <t-input-number v-model="payOntrialForm.max" name="max" style="width: 200px"
                    ...
                    :suffix="lang.ontrial_text18"></t-input-number>
                </t-form-item>
              </template>
              <t-form-item label=" " label-width="0px">
            ...
            <t-form :data="otherForm" ref="otherConfig" @submit="submitConfig" :rules="dataRules" class="other-form">
                  ...
                  <t-form-item :label="`${lang.confirm}${lang.version}`" label-align="top">
                    <template slot="label">
                      <div class="custom-label">
                        <span>{{lang.confirm}}{{lang.version}}</span>
                        <t-form-item label="">
                          <t-checkbox v-model="otherForm.manual_manage" :disabled="isAgent">
                            {{lang.manual_manage_product}}
                          </t-checkbox>
                ...
                <template v-if="otherForm.type !== 'hyperv'">
                  <t-col :xs="12" :xl="4">
                    <t-form-item :label="`NAT${lang.forward}`" label-align="top">
                      <template slot="label">
                        <div class="custom-label">
                          <span>NAT{{lang.forward}}</span>
                          <t-form-item label="">
                            <t-checkbox v-model="otherForm.default_nat_acl" :disabled="isAgent">
                              {{lang.default_nat_acl}}
                            </t-checkbox>
                            <t-tooltip :content="lang.default_nat_acl_tip" :show-arrow="false" theme="light"
                              placement="top-left">
                              <t-icon name="help-circle"></t-icon>
                            </t-tooltip>
                          </t-form-item>
                  ...
                  <t-col :xs="12" :xl="4">
                    <t-form-item :label="lang.nat_web" label-align="top">
                      <template slot="label">
                        <div class="custom-label">
                          <span>{{lang.nat_web}}</span>
                          <t-form-item label="">
                            <t-checkbox v-model="otherForm.default_nat_web" :disabled="isAgent">
                              {{lang.default_nat_web}}
                            </t-checkbox>
                            <t-tooltip :content="lang.default_nat_web_tip" :show-arrow="false" theme="light"
                              placement="top-left">
                              <t-icon name="help-circle"></t-icon>
                            </t-tooltip>
                          </t-form-item>
                      ...
                      <t-input v-model="otherForm.nat_web_limit" :disabled="isAgent"
                        :placeholder="lang.nat_web"></t-input>
                    </t-form-item>
                  </t-col>
                </template>
              </t-row>
              <template v-if="otherForm.type !== 'hyperv'">
                <t-row :gutter="{ xs: 0, sm: 20, md: 40, lg: 40, xl: 40, xxl: 40 }">
                  ...
                  <t-col :xs="12" :xl="4">
                    <t-form-item :label="lang.enable_rules" label-align="top">
                  ...
                  <t-col :xs="12" :xl="4">
                    <t-form-item :label="lang.mf_tip48" label-align="top">
                      <t-select v-model="otherForm.duration_id" :disabled="isAgent" multiple clearable
                        :min-collapsed-num="1">
                        <t-option v-for="item in cycleData" :value="item.id" :label="item.name"
              ...
              <!-- 嵌套虚拟化 -->
              <t-row :gutter="{ xs: 0, sm: 20, md: 40, lg: 40, xl: 40, xxl: 40 }" class="end-switch">
                <template v-if="otherForm.type !== 'hyperv'">
                  <t-col :xs="12" :xl="3">
                    <t-form-item :label="lang.virtualization">
                      ...
                      <t-switch :custom-value="[1,0]" v-model="otherForm.ip_mac_bind" size="large"
                        :disabled="isAgent"></t-switch>
                    </t-form-item>
                  </t-col>
                  <t-col :xs="12" :xl="3">
                    <t-form-item :label="lang.default_ipv4">
                      <template slot="label">
                        {{lang.default_ipv4}}
                        <t-tooltip :content="lang.default_ipv4_tip" :show-arrow="false" theme="light"
                          placement="top-left">
                          <t-icon name="help-circle"></t-icon>
                        </t-tooltip>
                      </template>
                      <t-switch :custom-value="[1,0]" v-model="otherForm.default_one_ipv4" size="large"
                        :disabled="isAgent"></t-switch>
                    </t-form-item>
                  </t-col>
                </template>
                <t-col :xs="12" :xl="3">
                  <t-form-item :label="lang.is_need_code1">
                    <t-switch :custom-value="[1,0]" v-model="otherForm.reset_password_sms_verify"
                      size="large"></t-switch>
                  </t-form-item>
                </t-col>
                <t-col :xs="12" :xl="3">
                  <t-form-item :label="lang.is_need_code2">
                    <t-switch :custom-value="[1,0]" v-model="otherForm.reinstall_sms_verify" size="large"></t-switch>
                  </t-form-item>
                </t-col>
                <t-col :xs="12" :xl="3" v-if="otherForm.type !== 'hyperv'">
                  <t-form-item :label="lang.allow_ssh">
                    <template slot="label">
                      <span>{{lang.allow_ssh}}</span>
                      <t-tooltip :content="lang.ssh_tip" :show-arrow="false" theme="light" placement="top-left">
                        <t-icon name="help-circle"></t-icon>
                      </t-tooltip>
                    </template>
                    <t-switch :custom-value="[1,0]" v-model="otherForm.support_ssh_key" size="large"
                      :disabled="isAgent"></t-switch>
                  </t-form-item>
                </t-col>
                <t-col :xs="12" :xl="9">
                  <!-- ssh端口 -->
                  <t-form-item label-align="left" class="ssh-port">
                    <template #label>
                      ...
                      <p class="item">
                        ...
                        <t-form-item label="" :required-mark="false" name="rand_ssh_port_start">
                          <t-input-number v-model="otherForm.rand_ssh_port_start" :placeholder="lang.start_port"
                            ...
                            @blur="changeNum($event, 100, 65534, 'rand_ssh_port_start')"
                            :disabled="isAgent"></t-input-number>
                        </t-form-item>
                      </p>
                      <p class="item">
                        ...
                        <t-form-item label="" :required-mark="false" name="rand_ssh_port_end">
                          <t-input-number v-model="otherForm.rand_ssh_port_end" :placeholder="lang.end_port"
                            theme="normal" :min="101" :max="65535" :decimal-places="0" :disabled="isAgent"
                            @blur="changeNum($event, 101, 65535, 'rand_ssh_port_end')"></t-input-number>
                        </t-form-item>
                      </p>
                    </div>
                    <!-- 指定端口 -->
              ...
              <span class="gap-line"></span>
              <t-row :gutter="{ xs: 0, sm: 20, md: 40, lg: 40, xl: 40, xxl: 40 }" v-if="otherForm.type === 'host'">
                <t-col :xs="12" :xl="12">
                  <t-form-item :label="lang.net_type">
                    <div class="net_type second-text-color">
                      <div class="check">
                        <t-checkbox v-model="otherForm.support_normal_network" :disabled="isAgent"
                          class="second-text-color">
                          {{lang.normal_network}}
                        </t-checkbox>
                      </div>
                      <div class="check">
                        <t-checkbox v-model="otherForm.support_vpc_network" :disabled="isAgent"
                          class="second-text-color">
                          {{lang.vpc_network}}
                        </t-checkbox>
                        <div v-if="otherForm.support_vpc_network" class="public_ip">
                          <t-switch :custom-value="[1,0]" v-model="otherForm.support_public_ip" size="large"
                            disabled></t-switch>
              ...
              <span class="gap-line"></span>
              <t-row :gutter="{ xs: 0, sm: 20, md: 40, lg: 40, xl: 40, xxl: 40 }">
                <t-col :xs="12" :xl="12" class="flex">
                  <t-form-item :label="lang.mf_demand_tip18">
                    <template slot="label">
                      {{lang.mf_demand_tip18}}
                      <t-tooltip :content="lang.mf_demand_tip19" :show-arrow="false" theme="light" placement="top-left">
                        <t-icon name="help-circle"></t-icon>
                      </t-tooltip>
                    </template>
                    <t-switch :custom-value="[1,0]" v-model="otherForm.custom_rand_password_rule" size="large"
                      :disabled="isAgent" class="ml32"></t-switch>
                  </t-form-item>
                  <t-form-item :label="lang.mf_demand_tip20" name="default_password_length"
                    v-if="otherForm.custom_rand_password_rule" :required-mark="false"
                    class="password-length second-text-color">
                    <t-input-number v-model="otherForm.default_password_length" style="width: auto" :min="6" :max="20"
                      theme="normal" :decimal-places="0" :disabled="isAgent"
                      :placeholder="lang.mf_demand_tip22"></t-input-number>
                    <span class="third-text-color ml8">{{lang.mf_demand_tip21}}</span>
                  </t-form-item>
                </t-col>
              </t-row>
              <span class="gap-line"></span>
              <t-row :gutter="{ xs: 0, sm: 20, md: 40, lg: 40, xl: 40, xxl: 40 }">
                <t-col :xs="12" :xl="12">
                  <!-- 备份管理 -->
                  <t-form-item :label="lang.backup_manage" class="backup-item">
                    <t-switch :custom-value="[1,0]" v-model="otherForm.backup_enable" size="large"
                      :disabled="isAgent"></t-switch>
                    <!-- <t-tooltip :content="lang.backup_tip" :show-arrow="false" theme="light" placement="top-left">
                  <t-icon name="help-circle"></t-icon>
                </t-tooltip> -->
                  </t-form-item>
                  <t-table row-key="id" :data="backList" size="medium" :columns="calcBakcColums('backup')"
                    :hover="hover" :loading="backLoading" :table-layout="tableLayout ? 'auto' : 'fixed'"
                    display-type="fixed-width" :hide-sort-tips="true" class="back-table" :loading="backLoading"
                    v-show="otherForm.backup_enable" :class="{demand: calcShowDemand}">
                    <template #num="{row}">
                      <t-input-number v-model="row.num" theme="column" :min="1" :max="999" style="width: 150px" auto
                        :disabled="!row.status"></t-input-number>
                    </template>
                    <template #id="{row,rowIndex}">{{rowIndex + 1}}</template>
                    <template #price="{row,rowIndex}">
                      <t-input-number v-model="row.price" theme="column" :min="0" :step="1.00" :decimal-places="2"
                        :disabled="!row.status" :label="currency_prefix" style="width: 150px"></t-input-number>
                      <template v-if="!isAgent && !calcShowDemand">
                    ...
                    <template #on_demand_price="{row,rowIndex}">
                      <t-input-number v-model="row.on_demand_price" theme="column" :min="0" :step="1.00"
                        :decimal-places="4" :disabled="!row.status" :label="currency_prefix" style="width: 150px"
                        :suffix="`/${lang.mf_hour}`"></t-input-number>
                      <template v-if="!isAgent && calcShowDemand">
                        <div class="com-icon" v-if="rowIndex === backList.length-1 && backList.length < 5"
                          @click="addGroup('backup')">
                          <t-icon name="add"></t-icon>
                    ...
                    <template #available="{row}">
                      <t-switch :custom-value="[1,0]" v-model="row.enable" @click.native="changeSwitch(row)"></t-switch>
                    </template>
                  </t-table>
                </t-col>
              </t-row>
              ...
              <template v-if="otherForm.type !== 'hyperv'">
                <t-row :gutter="{ xs: 0, sm: 20, md: 40, lg: 40, xl: 40, xxl: 40 }">
                  <t-col :xs="12" :xl="12">
                    <t-form-item :label="lang.snap_manage" class="backup-item">
                      <t-switch :custom-value="[1,0]" v-model="otherForm.snap_enable" size="large"
                        :disabled="isAgent"></t-switch>
                      <!-- <t-tooltip :content="lang.snap_tip" :show-arrow="false" theme="light" placement="top-left">
                    <t-icon name="help-circle"></t-icon>
                  </t-tooltip> -->
                    </t-form-item>
                    <t-table row-key="id" :data="snapList" size="medium" :columns="calcBakcColums('snap')"
                      :hover="hover" :loading="snapLoading" :table-layout="tableLayout ? 'auto' : 'fixed'"
                      display-type="fixed-width" :hide-sort-tips="true" class="back-table"
                      v-show="otherForm.snap_enable" :class="{demand: calcShowDemand}">
                      <template #num="{row}">
                        <t-input-number v-model="row.num" theme="column" :min="1" :max="999" style="width: 150px"
                          :disabled="!row.status"></t-input-number>
                      </template>
                      <template #id="{row,rowIndex}">{{rowIndex + 1}}</template>
                      <template #price="{row,rowIndex}">
                        <t-input-number v-model="row.price" theme="column" :min="0" :step="1.00" :decimal-places="2"
                          style="width: 150px" :disabled="!row.status" :label="currency_prefix"></t-input-number>
                        <template v-if="!isAgent && !calcShowDemand">
                      ...
                      <template #on_demand_price="{row,rowIndex}">
                        <t-input-number v-model="row.on_demand_price" theme="column" :min="0" :step="1.00"
                          :decimal-places="4" style="width: 150px" :disabled="!row.status" :label="currency_prefix"
                          :suffix="`/${lang.mf_hour}`"></t-input-number>
                        <template v-if="!isAgent && calcShowDemand">
                          <div class="com-icon" v-if="rowIndex === snapList.length-1 && snapList.length < 5"
                            @click="addGroup('snap')">
                      ...
                      <template #available="{row}">
                        <t-switch :custom-value="[1,0]" v-model="row.enable"
                          @click.native="changeSwitch(row)"></t-switch>
                      </template>
                    </t-table>
                  </t-col>
                </t-row>
              ...
              <template v-if="otherForm.is_agent">
                <t-form-item :label="lang.agent_docking"></t-form-item>
                ...
                <t-table row-key="id" :data="resourceList" size="medium" :columns="resourceColumns" :hover="hover"
                  ...
                  :hide-sort-tips="true" class="back-table" style="margin-bottom: 40px" v-else>
                  <template #rid="{row}">
                    <t-input v-model="row.rid" :disabled="!row.status"></t-input>
                  </template>
                  <template #id="{row,rowIndex}">{{rowIndex + 1}}</template>
                  <template #name="{row,rowIndex}">
                    <t-input v-model="row.name" :disabled="!row.status"></t-input>
                    <template v-if="row.status">
              ...
              <t-popconfirm :visible="showConfirm" theme="default" :content="deleteTip">
                <t-button theme="primary" type="submit" :loading="submitLoading" class="submit"
                  :style="{'margin-top': otherForm.snap_enable ?  '30px' : '5px'}">
                  {{lang.hold}}
                </t-button>
                <t-button slot="confirmBtn" size="small" theme="primary" @click="handlerConfig">
                  {{lang.sure}}
                </t-button>
      ...
      <t-form :data="cycleForm" ref="cycleForm" @submit="submitCycle" :rules="cycleRules" class="cycle-form"
        ...
        <t-form-item :label="lang.cycle_time" class="required">
          ...
          <t-form-item :required-mark="false">
            <t-select v-model="cycleForm.unit" :disabled="isAgent">
              <t-option :key="item.value" :value="item.value" :label="item.label" v-for="item in cycleTime"></t-option>
            </t-select>
          </t-form-item>
        </t-form-item>
        <t-form-item :label="lang.price_factor" name="price_factor">
          <t-input-number v-model="cycleForm.price_factor" :placeholder="lang.input+lang.factor" theme="normal"
            :decimal-places="2" :min="0" :max="9999"></t-input-number>
        </t-form-item>
        ...
        <t-form-item :label="lang.cycle_price">
          <t-input-number v-model="cycleForm.price" :placeholder="lang.input+lang.price" theme="normal"
            :decimal-places="2" :min="0"></t-input-number>
        </t-form-item>
        <div class="com-f-btn">
          <t-button theme="primary" type="submit" :loading="submitLoading">{{lang.hold}}</t-button>
          <t-button theme="default" variant="base" @click="cycleModel=false">
            {{lang.cancel}}
          </t-button>
        </div>
      </t-form>
    ...
    <t-dialog :header="lang.cycle_ratio" :visible.sync="ratioModel" :footer="false" width="600"
      ...
      <div class="com-f-btn">
        <t-button theme="primary" @click="saveRatio" :loading="submitLoading">
          {{lang.hold}}
        </t-button>
        <t-button theme="default" variant="base" @click="ratioModel=false">
          {{lang.cancel}}
        </t-button>
      </div>
    </t-dialog>
    ...
    <t-dialog :header="comTitle" :visible.sync="systemModel" :footer="false" width="600" @close="closeData"
      ...
      <t-form :data="createSystem" ref="cycleForm" @submit="submitSystem" :rules="cycleRules" class="system-form"
        ...
        <t-form-item :label="lang.mf_charge">
          <t-switch :custom-value="[1,0]" v-model="createSystem.charge" :disabled="isAgent"></t-switch>
        </t-form-item>
        ...
        <t-form-item :label="lang.mf_enable">
          <t-switch :custom-value="[1,0]" v-model="createSystem.enable" :disabled="isAgent"></t-switch>
        </t-form-item>
        <div class="com-f-btn">
          <t-button theme="primary" type="submit" :loading="submitLoading">{{lang.hold}}</t-button>
          <t-button theme="default" variant="base" @click="systemModel=false">
            {{lang.cancel}}
          </t-button>
        </div>
      </t-form>
    ...
    <t-dialog :header="`${lang.classific_manage}`" :visible.sync="classModel" :footer="false" width="600"
      ...
      <t-form :data="classParams" ref="classForm" @submit="submitSystemGroup" :rules="cycleRules" v-if="classModel">
        ...
        <t-form-item :label="lang.mf_icon" name="icon">
          ...
          <t-button theme="primary" type="submit" :loading="submitLoading" style="margin-left: 14px">
            {{lang.hold}}
          </t-button>
        </t-form-item>
      </t-form>
    ...
    <t-dialog :header="comTitle" :visible.sync="calcModel" :footer="false" width="600" @close="closeData"
      ...
      <t-form :data="calcForm" ref="cycleForm" @submit="submitCalc" :rules="calcRules" class="cycle-form"
        ...
        <t-form-item :label="`${lang.config}${lang.mf_way}`" name="type" v-if="calcType !== 'cpu'" class="mem-congfig">
          <t-form-item>
            <t-select v-model="calcForm.type" :placeholder="`${lang.select}${lang.config}${lang.mf_way}`"
              ...
              <t-option :key="item.value" :value="item.value" :label="item.label" v-for="item in configType"></t-option>
            </t-select>
          </t-form-item>
          <t-form-item>
        ...
        <!-- 周期价格 -->
        <t-form-item :label="lang.price_config" class="price-form-item">
          <div class="price-box" v-if="payType !== 'on_demand'">
            ...
            <div class="item">
              <p class="txt"></p>
              <p class="num">
                <t-button size="small" :disabled="isShowFill(calcForm.price)"
                  @click="autoFill('calcForm',calcForm.price)">
                  {{lang.autofill}}
                </t-button>
              </p>
            </div>
          </div>
          <div class="price-box" v-if="calcShowDemand">
            <div class="item">
              <p class="txt">{{lang.mf_demand}}</p>
              <p class="num">
                <t-form-item name="on_demand_price"
                  :ruels="[ calcForm.on_demand_price * 1 ? calcRules.price : {required: true} ]">
                  <t-input v-model="calcForm.on_demand_price" :placeholder="lang.input+lang.money"
                    @blur="blurDemandPrice($event, 'calcForm')" :suffix="`/${lang.mf_hour}`"></t-input>
                </t-form-item>
              </p>
            </div>
            <!-- 自动填充 -->
        ...
        <div class="com-f-btn">
          <t-button theme="primary" type="submit" :loading="submitLoading">{{lang.hold}}</t-button>
          <t-button theme="default" variant="base" @click="calcModel=false">
            {{lang.cancel}}
          </t-button>
        </div>
      </t-form>
    ...
    <t-dialog :header="comTitle" :visible.sync="natureModel" :footer="false" width="600" @close="closeData"
      ...
      <t-form :data="calcForm" ref="calcForm" @submit="submitNature" :rules="calcRules" class="cycle-form"
        ...
        <div class="com-f-btn">
          <t-button theme="primary" type="submit" :loading="submitLoading" :disabled="isAgent">
            {{lang.hold}}
          </t-button>
          <t-button theme="default" variant="base" @click="natureModel=false">
            {{lang.cancel}}
          </t-button>
        </div>
      </t-form>
        ...
        <t-form-item :label="lang.cloud_config" name="cloud_config">
          <t-form-item name="cloud_config" class="configs" style="margin-right: 10px; margin-bottom: 0"
            ...
            <t-select v-model="dataForm.cloud_config" class="flow-type" @change="changeType" :disabled="isAgent">
              <t-option :key="item.value" :value="item.value" :label="item.lable" v-for="item in dataConfig"></t-option>
            </t-select>
          </t-form-item>
          <t-form-item name="cloud_config_id" class="configs" :required-mark="false"
            ...
            <t-input v-model="dataForm.cloud_config_id" :placeholder="lang.input+lang[dataForm.cloud_config] + 'ID'"
              class="flow" :disabled="isAgent"></t-input>
          </t-form-item>
        </t-form-item>
          ...
          <div class="con">
            <div class="c-item">
              ...
              <t-table row-key="id" :data="gpuForm.gpu_data" size="medium" :columns="calcColums('line_gpu')"
                ...
                <template #fir="{row}">{{row.value}}</template>
                <template #price="{row}">
                  <p>
                    {{currency_prefix}}
                    <template v-if="lineType === 'add'">{{calcPrice(row.price)}}</template>
                    <template v-else>
              ...
              <div class="opt" v-if="lineRight">
                <t-form :data="gpuForm" ref="cycleForm" @submit="submitGpuSub" :rules="calcRules" class="sub-form"
                  ...
                  <div class="scroll">
                    ...
                    <!-- 周期价格 -->
                    <t-form-item :label="lang.price" class="price-form-item">
                      <div class="price-box">
                        ...
                        <div class="item">
                          <p class="txt"></p>
                          <p class="num">
                            <t-button size="small" :disabled="isShowFill(gpuForm.price)"
                              @click="autoFill('gpuForm',gpuForm.price)">
                              {{lang.autofill}}
                            </t-button>
                          </p>
                        </div>
                      ...
                      <div class="price-box" v-if="calcShowDemand">
                        <div class="item">
                          ...
                          <p class="num">
                            <t-form-item name="on_demand_price"
                              :ruels="[ gpuForm.on_demand_price * 1 ? calcRules.price : {required: true} ]">
                              <t-input v-model="gpuForm.on_demand_price" :placeholder="lang.input+lang.money"
                                @blur="blurDemandPrice($event, 'gpuForm')" :suffix="`/${lang.mf_hour}`"></t-input>
                            </t-form-item>
                          </p>
                        </div>
                        <!-- 自动填充 -->
                  ...
                  <div class="com-btn">
                    ...
                    <t-button theme="default" variant="base" @click="lineRight=false">
                      {{lang.back}}
                    </t-button>
                  </div>
                </t-form>
          ...
          <div class="bot-opt" :class="{'hide-opt': lineRight}">
            <div class="b-item">
              <div class="com-btn">
                ...
                <t-button theme="default" variant="base" @click="closeData">
                  {{lang.cancel}}
                </t-button>
              </div>
            </div>
      ...
      <t-form :data="defenceForm" ref="defenceForm" @submit="submitGlobalRule" :rules="dataRules" class="data-form"
        ...
        <div class="top">
          <t-form-item :label="lang.mf_tip50" label-width="auto">
            <t-switch :custom-value="[1,0]" :value="defenceForm.sync_firewall_rule" @change="changeSyncRule"></t-switch>
          </t-form-item>
        </div>
        <div class="config">
          <div class="con">
            <template v-if="defenceForm.sync_firewall_rule">
              <div class="c-item">
                ...
                <t-table row-key="id" :data="checkedFirewallRule" size="medium" :columns="calcColums('global_defence')"
                  ...
                  <template #fir="{row}">{{row.defense_peak}}</template>
                  ...
                  <template #op="{row,rowIndex}">
                    <div class="com-opt">
                      ...
                      <t-tooltip :content="lang.delete" :show-arrow="false" theme="light">
                        <t-icon name="delete" @click="delRuleItem(row)" :class="{'server-disabled': isAgent}"></t-icon>
                      </t-tooltip>
                    </div>
                  </template>
                </t-table>
                  ...
                  <t-form :data="gpuForm" ref="cycleForm" @submit="submitRuleSub" :rules="calcRules" class="sub-form"
                    ...
                    <div class="scroll">
                      ...
                      <template v-if="optType === 'add'">
                        ...
                        <p class="s-tip third-text-color">{{lang.mf_tip56}}</p>
                        <p class="s-tip third-text-color">{{lang.mf_tip57}}</p>
                        <p class="s-tip third-text-color">{{lang.mf_tip58}}</p>
                      </template>
                      <!-- 编辑 -->
                      <template v-else>
                        ...
                        <t-form-item :label="lang.price">
                          <div class="price-box">
                            ...
                            <div class="item">
                              <p class="txt"></p>
                              <p class="num">
                                <t-button size="small" :disabled="isShowFill(defenceForm.price)"
                                  @click="autoFill('defenceForm',defenceForm.price)">
                                  {{lang.autofill}}
                                </t-button>
                              </p>
                            </div>
                    ...
                    <div class="com-btn">
                      <t-button theme="primary" type="submit" :loading="submitLoading" v-show="optType !== 'add'">
                        {{lang.hold}}
                      </t-button>
                      <t-button theme="default" variant="base" @click="lineRight=false">
                        {{lang.back}}
          ...
          <div class="bot-opt" :class="{'hide-opt': lineRight && optType !== 'add'}">
            <div class="b-item">
              <div class="com-btn">
                ...
                <t-button theme="default" variant="base" @click="defenceModel = false">
                  {{lang.cancel}}
                </t-button>
              </div>
            </div>
    ...
    <!-- 新增/修改 线路 -->
    <t-dialog :header="lang.line" :visible.sync="lineModel" :footer="false" width="1250" @close="closeData"
      ...
      <t-form :data="lineForm" ref="lineForm" @submit="submitLine" :rules="dataRules" class="data-form"
            ...
            <t-form-item :label="lang.country_area" class="required" v-if="lineType === 'add'" label-width="120px">
              ...
              <t-form-item label="" name="city" class="configs" :required-mark="false">
                <t-select v-model="lineForm.city" class="flow-type" @change="changeCity"
                  ...
                  <t-option :key="c" :value="city.name" :label="city.name" v-for="(city, c) in calcCity"></t-option>
                </t-select>
              </t-form-item>
              <t-form-item label="" name="data_center_id" class="configs" :required-mark="false">
                <t-select v-model="lineForm.data_center_id" class="flow-type" :placeholder="lang.select+lang.area">
                  <t-option :key="area.id" :value="area.id" :label="area.name" v-for="area in calcArea"></t-option>
                </t-select>
              </t-form-item>
            </t-form-item>
          </div>
          <div class="sort">
            <t-form-item :label="lang.line_name" name="name" class="line-name" label-width="120px"
        ...
        <div class="config">
          <p class="p-tit">
            ...
            <t-checkbox v-model="lineForm.link_clone" :disabled="isAgent">
              {{lang.link_clone}}
            </t-checkbox>
          </p>
          <div class="con">
            ...
            <div class="c-item">
              ...
              <template v-if="lineForm.bill_type === 'bw'">
                ...
                <t-table row-key="id" :data="lineForm.bw_data" size="medium" :columns="bwColumns" :hover="hover"
                  ...
                  <template #price="{row}">
                    <!-- 新增线路的时候本地处理显示 -->
                    <template v-if="payType !== 'on_demand'">
                      {{currency_prefix}}
                      <template v-if="lineType === 'add'">{{calcPrice(row.price)}}</template>
                      <template v-else>
                        {{(row.price*1).toFixed(2)}}
              ...
              <template v-else>
                <template v-if="payType !== 'on_demand'">
                  <div class="com-top">
                    <p class="tit first-text-color">
                      ...
                      <!-- <template v-if="calcSubTitle(lineForm.flow_data)">{{calcSubTitle(lineForm.flow_data)}}</template> -->
                    </p>
                  ...
                  <t-table row-key="id" :data="lineForm.flow_data" size="medium" :columns="calcColums('flow')"
                    ...
                    <template #fir="{row}">{{row.value}}</template>
                    <template #out_bw="{row}">{{row.other_config?.out_bw}}</template>
                    <template #price="{row}">
                      <!-- 新增线路的时候本地处理显示 -->
                      <span>
                        {{currency_prefix}}
                        <template v-if="lineType === 'add'">
                          {{calcPrice(row.price)}}&nbsp;{{lang.rise}}
                        </template>
                        <template v-else>
                          {{(row.price*1).toFixed(2)}}
                          <template v-if="row.duration">/{{row.duration}}</template>
                ...
                <template v-if="calcShowDemand">
                  <div class="com-top">
                    <p class="tit first-text-color">
                      {{lang.traffic_billing}} - {{lang.mf_demand}}
                      <!-- <template v-if="calcSubTitle(lineForm.flow_data_on_demand)">
                        {{calcSubTitle(lineForm.flow_data_on_demand)}}
                      </template> -->
                    </p>
                    <div class="add-btn gray" @click="addLineSub('line_flow_on_demand')"
                      :class="{'server-disabled': isAgent}">
                      {{lang.order_text53}}
                    </div>
                  </div>
                  <t-table row-key="id" :data="lineForm.flow_data_on_demand" size="medium" :columns="calcColums('flow')"
                    :hover="hover" :loading="loading" :table-layout="tableLayout ? 'auto' : 'fixed'"
                    display-type="fixed-width" :hide-sort-tips="true">
                    <template slot="sortIcon">
                      <t-icon name="caret-down-small"></t-icon>
                    </template>
                    <template #fir="{row}">1</template>
                    <template #out_bw="{row}">{{row.other_config?.out_bw}}</template>
                    <template #price="{row}">
                      <!-- 新增线路的时候本地处理显示 -->
                      {{currency_prefix}}{{(row.on_demand_price*1).toFixed(4)}}/GB
                    </template>
                    <template #op="{row, rowIndex}">
                      <div class="com-opt">
                        <t-tooltip :content="lang.edit" :show-arrow="false" theme="light">
                          <t-icon name="edit-1" @click="editSubItem(row,rowIndex,'line_flow_on_demand')"></t-icon>
                        </t-tooltip>
                        <t-tooltip :content="lang.delete" :show-arrow="false" theme="light">
                          <t-icon name="delete" @click="comDel('line_flow_on_demand',row, rowIndex)"
                            :class="{'server-disabled': isAgent}"></t-icon>
                        </t-tooltip>
                      </div>
                    </template>
                ...
                <p class="s-tip third-text-color">{{lang.mf_tip21}}</p>
              </template>
              <!-- 防护价格配置 -->
              <div class="com-top">
                <p class="tit first-text-color">
                  <t-form-item :label="lang.protect_price_config">
                    <t-switch :custom-value="[1,0]" v-model="lineForm.defence_enable" size="small" :disabled="isAgent"
                      @change="changeLineDefenceEnable"></t-switch>
                  </t-form-item>
                </p>
                ...
                <p class="tit first-text-color">
                  <t-form-item :label="lang.mf_tip50">
                    <t-switch :custom-value="[1,0]" :value="lineForm.sync_firewall_rule" size="small"
                      :disabled="isAgent" @change="changeLineFirewallRule"></t-switch>
                  </t-form-item>
                </p>
              ...
              <t-table row-key="id" :data="lineForm.defence_data" size="medium" :columns="calcColums('defence')"
                ...
                <template #price="{row}">
                  ...
                  <p v-if="(calcShowDemand && !lineForm.sync_firewall_rule) || payType === 'on_demand'">
                    {{lang.mf_demand}}：{{currency_prefix}}{{(row.on_demand_price*1).toFixed(4)}}/{{lang.mf_hour}}
                  </p>
                </template>
                <template #op="{row,rowIndex}">
              ...
              <div class="com-top">
                <p class="tit first-text-color">
                  <t-form-item :label="lang.ipv6_config">
                    <t-switch :custom-value="[1,0]" v-model="lineForm.ipv6_enable" size="small"
                      :disabled="isAgent"></t-switch>
                  </t-form-item>
                </p>
              ...
              <div class="opt" v-if="lineRight">
                <t-form :data="subForm" ref="cycleForm" @submit="submitSub" :rules="calcRules" class="sub-form"
                  ...
                  <div class="scroll">
                    ...
                    <t-form-item :label="`${lang.billing_way}`" name="type"
                      ...
                      <t-select v-model="subForm.type" :placeholder="`${lang.select}${lang.billing_way}`"
                        ...
                        <t-option :key="item.value" :value="item.value" :label="item.label"
                          v-for="item in configType"></t-option>
                      </t-select>
                    </t-form-item>
                    <!-- line_defence -->
                      ...
                      <template v-else>
                        <template v-if="!isEditDefence">
                          <t-form-item :label="lang.defence + '（Gbps）'" name="value"
                            ...
                            <t-input v-model="subForm.value" :placeholder="lang.input+lang.defence"
                              :disabled="isAgent"></t-input>
                          </t-form-item>
                        </template>
                        <template v-else>
                    ...
                    <template v-if="subType === 'line_flow' || subType === 'line_flow_on_demand'">
                    ...
                    <!-- 周期价格 -->
                    <t-form-item :label="lang.price" v-if="isShowLinePrice" class="price-form-item">
                      <template v-if="payType !== 'on_demand'">
                        <div class="price-box" v-if="subType !== 'line_flow_on_demand'">
                          ...
                          <div class="item">
                            <p class="txt"></p>
                            <p class="num">
                              <t-button size="small" :disabled="isShowFill(subForm.price)"
                                @click="autoFill('subForm',subForm.price)">
                                {{lang.autofill}}
                              </t-button>
                            </p>
                          </div>
                      ...
                      <div class="price-box" v-if="calcShowDemand && subType !== 'line_flow'">
                        <template v-if="subType !== 'line_flow_on_demand'">
                          <div class="item">
                            <p class="txt">{{lang.mf_demand}}</p>
                            <p class="num">
                              <t-form-item name="on_demand_price"
                                :ruels="[ subForm.on_demand_price * 1 ? calcRules.price : {required: true} ]">
                                <t-input v-model="subForm.on_demand_price" :placeholder="lang.input+lang.money"
                                  @blur="blurDemandPrice($event, 'subForm')" :suffix="`/${lang.mf_hour}`"></t-input>
                              </t-form-item>
                            </p>
                          </div>
                          <!-- 自动填充 -->
                        ...
                        <template v-else>
                          <t-form-item name="on_demand_price" style="flex: 1"
                            :ruels="[ subForm.on_demand_price * 1 ? calcRules.price : {required: true} ]">
                            <t-input v-model="subForm.on_demand_price" :placeholder="lang.input+lang.money"
                              @blur="blurDemandPrice($event, 'subForm')" :suffix="`/GB`"></t-input>
                          </t-form-item>
                        </template>
                      </div>
                    </t-form-item>
                    ...
                    <template v-if="subType === 'line_flow' || subType === 'line_flow_on_demand'">
                      ...
                      <t-form-item :label="`${lang.billing_cycle}`" class="mem-congfig" name="other_config.bill_cycle"
                        ...
                        <t-select v-model="subForm.other_config.bill_cycle"
                          ...
                          <t-option :key="item.value" :value="item.value" :label="item.label"
                            v-for="item in billingCycle"></t-option>
                        </t-select>
                      </t-form-item>
                    </template>
                  ...
                  <div class="com-btn">
                    <t-button theme="primary" type="submit" :loading="submitLoading" v-show="isShowLinePrice">
                      {{lang.hold}}
                    </t-button>
                    <t-button theme="default" variant="base" @click="lineRight=false">
                      {{lang.back}}
    ...
    <t-dialog :header="comTitle" :visible.sync="recommendModel" :footer="false" width="1470" @close="closeData"
      ...
      <t-form :data="lineForm" ref="lineForm" @submit="submitRecommend" :rules="dataRules"
        ...
        <div class="form-item">
          ...
          <t-checkbox v-model="lineForm.due_not_free_gpu" :disabled="isAgent">
            {{lang.mf_tip47}}
          </t-checkbox>
          <!-- 网络配置 -->
          <p class="com-tit">{{lang.network_config}}</p>
          <p class="demand-flow" v-if="calcLineType==='flow'">{{lang.mf_demand_tip16}}</p>
          ...
          <div class="bot-tip third-text-color">
            <p>{{lang.mf_notes}}</p>
        ...
        <div class="form-item">
          ...
          <div class="short-item">
            ...
            <t-form-item :label="lang.line_name" name="line_id">
              <t-select v-model="lineForm.line_id" class="flow-type" @change="changeBillType"
                ...
                <t-option :key="line.id" :value="line.id" :label="line.name" v-for="line in calcSelectLine"></t-option>
              </t-select>
            </t-form-item>
          </div>
          <!-- 存储配置 -->
          ...
          <t-form-item :label="lang.price" class="price-form-item">
            <div class="price-box" v-if="payType !== 'on_demand'">
              ...
              <div class="item">
                <p class="txt"></p>
                <p class="num">
                  <t-button size="small" :disabled="isShowFill(lineForm.price)"
                    @click="autoFill('lineForm',lineForm.price)">
                    {{lang.autofill}}
                  </t-button>
                </p>
              </div>
            ...
            <div class="price-box" v-if="calcShowDemand">
              <div class="item">
                ...
                <p class="num">
                  <t-form-item name="on_demand_price"
                    :ruels="[ lineForm.on_demand_price * 1 ? calcRules.price : {required: true} ]">
                    <t-input v-model="lineForm.on_demand_price" :placeholder="lang.input+lang.money"
                      @blur="blurDemandPrice($event, 'lineForm')" :suffix="`/${lang.mf_hour}`"></t-input>
                  </t-form-item>
                </p>
              </div>
              <!-- 自动填充 -->
            ...
            <!-- 流量按需 -->
            <div class="price-box"
              v-if="calcLineType==='flow' && (payType == 'on_demand' || payType === 'recurring_prepayment_on_demand')">
              <div class="item">
                <p class="txt">{{lang.mf_flow_demand}}</p>
                <p class="num">
                  <t-form-item name="on_demand_flow_price"
                    :ruels="[ lineForm.on_demand_flow_price * 1 ? calcRules.price : {required: true} ]">
                    <t-input v-model="lineForm.on_demand_flow_price" :placeholder="lang.input+lang.money"
                      @blur="blurDemandPrice($event, 'lineForm', 'on_demand_flow_price')" :suffix="`/GB`"></t-input>
                  </t-form-item>
                </p>
              </div>
            </div>
          ...
          <template v-if="!isAgent">
            ...
            <div class="short-item store-config">
              <div class="short-item">
                <div class="ontrial-open">
                  <span>{{lang.ontrial_text4}}</span>
                  <t-switch :custom-value="[1,0]" v-model="lineForm.ontrial"></t-switch>
                </div>
                <t-form-item :label="lang.ontrial_text20" v-show="lineForm.ontrial === 1">
                  <t-input-number v-model="lineForm.ontrial_price" :placeholder="lang.ontrial_text20" theme="normal"
                    :min="0" :decimal-places="2"></t-input-number>
                </t-form-item>
              </div>
              <div class="short-item">
                <div class="ontrial-open">
                  <span>{{lang.ontrial_text21}}</span>
                  <t-switch :custom-value="[1,0]" v-model="lineForm.ontrial_stock_control"></t-switch>
                </div>
                <t-form-item :label="lang.ontrial_text21" v-show="lineForm.ontrial_stock_control === 1">
                  <t-input-number v-model="lineForm.ontrial_qty" :placeholder="lang.ontrial_text21" theme="normal"
                    :min="0" :decimal-places="0"></t-input-number>
                </t-form-item>
              </div>
            </div>
          </template>
          <div class="com-f-btn">
            ...
            <t-button theme="default" variant="base" @click="recommendModel = false">
              {{lang.cancel}}
            </t-button>
          </div>
        </div>
    ...
    <t-dialog :header="lang.upgrade_range" :visible.sync="upgradeModel" :footer="false" width="710"
      ...
      <div class="com-f-btn" style="margin-top: 20px">
        <t-button theme="primary" @click="saveUpgrade" :loading="submitLoading" :disabled="isAgent">
          {{lang.hold}}
        </t-button>
        <t-button theme="default" variant="base" @click="closeUpgradeRange">
          {{lang.cancel}}
        </t-button>
      </div>
    </t-dialog>
    ...
    <t-dialog :header="comTitle" :visible.sync="limitModel" :footer="false" :width="isEn ? 800 : 680" @close="closeData"
      class="limit-dialog">
      <div class="top-info">
        <p class="tit first-text-color">{{lang.mf_choose_config}}</p>
        <p class="s-tip third-text-color">{{lang.mf_choose_tip}}</p>
      </div>
          ...
          <div class="check-box">
            <t-checkbox-group v-model="ruleLimit">
              <t-checkbox :key="item.type" :value="item.type" :label="item.name" v-for="item in calcCheckbox"
                :disabled="disabeldCheck('rule',item.type)"></t-checkbox>
            </t-checkbox-group>
          </div>
            ...
            <t-form-item name="rule.data_center.id">
              <t-select v-model="limitForm.rule.data_center.id" class="flow-type"
                ...
                <t-option :key="item.id" :value="item.id" :label="`${item.country_name}-${item.city}-${item.area}`"
                  v-for="item in dataList"></t-option>
              </t-select>
            </t-form-item>
          </t-form-item>
          <!-- cpu -->
            ...
            <template v-else>
              <t-form-item lable="" name="rule.memory.min" :required-mark="false"
                ...
                <t-input-number v-model="limitForm.rule.memory.min" :placeholder="lang.min_value" :min="0"
                  ...
                  @blur="changeLimitRange($event,'memory','min', 'rule')"></t-input-number>
              </t-form-item>
          ...
          <t-form-item :label="lang.opt_system" class="required limit-bw" v-if="showLimitItem('rule','image')">
            ...
            <t-form-item name="rule.image.id">
              <t-cascader :placeholder="lang.select+lang.opt_system" v-model="limitForm.rule.image.id"
                ...
                :keys="{ label: 'name', value: 'id' }"></t-cascader>
            </t-form-item>
          </t-form-item>
        </div>
        <!-- 结果 -->
          ...
          <div class="check-box">
            <t-checkbox-group v-model="resultLimit">
              <t-checkbox :key="item.type" :value="item.type" :label="item.name" v-for="item in calcResultCheckbox"
                :disabled="disabeldCheck('result',item.type)"></t-checkbox>
            </t-checkbox-group>
          </div>
          <!-- cpu -->
              ...
              <template v-if="memoryList[0].type !== 'radio'">
                <t-form-item lable="" :name="`result.memory[${index}].min`" :required-mark="false"
                  ...
                  <t-input-number v-model="limitForm.result.memory[index].min" :placeholder="lang.min_value" :min="0"
                    :max="99999999" theme="normal" :decimal-places="0" :title="lang.max_value_tip"
                    @blur="changeLimitRange($event,'memory','min','result', index)"></t-input-number>
                </t-form-item>
            ...
            <div class="result-item" v-for="(item,index) in limitForm.result.system_disk" :key="index">
              ...
              <t-form-item lable="" :name="`result.system_disk[${index}].min`" :required-mark="false"
                ...
                <t-input-number v-model="limitForm.result.system_disk[index].min" :placeholder="lang.min_value" :min="1"
                  ...
                  @blur="changeLimitRange($event,'system_disk','min','result', index , 1048576)"></t-input-number>
              </t-form-item>
              <t-form-item lable="" :name="`result.memory[${index}].max`" :required-mark="false" class="range-memory"
                ...
                <t-input-number v-model="limitForm.result.system_disk[index].max" :placeholder="lang.max_value_tip"
                  ...
                  @blur="changeLimitRange($event,'system_disk','max', 'result', index , 1048576)"></t-input-number>
              </t-form-item>
              ...
              <t-icon name="add-circle" @click="addResult('system_disk')"
                v-show="index === limitForm.result.system_disk.length - 1"></t-icon>
            </div>
            <t-icon name="add-circle" @click="addResult('system_disk')"
              v-show="limitForm.result.system_disk.length === 0"></t-icon>
          </t-form-item>
        </div>
        <div class="com-f-btn">
          <t-button theme="primary" type="submit" :loading="submitLoading"
            :disabled="ruleLimit.length < 1 || resultLimit.length < 1">
            {{lang.hold}}
          </t-button>
          <t-button theme="default" variant="base" @click="limitModel = false">
            {{lang.cancel}}
          </t-button>
        </div>
      </t-form>
    ...
    <t-dialog theme="warning" :header="delTit" :visible.sync="delVisible" class="deleteDialog">
      ...
      <template slot="footer">
        <t-button theme="primary" @click="sureDelete" :loading="submitLoading">
          {{lang.sure}}
        </t-button>
    ...
    <t-dialog theme="warning" :header="isLocalSystem ? lang.get_system : lang.get_local_system"
      ...
      <div>
        ...
        lang.get_local_system_tip2 : lang.get_local_system_tip3}}{{lang.get_local_system_tip5}}
        <span style="color: var(--td-error-color)">{{lang.get_local_system_tip6}}</span>
        {{lang.get_local_system_tip7}}
        <span style="color: var(--td-error-color)">
          {{isLocalSystem ? lang.get_local_system_tip9 : lang.get_local_system_tip8}}
        </span>
      </div>
      <template slot="footer">
        <t-button theme="primary" @click="surePullOs" :loading="pullOsLoading">
          {{lang.sure}}
        </t-button>
