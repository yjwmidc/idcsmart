<?php /*a:2:{s:46:"../public/hcydxoep/template/default/header.php";i:1756623755;s:72:"../public/plugins/addon/idcsmart_certification/template/admin/index.html";i:1756623755;}*/ ?>
<!-- =======内容区域======= -->
  ...
  <com-config>
    <t-card class="list-card-container">
      ...
      <t-table row-key="id" :data="data" size="medium" :columns="columns" :hover="hover" :loading="loading"
        ...
        <template #status="{row}">
          <t-tag theme="primary" class="status" v-if="row.status===1" variant="light">{{lang.certified}}</t-tag>
          <t-tooltip :content="row.auth_fail" :show-arrow="false" theme="light">
            <t-tag theme="danger" class="status" v-show="row.status===2" variant="light">{{lang.fail}}</t-tag>
          </t-tooltip>
          <t-tag theme="success" class="status" v-if="row.status===3" variant="light">{{lang.to_audit}}</t-tag>
    ...
    <t-dialog theme="warning" :header="statusTip" :visible.sync="statusVisble">
      <t-form :data="formData" ref="payDialog" :label-width="120" v-if="optType === 'reject'">
        <t-form-item>
           <t-textarea v-model="formData.reason" :rows="3" :placeholder="lang.dismiss_the_reason"></t-textarea>
        </t-form-item>
      </t-form>
    ...
    <t-dialog :visible.sync="detailVisible" :header="payTit" :on-close="closePay" :footer="false" placement="center"
      ...
      <t-form :data="realDetai" ref="payDialog" :label-width="120" v-if="realDetai">
        ...
        <t-form-item :label="lang.legal_name">
          <p class="disabled">{{realDetai.card_name}}</p>
        </t-form-item>
        <t-form-item :label="lang.legal_id_card">
