<?php /*a:2:{s:46:"../public/hcydxoep/template/default/header.php";i:1756623755;s:60:"../public/plugins/addon/promo_code/template/admin/index.html";i:1756623756;}*/ ?>
    ...
    <t-card class="list-card-container">
      <div class="common-header top">
        ...
        <div class="client-search">
          ...
          <t-select v-model="params.type" :placeholder="lang.coupon_code_type" clearable  @change="seacrh">
            <t-option v-for="item in typeList" :value="item.value" :label="item.label" :key="item.value">
            </t-option>
          </t-select>
          <t-select v-model="params.status" :placeholder="lang.order_current_status" clearable @change="seacrh">
      ...
      <t-table row-key="id" :data="data" size="medium" :columns="columns" :hover="hover" :loading="loading"
        ...
        <template #status="{row}">
          <t-tag theme="primary" class="com-status" v-if="row.status === 'Active'" variant="light">
            {{lang.promo_active}}
          </t-tag>
          <t-tag theme="default" class="com-status" v-if="row.status === 'Suspended'" variant="light">
            {{lang.promo_suspended}}
          </t-tag>
          <t-tag theme="danger" class="com-status" v-if="row.status === 'Expiration'" variant="light">
            {{lang.promo_expiration}}
          </t-tag>
          <t-tag theme="success" class="com-status" v-if="row.status === 'Pending'" variant="light">
            {{lang.promo_pending}}
          </t-tag>
        </template>
        <template #op="{row}">
