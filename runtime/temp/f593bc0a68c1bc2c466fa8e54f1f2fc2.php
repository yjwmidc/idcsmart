<?php /*a:1:{s:112:"/mnt/sdc/Webs/zjmf-v10-console/public/plugins/server/mf_cloud/template/clientarea/pc/default/product_detail.html";i:1756623758;}*/ ?>
<!-- 统计图表 -->
<!-- 页面独有样式 -->
<link rel="stylesheet" href="/plugins/server/mf_cloud/template/clientarea/pc/default/css/cloudTop.css" />
<link rel="stylesheet" href="/plugins/server/mf_cloud/template/clientarea/pc/default/css/cloudDetail.css" />
    ...
    <div class="card-top">
      <div class="top-operation">
        <div class="operation-row1">
          <svg t="1749023272025" viewBox="0 0 1024 1024" @click="goBack" class="back-img" version="1.1"
        ...
        <div class="operation-row2">
          <div class="row2-left">
            ...
            <span class="ip">
              ...
              <el-popover placement="top" trigger="hover" v-if="ipDetails.ip_num > 1">
                ...
                <span slot="reference" class="base-color"> ({{ipDetails.ip_num}}) </span>
              </el-popover>
          ...
          <div class="row2-right" v-show="hostData.status == 'Active' && !initLoading">
            <!-- 停用-->
            <span class="refund" v-if="isRefundPlugin && cloudData.support_apply_for_suspend === 1">
            ...
            <!-- 控制台 -->
            <!-- <img class="console-img" src="/plugins/server/mf_cloud/template/clientarea/pc/default/img/cloudDetail/console.png" :title="lang.common_cloud_text8" @click="doGetVncUrl" v-show="status != 'operating'"> -->
            <!-- 开关机 -->
            <span class="on-off">
              <el-popover placement="bottom" v-model="onOffvisible" trigger="click">
                ...
                <img :src="'/plugins/server/mf_cloud/template/clientarea/pc/default/img/cloudDetail/'+status+'.png'"
                  :title="statusText" v-show="(status != 'operating') && (status != 'pending') && (status != 'fault')"
                  slot="reference" />
              </el-popover>
            ...
            <!-- <span class="restart">
              <el-popover placement="bottom" v-model="rebotVisibel" trigger="click">
                ...
                <img src="/plugins/server/mf_cloud/template/clientarea/pc/default/img/cloudDetail/restart.png" :title="lang.common_cloud_text13" v-show="status != 'operating'" slot="reference">
              </el-popover>
            </span> -->

            <!-- 救援模式 -->
            <img class="fault" src="/plugins/server/mf_cloud/template/clientarea/pc/default/img/cloudDetail/fault.png"
        ...
        <div class="operation-row3" v-show="hostData.status == 'Active'">
          ...
          <!-- 无备注 -->
          <span class="no-notes" v-else @click="doEditNotes"> {{lang.cloud_add_notes + ' +'}} </span>
        </div>
      </div>
      <div class="refund-msg" v-if="isRefundPlugin">
        ...
        <div class="refund-success" v-if="refundData && refundData.status == 'Suspending'">
          ...
          {{refundData.type=='Expire'?lang.common_cloud_tip8:lang.common_cloud_tip9}}，{{lang.common_cloud_tip13}}<span
            v-if="refundData.type=='Expire'">{{hostData.due_time | formateTime}}</span>{{refundData.type=='Expire'? lang.common_cloud_tip10:lang.common_cloud_tip11}}
        ...
        <div class="msg-l">
          <div class="l-t">
            <div class="l-t-l">
              ...
              <span @click="copyLoginInfo" v-show="rescueStatusData.username" class="copy-btn"> {{lang.copy_login_info}}
              </span>
              <span @click="downloadRpd" style="margin-left: 0.1rem" v-loading="downloadLoading"
                ...
                class="copy-btn" v-if="hostData.status === 'Active'">
                {{lang.download_rpd}}
              </span>
              <el-popover v-if="hostData.status === 'Active'"
                v-show="cloudData.image?.image_group_name === 'Windows' || cloudData.image?.image_group_name === 'windows'"
                placement="top" trigger="hover">
                <div class="sure-remind">{{lang.rdp_tip}}</div>
                <span class="help" slot="reference">?</span>
              </el-popover>
            </div>
            <!-- 模拟物理机运行 -->
            <div class="r-t-r" v-if="cloudConfig.manual_manage === 0">
              <span>{{lang.simulate_physical}}：</span>
              <el-switch :value="rescueStatusData.simulate_physical_machine" active-color="var(--color-primary)"
                @change="physicalChange" :active-value="1" :inactive-value="0"> </el-switch>
          ...
          <div class="l-b">
            ...
            <div class="info-item">
              ...
              <div class="value" v-if="cloudData.ssh_key?.id === 0">
                ...
                <span v-show="!isShowPass"> {{rescueStatusData?.password?.replace(/./g, '*')}} </span>
                <img class="eyes"
                  :src="isShowPass ? '/plugins/server/mf_cloud/template/clientarea/pc/default/img/cloud/pass-show.png':'/plugins/server/mf_cloud/template/clientarea/pc/default/img/cloud/pass-hide.png'"
                  @click="isShowPass=!isShowPass" />
                <i class="el-icon-document-copy copy-password" @click="copyPass(rescueStatusData.password)"></i>
              </div>
              <div class="value" v-else>
                <span style="color: var(--color-primary); cursor: pointer"
                  @click="goSSHpage">{{cloudData.ssh_key?.name}}</span>
              </div>
            </div>
            <div class="info-item" v-if="cloudData.line?.bill_type === 'flow'">
              ...
              <template v-else>{{lang.mf_demand_fee}}</template>
            </div>
            <div class="info-item">
            ...
            <div class="info-item">
              ...
              <div class="value">
                <!-- ipDetails.ip_num - cloudData.ipv6_num <= 0 -->
                <template v-if="ipv4Select.length <= 0"> {{lang.mf_none}} </template>
            ...
            <div class="info-item">
              ...
              <div class="value">
                <template v-if="cloudData.ipv6_num === 0"> {{lang.mf_none}} </template>
                <template v-else>{{cloudData.ipv6_num}}{{lang.mf_one}}</template>
              </div>
            </div>
            <div class="info-item">
              <div class="label">{{lang.panel_password}}:</div>
              <div class="value">
                <span v-show="isShowPanelPass">{{rescueStatusData.panel_pass}}</span>
                <span v-show="!isShowPanelPass"> {{rescueStatusData?.panel_pass?.replace(/./g, '*')}} </span>
                <img class="eyes"
                  :src="isShowPanelPass ? '/plugins/server/mf_cloud/template/clientarea/pc/default/img/cloud/pass-show.png':'/plugins/server/mf_cloud/template/clientarea/pc/default/img/cloud/pass-hide.png'"
                  @click="isShowPanelPass=!isShowPanelPass" />
                <i class="el-icon-document-copy copy-password" @click="copyPass(rescueStatusData.panel_pass)"></i>
              </div>
            </div>
            <div class="info-item" v-for="(item,index) in self_defined_field" :key="item.id">
              ...
              <img class="eyes" v-if="item.field_type === 'password' && item.value"
                :src="item.hidenPass ? '/plugins/server/mf_cloud/template/clientarea/pc/default/img/cloud/pass-show.png' : '/plugins/server/mf_cloud/template/clientarea/pc/default/img/cloud/pass-hide.png'"
                @click="item.hidenPass=! item.hidenPass" />
              <i class="el-icon-document-copy"
                v-if="(item.field_type === 'password' || item.field_type === 'link') && item.value"
                style="font-size: 14px; margin-left: 3px; cursor: pointer; color: var(--color-primary)"
            ...
            <div class="info-item" v-for="(item,index) in customManualField" :key="item.id">
              ...
              <img class="eyes" v-if="item.type === 'password' && item.value" :src="item.hidenPass ? '/plugins/server/mf_cloud/template/clientarea/pc/default/img/cloud/pass-show.png' :
                  '/plugins/server/mf_cloud/template/clientarea/pc/default/img/cloud/pass-hide.png'"
                @click="changeCustomPassword(item)" />
              <i class="el-icon-document-copy" v-if="item.value"
                style="font-size: 14px; margin-left: 3px; cursor: pointer; color: var(--color-primary)"
                @click="copyPass(item.value)"></i>
            </div>
          </div>
        ...
        <div class="msg-r">
          <div class="r-t">
            ...
            <div class="r-t-r" v-show="hostData.status == 'Active'"
              v-if="isRenewPlugin && !isDemandFee && hostData.change_billing_cycle_id === 0">
              <span>{{lang.common_cloud_text16}}：</span>
              <!-- <el-switch v-model="isShowPayMsg" active-color="var(--color-primary)" @change="autoRenewChange">
              </el-switch> -->
            ...
            <!-- 支付历史账单 -->
            <span class="history-bill" v-if="hostData.status === 'Grace' || hostData.status === 'Keep'"
              @click="showBillList">{{lang.mf_demand_tip19}}</span>
          </div>
          <div class="r-b">
            <div class="row">
              <div class="row-l" v-if="!isDemandFee">
              ...
              <div class="row-l" v-else>
                <div class="label">{{lang.mf_demand_tip4}}:</div>
                <div class="value">{{hostData.next_payment_time | formateTime}}</div>
              </div>
              <div class="row-r">
            ...
            <div class="row">
              <div class="row-l">
                ...
                <div class="value" v-if="!isDemandFee">
              ...
              <div class="row-r" v-if="!isDemandFee">
              ...
              <div class="row-r" v-if="isDemandFee">
                <div class="label">{{lang.mf_demand_tip6}}:</div>
                <div class="value">
                  {{hostData.auto_release_time | formateTime}}
            ...
            <div class="row">
              <div class="row-l" v-if="!isDemandFee">
                ...
                <div class="value">{{commonData.currency_prefix + showRenewPrice + commonData.currency_suffix}}</div>
                <!--  || 'Suspended' -->
                <template v-if="isRenewPlugin && hostData.change_billing_cycle_id === 0">
                  ...
                  <span v-show="hostData.status == 'Active'" class="renew-btn-disable" v-else>{{lang.cloud_re_btn}}
                  </span>
                </template>
              </div>
              <div class="row-l" v-else>
                <div class="label">{{lang.mf_demand_tip18}}:</div>
                <div class="value">{{commonData.currency_prefix + hostData.renew_amount}}/{{lang.mf_hour}}</div>
              </div>
              <div class="row-r">
            ...
            <div class="row" v-if="isDemandFee && cloudData.line?.bill_type === 'flow'">
              <div class="row-l">
                <div class="label">{{lang.mf_flow}}:</div>
                <div class="value">{{commonData.currency_prefix + hostData.on_demand_flow_price}}/GB</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <el-tabs class="tabs" v-model="activeName" @tab-click="handleClick"
        v-show="(hostData.status === 'Active' || hostData.status === 'Grace') && !initLoading">
            ...
            <el-row>
              <el-col :span="8">
                <div class="manage-item">
                  <div class="item-top">
                    ...
                    <div class="item-top-btn" @click="showPowerDialog">{{lang.common_cloud_btn10}}</div>
                  </div>
              ...
              <el-col :span="8">
                <div class="manage-item">
                  ...
                  <div class="item-top-btn" @click="getVncUrl" v-loading="loading2" v-else>{{lang.common_cloud_btn11}}
                  </div>
                  <div class="item-bottom">
              ...
              <el-col :span="8">
                <div class="manage-item">
                  <div class="item-top-btn" @click="showRePass">{{lang.common_cloud_btn12}}</div>
                  <div class="item-bottom">
              ...
              <!-- 第二行 -->
              <el-col :span="8" v-if="cloudConfig.manual_resource_control_mode !== 'not_support'">
                <div class="manage-item">
                  <div class="item-top-btn" @click="showRescueDialog" v-if="!isRescue">{{lang.common_cloud_btn13}}</div>
                  <div class="item-top-btn" @click="showQuitRescueDialog" v-else>{{lang.common_cloud_btn14}}</div>
                  <div class="item-bottom">
              ...
              <template v-else>
                <el-col :span="8">
                  <div class="manage-item">
                    <div class="item-top-btn" @click="showRescueDialog">{{lang.common_cloud_btn13}}</div>
                    <div class="item-bottom">
                      <div class="bottom-row">{{lang.common_cloud_tip21}}</div>
                      <div class="bottom-row">{{lang.common_cloud_tip22}}</div>
                    </div>
                ...
                <el-col :span="8">
                  <div class="manage-item">
                    <div class="item-top-btn" @click="showQuitRescueDialog">{{lang.common_cloud_btn14}}</div>
                    <div class="item-bottom">
                      <div class="bottom-row">{{lang.common_cloud_tip21}}</div>
                      <div class="bottom-row">{{lang.common_cloud_tip22}}</div>
                    </div>
              ...
              <el-col :span="8">
                <div class="manage-item">
                  <div class="item-top-btn" @click="showReinstall">{{lang.common_cloud_btn15}}</div>
                  <div class="item-bottom">
              ...
              <el-col :span="8">
                <div class="manage-item">
                  <!-- v-if="cloudConfig.manual_manage === 0"  style="background: #eee; cursor: not-allowed; color: #999"-->
                  <div class="item-top-btn" @click="showUpgrade" v-if="hostData.change_billing_cycle_id === 0">
                    {{lang.common_cloud_btn16}}
                  </div>
                  <div class="item-top-btn" v-else style="background: #eee; cursor: not-allowed; color: #999">
                    {{lang.common_cloud_btn16}}
                  </div>
                  <div class="item-bottom">
        ...
        <!-- 磁盘 -->
        <el-tab-pane :label="lang.common_cloud_tab3" name="3" v-if="cloudConfig.manual_manage === 0">
          ...
          <div class="yes-disk" v-else>
            <div class="main-table">
              <el-table v-loading="diskLoading" :data="diskList" style="width: 100%">
                ...
                <el-table-column prop="op" :label="lang.common_cloud_label30" width="150" align="left"
                  :show-overflow-tooltip="true">
                  <template slot-scope="scope">
          ...
          <span v-show="dataDiskList.length > 0 && !isPackage " class="buy-btn" @click="showDg">
            {{lang.common_cloud_btn22}}
          </span>
          <!-- 弹性磁盘 -->
          <div class="yes-disk net" v-if="elasticDisk.length > 0 && !isSync">
            <div class="title top-box">
              <span>{{lang.elastic_disk}}</span>
              <div></div>
            </div>
            <div class="main-table">
              <el-table v-loading="elasticDiskLoading" :data="elasticDisk" style="width: 100%">
                ...
                <el-table-column prop="due_time" min-width="200" :label="lang.common_cloud_text144" align="left">
                  <template slot-scope="scope"> {{scope.row.due_time | formateTime}} </template>
                </el-table-column>
                <el-table-column prop="size" :label="lang.common_cloud_label20" min-width="200" align="left"
                  :show-overflow-tooltip="true">
                  <template slot-scope="scope">
        ...
        <el-tab-pane :label="lang.common_cloud_tab4" name="4" v-if="cloudConfig.manual_manage === 0">
          <div class="net">
            <!-- 公网IP -->
            <div class="title top-box">
            ...
            <div class="main_table">
              <el-table v-loading="netLoading" :data="netDataList" style="width: 100%; margin-bottom: 0.2rem">
                ...
                <el-table-column prop="subnet_mask" min-width="200" :label="lang.common_cloud_label23" align="left">
                  <template slot-scope="{row}"> {{row.subnet_mask || '--'}} </template>
                </el-table-column>
              </el-table>
              <pagination :page-data="netParams" v-if="netParams.total" @sizechange="netSizeChange"
                @currentchange="netCurrentChange"> </pagination>
            </div>
            <!-- ipv6 -->
            <div class="main_table" style="margin-top: 0.2rem" v-if="cloudData.network_type !== 'vpc'">
              <el-table v-loading="ipv6Loading" :data="ipv6DataList" style="width: 100%; margin-bottom: 0.2rem">
                <el-table-column prop="ipv6" :label="lang.ipv6_address" min-width="200" align="left"> </el-table-column>
                <el-table-column prop="gateway" min-width="200" :label="lang.common_cloud_label22" align="left">
                  <template slot-scope="{row}"> {{row.gateway || '--'}} </template>
                </el-table-column>
                <el-table-column prop="subnet_mask" min-width="200" :label="lang.common_cloud_label23" align="left">
            ...
            <!-- 弹性IP -->
            <div class="title top-box" v-if="elasticList.length > 0 && !isSync">
              <span>{{lang.public_elastic_ip}}</span>
              <div></div>
            </div>
            <div class="main_table" v-if="elasticList.length > 0 && !isSync">
              <el-table v-loading="elasticLoading" :data="elasticList" style="width: 100%; margin-bottom: 0.2rem">
                <el-table-column prop="ip" :label="lang.common_cloud_label21" min-width="200" align="left">
                </el-table-column>
                <el-table-column prop="gateway" min-width="200" :label="lang.common_cloud_label22" align="left">
                </el-table-column>
              ...
              <pagination :page-data="elasticParams" @sizechange="elasticSizeChange"
                @currentchange="elasticCurrentChange"> </pagination>
            </div>
            <!-- VPC -->
            <template v-if="cloudData.network_type === 'vpc'">
              ...
              <div class="main_table">
                <el-table v-loading="vpcLoading" :data="vpcDataList" style="width: 100%; margin-bottom: 0.2rem">
                  ...
                  <el-table-column min-width="300" :label="lang.common_cloud_title23" align="left">
                    <template slot-scope="scope">
                      <el-tooltip placement="top" v-if="scope.row.host.length > 1">
                        <div slot="content">
                          <template v-for="items in scope.row.host"> {{items.name}}<br /> </template>
                        </div>
            ...
            <template v-if="!isSync">
              <div class="title">{{lang.common_cloud_title26}}</div>
              <div class="safe-conten">
                <div class="safe-box" v-if="cloudData.security_group && cloudData.security_group.id !==0">
                  <span>{{lang.common_cloud_title27}} <span
                      style="color: var(--color-primary)">{{cloudData.security_group.name}}</span></span>
                  <div class="flow-btn" @click="goSecurityPage">{{lang.common_cloud_title28}}</div>
                </div>
                <div class="safe-box" v-else>
                  <span>{{lang.common_cloud_title29}}</span>
            ...
            <template v-if="cloudData.type !== 'hyperv'">
              ...
              <div class="title flow-box">
                ...
                <div type="primary" @click="buyPackage"
                  v-if="flowData.total?.includes('GB') && addonsArr.includes('FlowPacket')" size="small"
                  class="flow-btn">{{lang.buy_package}}</div>
              </div>
              ...
              <div class="echart-main">
                <!-- 网络流量 -->
                <div id="flow-echart" class="my-echart" v-loading="echartLoading2"></div>
              </div>
            </template>
          </div>
        </el-tab-pane>
        <!-- nat转发建站 -->
        <el-tab-pane :label="calcNat" name="nat"
          ...
          <div class="main-content" v-if="cloudData.nat_acl_limit > 0">
            ...
            <div class="main-table">
              <el-table v-loading="aclLoading" :data="aclList" style="width: 100%">
                ...
                <el-table-column prop="ip" :label="lang.forward_ip_port" :show-overflow-tooltip="true" min-width="150">
                  <template slot-scope="{row}">
                    <span>{{row.ip}}</span>
                    <i class="el-icon-document-copy base-color" @click="copyIp(row.ip)"></i>
                  </template>
                </el-table-column>
                ...
                <el-table-column prop="protocol" :label="lang.protocol" width="150" :show-overflow-tooltip="true">
                  <template slot-scope="{row}"> {{calcProtocol(row.protocol)}} </template>
                </el-table-column>
          ...
          <div class="main-content" v-if="cloudData.nat_web_limit > 0">
            <div class="content-top">
              ...
              <div class="top-btn nat-btn" @click="showCreateNat('web')"
                v-if="webList.length < cloudData.nat_web_limit">{{lang.invoice_text47}}{{lang.nat_web}}</div>
            </div>
            <div class="main-table">
              <el-table v-loading="webLoading" :data="webList" style="width: 100%">
                <el-table-column prop="id" label="ID" width="100"> </el-table-column>
                <el-table-column prop="domain" :label="lang.domain" :show-overflow-tooltip="true"> </el-table-column>
        ...
        <el-tab-pane :label="cloudData.type === 'hyperv' ? lang.common_cloud_title5 : lang.common_cloud_tab5" name="5"
          ...
          <div class="main-content" v-if="cloudData.backup_num > 0">
            <div class="content-top">
              <div class="top-title">
                ...
                <span class="ip-btn" @click="handleBackUpgrade('back')">{{lang.common_cloud_btn16}}</span>
              </div>
              <div class="top-btn" @click="showCreateBs('back')" v-if="dataList1.length < cloudData.backup_num">
            ...
            <div class="main-content" v-if="cloudData.snap_num > 0">
              <div class="content-top">
                <div class="top-title">
                  ...
                  <span class="ip-btn" @click="handleBackUpgrade('snap')">{{lang.common_cloud_btn16}}</span>
                </div>
                <div class="top-btn" @click="showCreateBs('snap')" v-if="dataList2.length < cloudData.snap_num">
        ...
        <el-tab-pane :label="lang.ip_defase_config" name="ip_defance" v-if="cloudData.line?.sync_firewall_rule == 1">
          <div v-if="activeName === 'ip_defance'">
            <ip-defase module="mf_cloud" :listmode="hostData?.agent ? 'remf_cloud' : 'mf_cloud'" :showip="showIp"
              :id="id" :product_id="product_id" :billing_cycle_time="hostData.billing_cycle_time">
            </ip-defase>
          </div>
        </el-tab-pane>
      </el-tabs>
      ...
      <div class="notes-dialog">
        <el-dialog width="6.2rem" :visible.sync="isShowNotesDialog" :show-close="false" @close="notesDgClose">
          <div class="dialog-title">{{hostData.notes?lang.common_cloud_title7:lang.common_cloud_title8}}</div>
      ...
      <div class="reinstall-dialog">
        <el-dialog width="6.2rem" :visible.sync="isShowReinstallDialog" :show-close="false" @close="reinstallDgClose">
          <div class="dialog-title">{{lang.common_cloud_title9}}</div>
          <div class="dialog-main">
                ...
                <el-option v-for="item in calcImageList" :key="item.id" :value="item.id" :label="item.name">
                  <div class="option-label">
                    <img class="option-img"
                      :src="'/plugins/server/mf_cloud/template/clientarea/pc/default/img/mf_cloud/' + item.icon + '.svg'"
                      alt="" />
            ...
            <div class="pass-content">
              <el-select class="pass-select" v-model="reinstallData.type">
                ...
                <el-option value="key" v-if="configObj.support_ssh_key === 1 && cloudConfig.manual_manage === 0"
                  label="ssh"></el-option>
              </el-select>
            ...
            <div class="pay-img" v-show="isPayImg">
              ...
              <el-popover placement="top-start" width="200" trigger="hover">
                <div class="show-config-list">{{lang.shoppingCar_tip_text2}}：{{commonData.currency_prefix}}{{
                  payDiscount }}</div>
                <div class="show-config-list">{{lang.common_cloud_text28}}：{{commonData.currency_prefix}}{{ payCodePrice
                  }}</div>
            ...
            <el-checkbox v-model="reinstallData.format_data_disk"
              style="margin-top: 0.2rem">{{lang.format_data_disk}}</el-checkbox>
            <div class="alert">
              <el-alert :title="errText" type="error" show-icon :closable="false" v-show="errText"> </el-alert>
      ...
      <!-- 加入安全组 -->
      <el-dialog width="6.8rem" :visible.sync="safeDialogShow" :show-close="false" @close="safeClose"
        class="safe-dialog">
          ...
          <div class="safe-main-card">
            <div class="safe-box">
              ...
              <el-select v-model="safeID">
                <el-option v-for="item in safeOptions" :key="item.id" :label="item.name" :value="item.id"> </el-option>
              </el-select>
            </div>
          </div>
        </div>
        ...
        <el-dialog width="6.9rem" :visible.sync="isShowRenew" :show-close="false" @close="renewDgClose">
              ...
              <div class="pay-price">
                <div class="money" v-loading="renewLoading">
                  ...
                  <el-popover placement="top-start" width="200" trigger="hover" v-if="(isShowLevel && renewParams.clDiscount*1 > 0) 
                    || (isShowPromo && renewParams.isUseDiscountCode) 
                    || ( isShowCash && renewParams.customfield.voucher_get_id)">
                  ...
                  <div class="code-box">
                    ...
                    <cash-coupon ref="cashRef" v-show="isShowCash && !cashObj.code"
                      ...
                      :scene="isDemandFee ? 'change_billing_cycle' : 'renew'" :product_id="[product_id]"
                      :price="renewParams.original_price"></cash-coupon>
                    <!-- 优惠码 -->
          ...
          <div class="dialog-footer">
            <el-button :loading="submitLoading" type="primary" class="btn-ok" @click="subRenew">
              {{isDemandFee ? lang.mf_demand_tip9 : lang.common_cloud_btn30}}
            </el-button>
      ...
      <!-- 新增VPC网络 -->
      <el-dialog width="6.8rem" :visible.sync="isShowAddVpc" :show-close="false" @close="addVpcClose"
        class="addVpc-dialog">
              ...
              <div class="vpc">
                ...
                <div class="custom">
                  <el-select v-model="vpc_ips.vpc1.value" @change="changeVpcIp">
                    <el-option v-for="item in vpc_ips.vpc1.select" :key="item" :label="item" :value="item"> </el-option>
                  </el-select>
                  <span>·</span>
                  ...
                  <el-tooltip :content="vpc_ips.vpc3Tips" placement="top" effect="light" :disabled="!vpc_ips.vpc3Tips">
                    <el-input-number :disabled="vpc_ips.vpc6.value === 16" @blur="changeVpc3" v-model="vpc_ips.vpc3"
                      :step="1" :controls="false" :min="0" :max="255"> </el-input-number>
                  </el-tooltip>
                  <span class="mr-5 ml-5">·</span>
                  ...
                  <el-select v-model="vpc_ips.vpc6.value" style="width: 70px" @change="changeVpcMask">
                    <el-option v-for="item in vpc_ips.vpc6.select" :key="item" :label="item" :value="item"> </el-option>
                  </el-select>
                </div>
              </div>
            </div>
      ...
      <!-- 分配主机 -->
      <el-dialog width="6.8rem" :visible.sync="isShowengine" :show-close="false" @close="engineClose"
        class="engine-dialog">
      ...
      <el-dialog width="6.8rem" :visible.sync="isShowIp" :show-close="false" @close="ipClose" class="ip-dialog">
        ...
        <div class="dialog-main">
          ...
          <div class="label" v-if="ipValueData.length > 0">
            ...
            {{lang.common_cloud_title43 + ',' + lang.common_cloud_title41}}：{{cloudData.ip_num}}{{lang.common_cloud_title43
            +
          ...
          <div class="ip-main-card" v-if="ipValueData.length > 0">
            ...
            <!-- cloudData.line?.sync_firewall_rule == 1 ? cloudData.ip_num :  -->
          ...
          <div class="ip-down" v-if="ipValueData.length > 0 && calcDelNum('ipv4') > 0">
            ...
            <el-select v-model="ipv4DelArr" :placeholder="lang.placeholder_pre2" clearable filterable multiple
              collapse-tags :disabled="calcDelNum('ipv4') === 0" :multiple-limit="calcDelNum('ipv4')">
              <el-option v-for="item in ipv4Select" :key="item" :label="item" :value="item"> </el-option>
            </el-select>
            <span class="tip">
          ...
          <template v-if="ipv6ValueData.length > 0 && cloudData.network_type !== 'vpc'">
            <div class="label">
              ...
              {{lang.common_cloud_title43 + ',' + lang.common_cloud_title41}}：{{cloudData.ipv6_num}}{{lang.common_cloud_title43
              + ',' +lang.cur_ipv6_tip}}
            </div>
            ...
            <!-- 指定IPv6降级 -->
            <div class="ip-down" v-if="ipv6ValueData.length > 0 && calcDelNum('ipv6') > 0">
              <p class="label">{{lang.ip_down_tip}}</p>
              <el-select v-model="ipv6DelArr" :placeholder="lang.placeholder_pre2" clearable filterable multiple
                collapse-tags :disabled="calcDelNum('ipv6') === 0" :multiple-limit="calcDelNum('ipv6')">
                <el-option v-for="item in ipv6Select" :key="item" :label="item" :value="item"> </el-option>
              </el-select>
              <span class="tip">
                {{lang.ip_down_tip1}}<span
        ...
        <div class="dialog-footer">
          <span class="text">{{isDemandFee ? lang.mf_demand_tip13 : lang.common_cloud_title44}}:</span>
          <span class="num" v-loading="ipPriceLoading">
            ...
            <el-popover placement="top-start" width="200" trigger="hover">
              <div class="show-config-list">{{lang.common_cloud_text27}}：{{commonData.currency_prefix}}{{
                calcFilterPrice(ipDiscountkDisPrice) }}</div>
        ...
        <el-dialog width="6.8rem" :visible.sync="isShowRefund" :show-close="false" @close="refundDgClose">
          ...
          <div class="dialog-footer">
            <span class="refund-tip"
              v-show="refundPageData.allow_refund === 0 && !isDemandFee">{{lang.refund_tip}}</span>
            <el-button class="btn-ok" @click="subRefund"
              :loading="submitLoaing">{{refundPageData.allow_refund == 1? lang.common_cloud_btn31: lang.common_cloud_btn32}}</el-button>
        ...
        <el-dialog width="6.8rem" :visible.sync="isShowRescue" :show-close="false" @close="rescueDgClose">
          ...
          <div class="dialog-main">
            ...
            <el-select class="os-select" v-model="rescueData.type">
              <img class="os-img"
                :src="'/plugins/server/mf_cloud/template/clientarea/pc/default/img/mf_cloud/' + (rescueData.type==1?'Windows':'Linux') + '.svg'"
                slot="prefix" alt="" />
              <el-option value="1" label="Windows">
                <div class="os-item option-label">
                  <img class="item-os-img"
                    src="/plugins/server/mf_cloud/template/clientarea/pc/default/img/mf_cloud/Windows.svg" alt="" />
                  <span class="item-os-text option-text">Windows</span>
                </div>
              </el-option>
              <el-option value="2" label="Linux">
                <div class="os-item option-label">
                  <img class="item-os-img"
                    src="/plugins/server/mf_cloud/template/clientarea/pc/default/img/mf_cloud/Linux.svg" alt="" />
                  <span class="item-os-text option-text">Linux</span>
                </div>
              </el-option>
            </el-select>
      ...
      <div class="quitRescu">
        <el-dialog width="6.8rem" :visible.sync="isShowQuit" :show-close="false" @close="quitDgClose">
          <div class="dialog-title">{{lang.common_cloud_btn14}}</div>
          <div class="dialog-main">{{lang.common_cloud_btn33}}</div>
          <div class="dialog-footer">
      ...
      <div class="upgrade-dialog">
        <el-dialog width="9.5rem" :visible.sync="isShowUpgrade" :show-close="false" @close="upgradeDgClose">
          <div class="dialog-title">{{lang.common_cloud_btn16}}</div>
          <div class="dialog-main">
            <!-- 自定义配置升降级 -->
            <el-form ref="orderForm" label-position="left" label-width="100px" hide-required-asterisk v-if="!isPackage">
              ...
              <el-form-item label="CPU">
                <el-radio-group v-model="params.cpu" @input="changeCpu">
                  <el-radio-button :label="c.value" v-for="(c,cInd) in calcCpuList" :key="cInd">
                    {{c.value}}{{lang.mf_cores}} </el-radio-button>
                </el-radio-group>
              </el-form-item>
              <!-- 内存 -->
              <el-form-item :label="lang.cloud_memery" v-if="memoryList.length > 0" class="move">
                ...
                <el-radio-group v-model="params.memory" v-if="memoryType" @input="changeMemory">
                  <el-radio-button :label="c.value" v-for="(c,cInd) in calaMemoryList" :key="cInd"
                    :class="{'com-dis': c.disabled}"> {{c.value}}{{memory_unit}} </el-radio-button>
                </el-radio-group>
                <!-- 拖动框 -->
                <template v-else>
              ...
              <el-form-item :label="lang.mf_bw" v-if="lineDetail.bill_type === 'bw' && lineDetail.bw.length > 0">
                ...
                <el-tooltip effect="light" v-else :content="lang.mf_range + bwTip" placement="top">
                  <el-slider v-model="params.bw" show-input :step="1" :min="lineDetail.bw[0].min_value"
                    :max="lineDetail.bw[lineDetail.bw.length - 1].max_value" @change="changeBwNum"> </el-slider>
                </el-tooltip>
              </el-form-item>
              <el-form-item label=" "
              ...
              <el-form-item :label="lang.mf_flow"
                v-if="!isDemandFee && lineDetail.bill_type === 'flow' && lineDetail.flow.length > 0">
                <el-radio-group v-model="flowName" @input="changeFlow">
                  <el-radio-button :label="c.value > 0 ? (c.value + 'G') : lang.mf_tip28"
                    v-for="(c,cInd) in lineDetail.flow" :key="cInd"> </el-radio-button>
                </el-radio-group>
              </el-form-item>
              <!-- 按需流量 -->
              <el-form-item :label="lang.mf_flow"
                v-if="lineDetail.bill_type === 'flow' && lineDetail.flow_on_demand.length > 0 && isDemandFee">
                <p class="flow-bw">{{lang.mf_demand_fee}}</p>
              </el-form-item>
              ...
              <el-form-item :label="lang.mf_bw" v-if="lineDetail.bill_type === 'flow'">
                ...
                <el-radio-group v-model="params.bw" @change="getCycleList">
                  <el-radio-button :label="c.out_bw" v-for="(c,cInd) in showFlowBw" :key="cInd">
                    {{c.out_bw === 0 ? lang.not_limited : c.out_bw + 'M'}}
                  </el-radio-button>
                </el-radio-group>
              </el-form-item>
              <!-- 防御 -->
              <el-form-item :label="lang.mf_defense"
                ...
                <el-radio-group v-model="defenseName">
                  <el-radio-button :label="c.desc" v-for="(c,cInd) in lineDetail.defence" :key="cInd"
                    @click.native="chooseDefence($event,c)"> </el-radio-button>
                </el-radio-group>
              </el-form-item>
            </el-form>
            <!-- 套餐升降级 -->
            <div class="package-box" v-else>
              <div class="cur-package">
                ...
                <div class="cloud-item active">
                  <div class="top">{{recommend_config.name}}<span class="des">（{{recommend_config.description}}）</span>
                  </div>
                  <div class="info">
                    <p><span
                    ...
                    <p><span class="name">{{lang.mf_bw}}：</span>{{recommend_config.bw}}M</p>
                    ...
                    <p v-if="recommend_config.peak_defence"><span
                        class="name">{{lang.peak_defence}}：</span>{{ recommend_config.peak_defence + 'G'}}</p>
                    <p><span
                        class="name">IPv4{{lang.shoppingCar_goodsNums}}：</span>{{recommend_config.ip_num === 0 ? lang.mf_none : recommend_config.ip_num + lang.common_cloud_title43}}
                    </p>
                    <p><span
                        class="name">IPv6{{lang.shoppingCar_goodsNums}}：</span>{{recommend_config.ipv6_num === 0 ? lang.mf_none : recommend_config.ipv6_num + lang.common_cloud_title43}}
                    </p>
                    <p v-if="recommend_config.gpu_num">
                      <span class="name">GPU{{lang.shoppingCar_goodsNums}}：</span>
                      <span
                        :title="`${recommend_config.gpu_num}*${recommend_config.gpu_name}`">{{cloudData.gpu || '--'}}</span>
                    </p>
                  </div>
                </div>
              </div>
              <div class="package-list">
                ...
                <div class="package-box">
                  <div class="cloud-item" :class="{active: recommend_config_id=== item.id}"
                    ...
                    <div class="top">{{item.name}}<span class="des">（{{item.description}}）</span></div>
                    <div class="info">
                      ...
                      <p v-if="item.peak_defence"><span
                          class="name">{{lang.peak_defence}}：</span>{{ item.peak_defence + 'G'}}</p>
                      <p><span
                          class="name">IPv4{{lang.shoppingCar_goodsNums}}：</span>{{item.ip_num === 0 ? lang.mf_none : item.ip_num + lang.common_cloud_title43}}
                      </p>
                      <p><span
                          class="name">IPv6{{lang.shoppingCar_goodsNums}}：</span>{{item.ipv6_num === 0 ? lang.mf_none : item.ipv6_num + lang.common_cloud_title43}}
                      </p>
                      <p v-if="item.gpu_num">
                        <span class="name">GPU{{lang.shoppingCar_goodsNums}}：</span>
          ...
          <div class="dialog-footer">
            <div class="footer-top">
              ...
              <div class="money" v-loading="upgradePriceLoading">
                <span class="money-num">
                  {{commonData.currency_prefix }} {{ calcFilterPrice(upParams.totalPrice) }}
                </span>
                <span class="demand-unit" v-if="isDemandFee">/{{lang.mf_hour}}</span>
                <el-popover placement="top-start" width="200" trigger="hover"
                  v-if="upParams.clDiscount * 1 > 0 || (isShowPromo && upParams.isUseDiscountCode) || isShowCash">
                  <div class="show-config-list">
                    <p v-if="upParams.clDiscount * 1 > 0">{{lang.shoppingCar_tip_text2}}：{{commonData.currency_prefix}}
                ...
                <p class="original-price" v-if="upParams.totalPrice*1 != upParams.original_price*1">
                  {{commonData.currency_prefix}} {{ calcFilterPrice(upParams.original_price)}}
                </p>
                <p class="demand-price" v-if="isDemandFee && lineDetail.bill_type === 'flow' && demandFlowPrice > 0">
                  ...
                  <span class="demand-discout"
                    v-if="demandFlowPrice * 1 > 0">{{commonData.currency_prefix}}{{calcFilterPrice(demandFlowPrice)}}</span>/GB)
                </p>
            ...
            <div class="footer-bottom">
              <el-button class="btn-ok" @click="upgradeSub" :loading="loading4"> {{lang.security_btn5}} </el-button>
      ...
      <div class="dg-dialog">
        <el-dialog width="9.12rem" :visible.sync="isShowDg" :show-close="false" @close="dgClose">
          <div class="dialog-title">{{lang.common_cloud_btn22}}</div>
          <!-- 当前配置磁盘 -->
          <div class="disk-card disk-old" v-if="oldDiskList2.length > 0">
            ...
            <div class="disk-main-card">
              <div class="old-disk-item" v-for="item in oldDiskList2" :key="item.id">
                ...
                <i class="el-icon-remove-outline del" v-if="item.type2 !== 'system' && item.is_free !== 1"
              ...
              <div class="new-disk-item" v-for="(item,i) in moreDiskData" :key="i">
                <template v-if="item.selectList.length > 0 && (item.selectList[0].type !== 'radio')">
                  ...
                  <el-select v-model="item.type" :placeholder="lang.placeholder_pre2"
                    ...
                    <el-option v-for="(items,index) in item.selectList[0].disk_typeList" :key="index"
                  ...
                  <el-tooltip effect="light" :content="lang.mf_range + item.selectList[0][item.type].tips"
                    ...
                    <el-input-number v-model="item.size" :min="item.selectList[0][item.type].min_value"
                      ...
                      @change="(val,oldVal)=>changeDataNum(val,oldVal,item)">
                    </el-input-number>
                  </el-tooltip>
                ...
                <template v-if="item.selectList.length === 0"> {{lang.common_cloud_btn67}} </template>
              </div>
            </div>
            <div class="disk-main-card" v-if="moreDiskData.length === 0">
          ...
          <div class="dialog-footer">
            <span class="text">{{isDemandFee ? lang.mf_demand_tip25 : lang.common_cloud_btn41}}:</span>
            <span class="num" v-loading="diskPriceLoading">
              {{commonData.currency_prefix}}{{calcFilterPrice(moreDiskPrice)}}
              <span class="demand-unit" v-if="isDemandFee">/{{lang.mf_hour}}</span>
              <el-popover placement="top-start" width="200" trigger="hover">
                <div class="show-config-list">
                ...
                <div class="show-config-list" v-if="!isDemandFee">
                  {{lang.common_cloud_text28}}：{{commonData.currency_prefix}}{{ moreCodePrice }}
                </div>
                <i class="el-icon-warning-outline total-icon" slot="reference" v-if="isShowLevel || isShowPromo"></i>
              </el-popover>
            </span>
      ...
      <!-- 扩容磁盘弹窗 -->
      <div class="kr-dialog">
        <el-dialog width="10rem" :visible.sync="isShowExpansion" :show-close="false" @close="krClose">
          <div class="dialog-title">{{lang.common_cloud_btn42}}</div>
          <div class="dialog-main">
              ...
              <div class="old-disk-item" v-for="item in oldDiskList" :key="item.id">
                <template
                  ...
                  <el-slider :min="item.min_value" :step="item.selectList[0].step" :max="item.max_value"
                    v-model="item.size" show-input @change="(val)=>sliderChange(val,item)"> </el-slider>
                ...
                <template
                  ...
                  <span class="disk-item-size">{{item.min_value + 'G'}}</span>
                  <el-select v-model="item.size" style="margin-left: 50px">
                    <el-option v-if="isShowAppendItem(item)" :key="freeDataObj.id" :value="freeDataObj.size"
                      :label="freeDataObj.size + 'G'"></el-option>
          ...
          <div class="dialog-footer">
            <span class="text">{{isDemandFee ? lang.mf_demand_tip26 : lang.common_cloud_btn44}}:</span>
            <span class="num" v-loading="diskPriceLoading">
              {{commonData.currency_prefix}}
              {{calcFilterPrice(expansionDiskPrice)}}
              <span class="demand-unit" v-if="isDemandFee">/{{lang.mf_hour}}</span>
              <el-popover placement="top-start" width="200" trigger="hover">
                <div class="show-config-list">{{lang.shoppingCar_tip_text2}}：{{commonData.currency_prefix}}{{
                  calcFilterPrice(expansionDiscount) }}
                </div>
                <div class="show-config-list" v-if="!isDemandFee">
      ...
      <div class="create-bs-dialog">
        <el-dialog width="6.8rem" :visible.sync="isShwoCreateBs" :show-close="false" @close="bsCgClose">
          <div class="dialog-title">{{isBs? lang.common_cloud_btn45 :lang.common_cloud_btn46}}</div>
          <div class="title-second-text">
            {{lang.common_cloud_btn47}}{{isBs? lang.common_cloud_btn45 :lang.common_cloud_btn46}}{{lang.common_cloud_btn48}}
          </div>
          <div class="dialog-main">
      ...
      <div class="restore-dialog">
        <el-dialog width="6.8rem" :visible.sync="isShowhyBs" :show-close="false" @close="bshyClose">
          <div class="dialog-title">{{lang.common_cloud_btn49}}</div>
          <div class="dialog-main">
            <span>{{lang.common_cloud_btn50}}{{restoreData.cloud_name}}{{lang.common_cloud_btn51}}</span>
            <span>{{restoreData.time | formateTime}}{{lang.common_cloud_btn52}}{{isBs?lang.common_cloud_btn53:lang.common_cloud_btn54}}</span>
      ...
      <div class="bs-dialog">
        <el-dialog width="6.2rem" :visible.sync="isShowOpenBs" :show-close="false" @close="bsopenDgClose">
          <div class="dialog-title">
            <span v-if="backupType === 'add'"> {{isBs?lang.common_cloud_btn62:lang.common_cloud_btn63}} </span>
          ...
          <div class="dialog-main">
            ...
            <el-select v-model="bsData.backNum" v-show="isBs" @change="bsSelectChange">
              <el-option v-for="item in backup_config" :key="item.id" :value="item.num"
                :label="item.num + lang.common_cloud_text3"> </el-option>
            </el-select>
            <!-- 快照下拉框 -->
            <el-select v-model="bsData.snapNum" v-show="!isBs" @change="bsSelectChange">
              <el-option v-for="item in snap_config" :key="item.id" :value="item.num"
                :label="item.num + lang.common_cloud_text4"> </el-option>
            </el-select>
            <span class="num"
              v-if="!isDemandFee">{{lang.common_cloud_text2}}{{isBs?bsData.backNum:bsData.snapNum}}{{lang.mf_one}}{{isBs?lang.common_cloud_title5:lang.common_cloud_btn54}}</span>
            <span class="num" v-else>{{lang.mf_demand_tip27}}</span>
            <span class="price" v-loading="bsDataLoading">
              {{commonData.currency_prefix}}{{calcFilterPrice(bsData.money)}}
              <template v-if="!isDemandFee">/{{bsData.duration}}</template>
              <template v-else>/{{lang.mf_hour}}</template>
              <el-popover placement="top-start" width="200" trigger="hover">
                <div class="show-config-list">{{lang.common_cloud_text27}}：{{commonData.currency_prefix}}{{
                  calcFilterPrice(bsData.moneyDiscount)}}</div>
                <!-- <div class="show-config-list">{{lang.common_cloud_text28}}：{{commonData.currency_prefix}}{{ bsData.codePrice | filterMoney}}</div> -->
                <i class="el-icon-warning-outline total-icon" slot="reference" v-if="isShowLevel || isShowPromo"></i>
              </el-popover>
            </span>
          </div>
          <div class="dialog-footer">
            <el-button class="btn-ok" @click="bsopenSub" :loading="submitLoading"> {{lang.common_cloud_btn28}}
            </el-button>
      ...
      <div class="power-dialog">
        <el-dialog width="6.2rem" :visible.sync="isShowPowerChange" :show-close="false" @close="powerDgClose">
          <div class="dialog-title">{{lang.common_cloud_text34}}{{powerTitle}}{{lang.common_cloud_text35}}</div>
          <div class="dialog-main">
      ...
      <div class="create-bs-dialog create-nat">
        <el-dialog width="6.8rem" :visible.sync="natDialog" :show-close="false" @close="bsCgClose">
          <div class="dialog-title">{{lang.invoice_text47}}{{natType === 'acl' ? lang.nat_acl :lang.nat_web}}</div>
          <div class="dialog-main">
            <el-form :model="natForm" :rules="natRules" ref="natForm" label-width="100px">
              ...
              <el-form-item :label="lang.ext_port" prop="ext_port">
                <el-input :value="80" disabled v-if="natType === 'web'"></el-input>
                <el-input-number :controls="false" :min="0" :max="65535" :precision="0" v-model="natForm.ext_port"
                  v-else :placeholder="lang.nat_tip1"></el-input-number>
              </el-form-item>
              <el-form-item :label="lang.int_port" prop="int_port">
                <el-input-number :value="natForm.int_port" :controls="false" :min="1" :max="65535" :precision="0"
                  :placeholder="`${lang.int_port}(1-65535)`" @input="changeIntPort"> </el-input-number>
              </el-form-item>
      ...
      <!-- 关联/取消关联 IP,磁盘 -->
      <div class="repass-dialog">
        <el-dialog width="6.8rem" :visible.sync="isShowConnect" :show-close="false" @close="isShowConnect = false">
          <div class="dialog-title">{{calcDes}}</div>
          <div class="dialog-main">
            <div class="label">{{lang.common_cloud_text36}}</div>
            <p>{{curIp}}</p>
            <template v-if="powerStatus=='off' && connectType === 'mf_cloud_ip'">
              <div class="alert">
                <div class="title">{{lang.common_cloud_tip30}}</div>
                <div class="row"><span class="dot"></span> {{lang.common_cloud_tip31}}</div>
                <div class="row"><span class="dot"></span> {{lang.common_cloud_tip32}}</div>
              </div>
              <el-checkbox v-model="connectCheck">{{lang.common_cloud_text24}}</el-checkbox>
              <div class="alert-err-text">
                <el-alert :title="errText" type="error" show-icon :closable="false" v-show="errText"> </el-alert>
              </div>
            </template>
          ...
          <div class="dialog-footer">
            <div class="btn-ok" @click="submitConnect" v-loading="loading5">{{lang.common_cloud_btn28}}</div>
            <div class="btn-no" @click="isShowConnect = false">{{lang.common_cloud_btn29}}</div>
          </div>
        </el-dialog>
      </div>
      ...
      <!-- 模拟物理机运行 -->
      <div class="repass-dialog">
        <el-dialog width="6.8rem" :visible.sync="physicalVisible" :show-close="false" @close="rePassDgClose">
          <div class="dialog-title">{{physicalTitle}}</div>
          <div class="dialog-main">
            <div class="alert" v-show="powerStatus=='off'">
              <div class="title">{{lang.common_cloud_tip30}}</div>
              <div class="row"><span class="dot"></span> {{lang.common_cloud_tip31}}</div>
            ...
            <el-checkbox v-model="physicalChecked" v-show="powerStatus=='off'"> {{lang.common_cloud_text24}}
            </el-checkbox>
            <div class="alert-err-text">
              <el-alert :title="errText" type="error" show-icon :closable="false" v-show="errText"> </el-alert>
            </div>
          ...
          <div class="dialog-footer">
            <el-button type="primary" class="btn-ok" @click="handlePhysical" :loading="submitLoaing">
              {{lang.common_cloud_btn28}} </el-button>
            <div class="btn-no" @click="physicalVisible = false">{{lang.common_cloud_btn29}}</div>
          </div>
        </el-dialog>
      </div>
      ...
      <div class="realease-dialog">
        <el-dialog width="6rem" :visible.sync="showReleaseTime" :show-close="false" @close="rePassDgClose">
          <div class="dialog-title">{{lang.mf_demand_tip6}}</div>
          <div class="dialog-main">
            <el-form :model="releaseTimeForm" :rules="natRules" ref="releaseTimeForm" label-position="left"
              label-width="auto" hide-required-asterisk>
              <el-form-item :label="lang.placeholder_pre2" prop="auto_release_time">
                <el-date-picker v-model="releaseTimeForm.auto_release_time" type="datetime"
                  :placeholder="lang.mf_demand_tip6" format="yyyy-MM-dd HH:mm">
                </el-date-picker>
              </el-form-item>
              <p class="des">{{lang.mf_demand_tip7}}</p>
            </el-form>
          </div>
          <div class="dialog-footer">
            <el-button type="primary" class="btn-ok" @click="handleSubmitReleaseTime" :loading="submitLoaing">
              {{lang.common_cloud_btn28}} </el-button>
            <div class="btn-no" @click="showReleaseTime = false">{{lang.common_cloud_btn29}}</div>
          </div>
        </el-dialog>
      </div>

      <!-- 支付历史账单 -->
      <el-dialog width="8rem" :visible.sync="historyVisible" :show-close="false" @close="payHistoryClose">
        <div class="dialog-title">{{lang.mf_demand_tip21}}</div>
        <div class="dialog-main">
          <el-table :data="historyList" style="width: 100%">
            <el-table-column prop="id" label="ID" width="100"></el-table-column>
            <el-table-column prop="product_names" :label="lang.appstore_text221" show-overflow-tooltip>
              <template slot-scope="scope">
                <span v-for="(item,index) in scope.row.product_names" :key="index">
                  <span v-if="index > 0">、</span>{{item}}
                </span>
              </template>
            </el-table-column>
            <el-table-column prop="billing_cycle" :label="lang.finance_label2" width="180">
              <template slot-scope="scope">
                <span>{{ commonData.currency_prefix + scope.row.amount}}</span>
              </template>
            </el-table-column>
            <el-table-column prop="create_time" :label="lang.finance_label3" width="180">
              <template slot-scope="scope">
                <span>{{scope.row.create_time | formateTime}}</span>
              </template>
            </el-table-column>
        ...
        <div class="dialog-footer">
          <el-button class="btn-ok" v-if="historyList.length" @click="submitCombine"
            :loading="submitLoading">{{lang.mf_demand_tip20}}</el-button>
          <div class="btn-no" @click="payHistoryClose">{{lang.common_cloud_btn29}}</div>
        </div>
      </el-dialog>

      <!-- 包年包月转按需 -->
      <div class="com-auto-renew transfer">
        <el-dialog :visible.sync="showTransferDemand" :show-close="false" custom-class="common-renew-dialog"
          width="6.2rem">
          <div class="dialog-title">
            {{lang.mf_demand_tip14}}
          </div>
          <div class="con">
            <p class="item">
            ...
            <!-- <p class="item">
              <span class="label">{{lang.mf_flow}}({{lang.mf_demand}})：</span>
              <span class="value">{{transferDemandForm.on_demand_price}}/{{}}</span>
            </p>
            <p class="item">
              <span class="label">{{lang.common_cloud_text7}}：</span>
              <span class="value">{{transferDemandForm.name}}</span>
            </p> -->
          </div>
          ...
          <div class="dialog-footer">
            <el-button class="btn-ok" @click="handleTransferDemand"
              :loading="submitLoading">{{lang.mf_demand_tip9}}</el-button>
            <el-button class="btn-no" @click="showTransferDemand = false">{{lang.auto_renew_cancel}}</el-button>
          </div>
        </el-dialog>
      </div>
      ...
      <!-- 取消包年包月转按需 -->
      <div class="power-dialog">
        <el-dialog width="5.2rem" :visible.sync="cancelTransfer" :show-close="false" @close="powerDgClose">
          <div class="dialog-title">{{lang.mf_demand_tip17}}</div>
          <div class="dialog-footer">
            <el-button class="btn-ok" @click="submitCancelTransfer"
              :loading="submitLoading">{{lang.referral_btn6}}</el-button>
            <div class="btn-no" @click="cancelTransfer = false">{{lang.common_cloud_btn29}}</div>
          </div>
        </el-dialog>
      </div>
      ...
      <flow-packet :id="id" :show-package="showPackage" @cancledialog="cancleDialog" v-if="showPackage"
        :currency_prefix="commonData.currency_prefix" @sendPackId="handlerPay"> </flow-packet>
      <!-- 安全验证 -->
...
<script src="/plugins/server/mf_cloud/template/clientarea/pc/default/js/echarts.js"></script>
<script src="/plugins/server/mf_cloud/template/clientarea/pc/default/js/cloudDetail.js"></script>
