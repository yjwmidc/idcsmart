<?php /*a:3:{s:50:"../public/hcydxoep/template/default/notice_sms.php";i:1758278328;s:46:"../public/hcydxoep/template/default/header.php";i:1758278328;s:46:"../public/hcydxoep/template/default/footer.php";i:1758278328;}*/ ?>
<!DOCTYPE html>
  ...
  <com-config>
    <t-card class="list-card-container">
      <div class="common-header top">
        <div class="left">
          <t-button @click="getMore" class="add" theme="primary"
      ...
      <t-table row-key="id" :data="data" size="medium" :columns="columns" :hover="hover" :loading="loading"
        ...
        <template #status="{row}">
          <t-tag theme="primary" class="status" v-if="row.status===1" variant="light">{{lang.enable}}</t-tag>
    ...
    <t-dialog :header="configTip" :visible.sync="configVisble" :footer="false" width="600">
      <t-form :rules="rules" ref="userDialog" @submit="onSubmit" :label-width="170">
        <t-form-item :label="item.title" v-for="item in configData" :key="item.title">
          ...
          <!-- text -->
          <t-input v-if="item.type==='text'" v-model="item.value" :placeholder="item.title" :disabled="item.attribute === 'disabled'"></t-input>
          <!-- password -->
          ...
          <!-- pull_sign -->
          <template v-if="item.type==='pull_sign'">
            <span class="pull-sign" @click="pullSign" v-show="!pullLoading">
              {{item.tip}}
            </span>
            <t-icon name="loading" v-show="pullLoading"></t-icon>
          </template>
        </t-form-item>
        <div class="com-f-btn">
...
<script src="/<?php echo htmlentities($template_catalog); ?>/template/<?php echo htmlentities($themes); ?>/js/notice_sms.js"></script>
<!-- footer -->
