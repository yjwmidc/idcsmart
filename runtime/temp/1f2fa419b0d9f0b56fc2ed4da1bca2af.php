<?php /*a:1:{s:96:"/mnt/sdc/Webs/zjmf-v10-console/public/plugins/server/mf_cloud/template/admin/module_operate.html";i:1755671708;}*/ ?>
...
<div class="operate-div">
  <com-config>
    <p class="top-status">
      ...
      <t-popup :content="lang[`cloud_${status}`]" v-if="status">
        ...
        <span v-else class="status">
          <img :src="`/plugins/server/mf_cloud/template/admin/img/${status}.png`" alt="">
        </span>
      </t-popup>
    </p>
    ...
    <!-- 开关机操作弹窗 -->
    <t-dialog theme="warning" :header="optTilte"  :visible.sync="moduleVisible"
        ...
        <t-form :data="reinstallData" :required-mark="false" ref="reinstallForm" @submit="saveReinstall" :rules="rules"
          ...
          <t-form-item :label="lang.setting_text52" name="image_id">
            <t-select v-model="selectOsId" style="width: 50%;" @change="osChange">
              <img class="os-option-img" slot="prefixIcon" style="margin-right: 5px"
                :src="`/plugins/server/mf_cloud/template/admin/img/${slectOsImg}.svg`" alt="">

              <t-option :key="item.id" :value="item.id" :label="item.name" v-for="item in osList">
                <div class="os-option">
                  <img class="os-option-img" :src="`/plugins/server/mf_cloud/template/admin/img/${item.icon}.svg`"
                    alt="">
                  {{item.name}}
                </div>
              </t-option>
          ...
          <t-form-item name="password">
            <div slot="label" class="custom-label">
              <span class="label">{{lang.setting_text53}}</span>
              <t-tooltip placement="top-right" :content="lang.setting_text55" :show-arrow="false" theme="light">
                <t-icon name="help-circle" size="18px" />
              </t-tooltip>
            </div>
            <t-select v-model="reinstallType" style="width: 50%;" @change="sshChange">
              <t-option value="passworld" :label="lang.setting_text53"></t-option>
              <t-option v-if="configObj.support_ssh_key === 1" value="ssh" :label="lang.setting_text54"></t-option>
            </t-select>
            <t-select style="width: 50%;" v-model="reinstallData.ssh_key_id" v-if="reinstallType === 'ssh'">
              <t-option :key="item.id" :value="item.id" :label="item.name" v-for="item in sshList">
              </t-option>
            </t-select>
            <t-input style="width: 50%;" v-if="reinstallType === 'passworld'" :placeholder="lang.setting_text53"
              v-model="reinstallData.password">
              <template #suffix-icon v-if="configObj.rand_ssh_port !== 2">
          ...
          <t-form-item :label="lang.format_data_disk" name="format_data_disk">
            <t-checkbox style="width: 50%;" v-model="reinstallData.format_data_disk"></t-checkbox>
          </t-form-item>
          <div class="com-f-btn">
        ...
        <t-form :data="rescueData" ref="rescueForm" :required-mark="false" @submit="saveReinstall" reset-type='initial'>
          <t-form-item :label="lang.setting_text57">
            <t-select v-model="rescueData.type">
              <img class="os-option-img" slot="prefixIcon" style="margin-right: 5px"
                :src="`/plugins/server/mf_cloud/template/admin/img/${rescueData.type === 'windows' ? 'Windows' : 'Linux'}.svg`"
                alt="">
              <t-option :value="1" label="Windows">
                <div class="os-option">
                  <img class="os-option-img" :src="`/plugins/server/mf_cloud/template/admin/img/Windows.svg`" alt="">
                  Windows
                </div>
              </t-option>
              <t-option :value="2" label="Linux">
                <div class="os-option">
                  <img class="os-option-img" :src="`/plugins/server/mf_cloud/template/admin/img/Linux.svg`" alt="">
                  Linux
                </div>
              </t-option>
            </t-select>
    ...
    <!-- 添加|更换|删除 IP -->
    <t-dialog :header="ipOptTitle" :visible.sync="ipModel" :footer="false" width="500"
    :close-on-overlay-click="false">
    <div class="ip-box reinstall-box">
      <t-form :data="ipForm" ref="ipForm" :required-mark="false" @submit="submitIpOPt" reset-type='initial'
        label-width="60px" label-align="left">
        <t-form-item name="ip_id" label="IP" :rules="[{ required: true, message: lang.select + 'IP' }]">
          <t-tree-select v-model="ipForm.ip_id" :data="ipList" multiple :placeholder="lang.select" 
           :tree-Props="treeProps" :min-collapsed-num="1" clearable>
          </t-tree-select>
        </t-form-item>
        <template v-if="networkType === 'normal' && status === 'on'">
          <div class="alert-box">
            <div class="alert-title">{{lang.setting_text59}}</div>
            <div class="alert-row"><span class="dot"></span>{{lang.setting_text60}}
            </div>
          ...
          <t-checkbox v-model="ipForm.checked">{{lang.setting_text62}}</t-checkbox>
        </template>
        <div class="com-f-btn">
          <t-button theme="primary" type="submit" :loading="moduleLoading">{{lang.hold}}</t-button>
          <t-button theme="default" variant="base" @click="ipModel = false">{{lang.cancel}}
          </t-button>
        </div>
      </t-form>
    </div>
...
<script src="/plugins/server/mf_cloud/template/admin/js/module_operate.js"></script>
