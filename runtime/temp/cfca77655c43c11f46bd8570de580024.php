<?php /*a:2:{s:46:"../public/hcydxoep/template/default/header.php";i:1756623755;s:67:"../public/plugins/addon/idcsmart_withdraw/template/admin/index.html";i:1756623756;}*/ ?>
  ...
  <com-config>
    <t-card class="list-card-container">
      ...
      <t-table row-key="1" :data="data" size="medium" :columns="columns" :hover="hover" :loading="loading"
        ...
        <template #status="{row}">
          <t-tag theme="success" class="status" v-if="row.status===0" variant="light">{{lang.to_audit}}</t-tag>
          <t-tag theme="primary" class="status" v-if="row.status===1" variant="light">{{lang.approved}}</t-tag>
          ...
          <t-tag class="order-refunded" class="status" v-if="row.status===3" variant="light">{{lang.confirmed}}</t-tag>
        </template>
