<?php /*a:1:{s:110:"/mnt/sdc/Webs/zjmf-v10-console/public/plugins/server/mf_cloud/template/clientarea/pc/default/product_list.html";i:1756623758;}*/ ?>
<!-- 页面独有样式 -->
    ...
    <div class="main-card-table">
      ...
      <div class="main-card-search">
        <batch-renewpage :tab="params.tab" :ids="multipleSelection" module-type="mf_cloud" @success="inputChange"
          ref="batchRenewRef"> </batch-renewpage>
        <traffic-warning module="mf_cloud"></traffic-warning>
        <div class="right-search">
          <el-select v-model="params.index" @change="centerSelectChange" :filterable="true" :clearable="true"
            :placeholder="lang.cloud_tip_1" v-if="center.length > 0">
          ...
          <el-input v-model="params.keywords" :placeholder="lang.cloud_tip_2" clearable @clear="clearKey"
            @keyup.enter.native="inputChange"> </el-input>
          <div class="search-btn" @Click="inputChange">{{lang.baidu_plugin_text7}}</div>
        </div>
      </div>
      <div class="table">
        <el-table v-loading="loading" :data="cloudData" style="width: 100%; margin-bottom: 0.2rem"
          row-class-name="border-r-none" header-row-class-name="border-r-hover" border class="list-table-border"
          ...
          <el-table-column prop="product-name" :label="lang.appstore_text221" min-width="170"
          ...
          <el-table-column prop="billing_cycle" :label="lang.mf_bill_way" width="130" :show-overflow-tooltip="true">
            <template slot-scope="{row}">
              <span v-if="row.billing_cycle === 'on_demand' || row.billing_cycle === 'recurring_prepayment_on_demand'">
                {{lang.mf_demand_fee}}
              </span>
          ...
          <el-table-column prop="renew" width="120" :label="lang.auto_renew" v-if="hasAutoRenew">
            <template slot-scope="{row}">
              <auto-renew
                v-if="row.status === 'Active' && row.billing_cycle !== 'on_demand' && row.billing_cycle !== 'recurring_prepayment_on_demand'"
                :id="row.id" :is-auto-renew="row.is_auto_renew" @update="getCloudList">
              </auto-renew>
              <span v-else>--</span>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="base_info" :label="lang.module_base_info" min-width="130" v-if="isShowBaseInfo"
            :show-overflow-tooltip="true">
            <template slot-scope="scope"> {{(scope.row.show_base_info === 1 && scope.row.base_info) || '--'}}
            </template>
          </el-table-column>
              ...
              <template v-if="scope.row.dedicate_ip && scope.row.status !== 'Deleted'">
                <div class="com-ip-box">
                  ...
                  <el-popover placement="top" trigger="hover" v-if="scope.row.ip_num > 1">
                    ...
                    <span slot="reference" class="base-color"> ({{scope.row.ip_num}}) </span>
                  </el-popover>
                </div>
