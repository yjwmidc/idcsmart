<?php /*a:3:{s:52:"../public/hcydxoep/template/default/client_order.php";i:1755667656;s:46:"../public/hcydxoep/template/default/header.php";i:1755667656;s:46:"../public/hcydxoep/template/default/footer.php";i:1755667656;}*/ ?>
<!DOCTYPE html>
  ...
  <com-config>
    ...
    <t-card class="list-card-container">
      ...
      <div class="common-header">
        <div class="flex">
          <t-button @click="addOrder" v-permission="'auth_user_detail_order_create_order'">{{lang.create_order}}
          </t-button>
        </div>
        <div class="right-create-order">
          <com-tree-select class="search-input" :autowidth="true" :multiple="true" :value="params.product_ids"
            @choosepro="choosePro">
          </com-tree-select>
          <t-select v-model="params.status" :placeholder="lang.box_title3" clearable @change="searchOrderList">
            <t-option v-for="item in orderStatus" :value="item.value" :label="item.label" :key="item.value">
            </t-option>
          </t-select>
          <t-select v-model="params.host_id" :placeholder="lang.tailorism" clearable @change="searchOrderList"
            filterable>
      ...
      <t-enhanced-table ref="table" row-key="id" drag-sort="row-handler" :data="data" :columns="columns"
        @sort-change="sortChange" :tree="{ childrenKey: 'list', treeNodeColumnIndex: 0 }" :loading="loading"
        class="user-order" :hide-sort-tips="true" :key="new Date().getTime()">
        ...
        <template #status="{row}">
          ...
          <t-tag theme="warning" variant="light" v-if="(row.status || row.host_status)==='Unpaid'" class="order-unpaid">
            {{lang.Unpaid}}
          </t-tag>
          ...
          <t-tag theme="default" variant="light" v-if="(row.status || row.host_status)==='Deleted'" class="delted">
            {{lang.Deleted}}
          </t-tag>
          ...
          <template v-if="row.type">
            ...
            <t-tooltip :content="lang.update_price" :show-arrow="false" theme="light">
              <t-icon name="money-circle" @click="updatePrice(row, 'order')" class="common-look"
                v-if="row.status!=='Paid' && row.status!=='Cancelled' && row.status!=='Refunded' && $checkPermission('auth_user_detail_order_adjust_order_amount')">
              </t-icon>
            </t-tooltip>
    ...
    <t-dialog :header="lang.update_price" :visible.sync="priceModel" :footer="false" @close="closePrice">
      <t-form :data="formData" ref="priceForm" @submit="onSubmit" :rules="rules">
        <t-form-item :label="lang.change_money" name="amount">
          <t-input v-model="formData.amount" type="tel" :label="currency_prefix" :placeholder="lang.update_price_tip">
          </t-input>
        </t-form-item>
...
<script src="/<?php echo htmlentities($template_catalog); ?>/template/<?php echo htmlentities($themes); ?>/js/client_order.js"></script>
<!-- footer -->
