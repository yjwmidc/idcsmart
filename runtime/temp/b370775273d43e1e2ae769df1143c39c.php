<?php /*a:2:{s:46:"../public/hcydxoep/template/default/header.php";i:1755667656;s:65:"../public/plugins/addon/idcsmart_refund/template/admin/index.html";i:1755667657;}*/ ?>
  ...
  <com-config>
    <t-card class="list-card-container">
      ...
      <div class="order-search-wrapper">
        <t-select v-model="params.host_status" multiple :min-collapsed-num="2" clearable
        ...
        <t-select v-model="params.status" multiple :min-collapsed-num="2" clearable
        ...
        <t-input v-model="params.keywords" :placeholder="lang.refund_check_input" clearable
      ...
      <t-table row-key="id" :data="listData" :columns="columns">
        ...
        <template #host_status="slotProps">
          <template v-if="slotProps.row.host_status=='Unpaid'">
            <t-tag theme="danger" variant="light">{{lang.Unpaid}}</t-tag>
          </template>
          <template v-else-if="slotProps.row.host_status=='Pending'">
            <t-tag theme="primary" variant="light">{{lang.Pending}}</t-tag>
          </template>
          <template v-else-if="slotProps.row.host_status=='Suspended'">
            <t-tag theme="default" variant="light">{{lang.Suspended}}</t-tag>
          </template>
          ...
          <template v-else-if="slotProps.row.host_status=='Failed'">
            <t-tag theme="default" class="order-canceled" variant="light">{{lang.Failed}}</t-tag>
          </template>
        </template>
        <template #status="slotProps">
          <template v-if="slotProps.row.status=='Pending'">
            <t-tag theme="success" variant="light">{{lang.to_audit}}</t-tag>
          </template>
          <template v-else-if="slotProps.row.status=='Suspending'">
            <t-tag variant="light">{{lang.to_stop_using}}</t-tag>
          </template>
          <template v-else-if="slotProps.row.status=='Suspend'">
            <t-tag theme="danger" variant="light">{{lang.stop_using_the}}</t-tag>
          </template>
          <template v-else-if="slotProps.row.status=='Suspended'">
            <t-tag variant="light">{{lang.has_been_discontinued}}</t-tag>
          </template>
          <template v-else-if="slotProps.row.status=='Refund'">
            <t-tag variant="light">{{lang.refunded}}</t-tag>
          </template>
          ...
          <template v-else-if="slotProps.row.status=='Cancelled'">
            <t-tag variant="light" class="order-canceled">{{lang.canceled}}</t-tag>
          </template>
        </template>
        <template #op-column>
          <p>{{lang.operation}}</p>
        ...
        <template #op="slotProps">
          <span class="refund-icon" :title="lang.get_approved"
            ...
            @click="handlePass(slotProps.row)">
    ...
    <!-- 删除弹窗 -->
    <t-dialog theme="warning" :header="delTitle" :visible.sync="delVisible">
