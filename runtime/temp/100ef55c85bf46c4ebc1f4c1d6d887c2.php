<?php /*a:3:{s:47:"../public/hcydxoep/template/default/product.php";i:1758278328;s:46:"../public/hcydxoep/template/default/header.php";i:1758278328;s:46:"../public/hcydxoep/template/default/footer.php";i:1758278328;}*/ ?>
<!DOCTYPE html>
  ...
  <com-config>
    <t-card class="list-card-container" :class="{stop: data.status===0}">
      <div class="common-header top">
        <div class="left">
          ...
          <t-button @click="addProduct" class="add"
            v-permission="'auth_product_management_create_product'">{{lang.create_product}}</t-button>
          <t-button v-if="hasBaidu" @click="addBaiduProduct" class="add">
            {{lang.baidu_create}}
          </t-button>
          <t-button theme="default" @click="addAgency" class="add com-gray-btn"
            v-permission="'auth_product_management_agent_product'">
            {{lang.immediate_agency}}
          </t-button>
          <t-button theme="default" class="add com-gray-btn" @click="handelAngenBtn"
            v-permission="'auth_product_management_agentable_product'">
            {{lang.manage_agency}}
          </t-button>
          <t-button theme="default" class="add com-gray-btn" @click="noticeManage"
            v-permission="'auth_product_management_global_setting'">
            {{lang.product_set_text1}}
          </t-button>
        </div>
        <div class="flex">
          <t-input v-model="params.keywords" class="search-input" :placeholder="`${lang.product_set_text99}`"
            ...
            <t-icon size="20px" name="search" @click="search" class="com-search-btn" slot="suffix-icon"></t-icon>
          </t-input>
      ...
      <t-enhanced-table ref="table" row-key="key" drag-sort="row-handler" :data="data" :columns="columns"
        ...
        <template #hidden="{row}">
          <t-switch :custom-value="[0,1]" v-model="row.hidden" @change="onChange(row)"
            :disabled="!$checkPermission('auth_product_management_product_show_hide')">
          </t-switch>
        </template>
        <template #op="{row}">
    ...
    <t-dialog :visible.sync="agentVisble" :header="lang.can_be_agency" :on-close="closeAgentDia" :footer="false"
      ...
      <div class="opt">
        <span>
          ...
          <t-checkbox v-model="checkAll" @change="chooseAllAgent" :disabled="formData.id===1">
            {{lang.isCheckAll}}
          </t-checkbox>
        </span>
      </div>
    ...
    <t-dialog :header="updateNameTip" :visible.sync="updateNameVisble" :footer="false">
      <t-form :data="updataData" ref="groupForm" @submit="submitUpdateName" :rules="rules">
        ...
        <t-form-item :label="lang.description" name="description" v-if="isUpdateType === 'second'">
          <t-textarea v-model="updataData.description" :placeholder="`${lang.description}`"></t-textarea>
        </t-form-item>
        <div class="com-f-btn">
    ...
    <t-dialog theme="warning" :header="copyTitle" :visible.sync="copyVisble" :footer="false" class="product-dialog">
      <t-form :data="copyData" ref="copyProForm" @submit="sureCopy" :rules="productRules">
        <t-form-item :label="lang.first_group" name="firstId">
          <t-select v-model="copyData.firstId" :placeholder="lang.first_group" @change="changeCopyFirId">
            <t-option v-for="item in firstGroup" :value="item.id" :label="item.name" :key="item.id">
            </t-option>
          </t-select>
        </t-form-item>
