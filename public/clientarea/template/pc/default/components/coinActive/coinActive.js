const coinActive = {
  ...
  <div class="coin-active-box" v-loading="loading">
    ...
    <el-tabs v-model="activeTab" @tab-click="handleTabClick" v-if="product_id || host_id" class="coin-active-tabs">
          ...
          <div class="coin-item-detail" v-if="item.show_desc">
            ...
            <div class="coin-active-detail">
              ...
              <template v-if="item.type === 'property'">
                <!-- 属性送 -->
                <p class="coin-detail-main-desc">
                <p>{{lang.coin_text81}}：{{item.property_type === 'register' ? lang.coin_text82 : lang.coin_text83}}</p>
              ...
              <template v-if="item.type === 'single_consume'">
                ...
                {{lang.coin_text107}}：<template v-if="item.single_consume_able_product_ids_arr?.length > 0">
                  <span v-for="(product_item,index) in item.single_consume_able_product_ids_arr" :key="product_item.id">
            ...
            <p v-if="item.cycle_limit === 1 ">
              {{lang.coin_text5}}：<span v-for="(cycle_item,index) in item.cycle" :key="cycle_item">{{lang[cycle_item]}}
    ...
    async getCoinInfo() {
      try {
        ...
        if (this.home_show_coin_activity == 1) {
          this.setProductId();
          await this.getCoinActiveList();
        }
      } catch (error) {
        console.log(error);
      }
    },
    async getCoinActiveList() {
      this.loading = true;
      const params = {};
