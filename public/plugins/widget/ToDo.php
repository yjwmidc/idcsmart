...
class ToDo extends Widget
{
    ...
    public function getData()
    {
        ...
        if(in_array('TicketPremium', $addons)){
            ...
            $data['pending_work_orders_url'] = request()->domain().'/'.env('DIR_ADMIN', 'admin').'/plugin/ticket_premium/index.htm';
            ...
            $data['pending_work_orders_url'] = request()->domain().'/'.env('DIR_ADMIN', 'admin').'/plugin/idcsmart_ticket/index.htm';
        }
        ...
        if(in_array('IdcsmartRefund', $addons)){
            ...
            $data['pending_refunds_url'] = request()->domain().'/'.env('DIR_ADMIN', 'admin').'/plugin/idcsmart_refund/index.htm';
        }
        ...
        if(in_array('IdcsmartCertification', $addons)){
            ...
            $data['pending_real_name_authentication_url'] = request()->domain().'/'.env('DIR_ADMIN', 'admin').'/plugin/idcsmart_certification/index.htm';
        }
        ...
        if(in_array('IdcsmartWithdraw', $addons)){
            ...
            $data['pending_withdrawals_url'] = request()->domain().'/'.env('DIR_ADMIN', 'admin').'/plugin/idcsmart_withdraw/index.htm';
        }
        ...
        if(in_array('IdcsmartRecommend', $addons)){
            ...
            $data['to_be_confirmed_recommend_url'] = request()->domain().'/'.env('DIR_ADMIN', 'admin').'/plugin/idcsmart_recommend/index.htm';
        }
        ...
        if(in_array('IdcsmartInvoice', $addons)){
            ...
            $data['pending_invoices_url'] = request()->domain().'/'.env('DIR_ADMIN', 'admin').'/plugin/idcsmart_invoice/index.htm';
        }
        ...
        if(in_array('IdcsmartSmsSign', $addons)){
            ...
            $data['pending_sms_sign_url'] = request()->domain().'/'.env('DIR_ADMIN', 'admin').'/plugin/idcsmart_sms_sign/index.htm?status=0';
        }

        $HostModel = new HostModel();
        $data['pending_host_num'] = $HostModel->where('status', 'Pending')->where('is_sub',0)->count();
        $data['pending_host_num_url'] = request()->domain().'/'.env('DIR_ADMIN', 'admin').'/host.htm?type=status&keywords=Pending';

        $OrderModel = new OrderModel();
        $data['pending_offline_transfer'] = $OrderModel->where('status', 'WaitReview')->count();
        $data['pending_offline_transfer_url'] = request()->domain().'/'.env('DIR_ADMIN', 'admin').'/order.htm';

        $data['host_failed_action_count'] = $HostModel->failedActionCount();
        $data['host_failed_action_count_url'] = request()->domain().'/'.env('DIR_ADMIN', 'admin').'/host.htm?tab=failed';

        $data['host_expiring_count'] = $HostModel->expiringCount();
        $data['host_expiring_count_url'] = request()->domain().'/'.env('DIR_ADMIN', 'admin').'/host.htm?tab=expiring';
