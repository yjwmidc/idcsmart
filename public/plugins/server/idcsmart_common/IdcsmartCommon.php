...
class IdcsmartCommon
{
    ...
    {
        return ['display_name'=>'通用产品', 'version'=>'3.0.1'];
    }
  ...
  `upgrade` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否支持升降级：1是,0否默认',
  PRIMARY KEY (`id`),
  KEY `product_id` (`product_id`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8mb4;",
...
//        return $res;
        ...
        try{
            ...
            if ($upstreamProduct){ // 同步代理模式
                ...
                if ($result['status']==200){
                    ...
                    if ($result['status']==200){
                        // 使用平台币支付
                        if (!empty($result['data']['customfields']['agent_use_coin'])){
                            idcsmart_api_curl($upstreamProduct['supplier_id'],'/console/v1/coin/pay',[
                                'auto'=>1,
                                'coin_coupon_ids'=>[],
    ...
    public function changeProduct($params)
    {
        ...
        # 配置中关联子模块,调子模块中开通,使用以前的逻辑
        $ProvisionLogic = new ProvisionLogic();
        $res = $ProvisionLogic->changeProduct($params['host']['id']);
        return $res;
    }
    ...
    public function cartCalculatePrice($params)
    {
        ...
        $hookResults = hook('idcsmart_common_cart_calculate_price',$params);
        foreach ($hookResults as $hookResult){
            if ($hookResult['status'] && $hookResult['status']!=200){
                return $hookResult;
            }
        }
