...
class IdcsmartCommonProductController extends BaseController
{
     ...
     * @title 批量开机
     * @desc 批量开机
     * @url /console/v1/idcsmart_common/batch_operate
     * @method  POST
     * @author wyh
     * @version v1
     * @param array id - 产品ID数组
     * @param string action - 动作：on开机off关机reboot重启hard_off强制关机hard_reboot强制重启crack_pass重置密码reinstall重装系统 require
     * @param string client_operate_password - 操作密码,需要验证时传
     * @param string password - 密码，动作为crack_pass时，需要传此字段
     * @param object reinstall - 动作为reinstall时，重装系统参数{"产品ID":{"os":"操作系统ID（option_param）","os_name":"操作系统名称(option_name)","option_id":"配置项ID(option_id)","sub_id":"配置子项ID(sub_id)"}}
     */
    public function batchOperate()
    {
        ...
        $result = $IdcsmartCommonProductModel->batchOperate($param);
     ...
     * @title 批量重装系统页面
     * @desc 批量重装系统页面
     * @url /console/v1/idcsmart_common/batch_reinstall
     * @method  GET
     * @author wyh
     * @version v1
     * @param array ids - 产品ID数组
     * @return object 配置 - 配置："/console/v1/idcsmart_common/host/:host_id/configoption此接口返回参数“拼接的数组
     */
    public function batchReinstallPage()
    {
        $param = $this->request->param();

        $IdcsmartCommonProductModel = new IdcsmartCommonProductModel();

        $result = $IdcsmartCommonProductModel->batchReinstallPage($param);
