...
class IdcsmartCommonProductConfigoptionModel extends Model
{
     ...
     * @return  int configoption.upgrade - 是否支持升降级：1是,0否默认
     */
    public function configoptionList($param)
    {
        $productId = $param['product_id']??0;

        $configoptions = $this->field('id,product_id,option_name,option_type,hidden,upgrade')
    ...
    public function upgradeConfigoption($param)
    {
        $this->startTrans();
        ...
        try{
            ...
            if (!isset($param['upgrade']) || !in_array($param['upgrade'],[0,1])){
                throw new \Exception(lang_plugins('param_error'));
            }
            ...
            if ($configoption['upgrade'] == $param['upgrade']){
                throw new \Exception(lang_plugins('cannot_repeat_operate'));
            }

            $configoption->save([
                'upgrade' => intval($param['upgrade'])
            ]);
