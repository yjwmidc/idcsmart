	...
	*/
	...
	public function vncPage()
	{
		...
		if(!empty($cache) && isset($param['tmp_token']) && $param['tmp_token'] === $cache['token']){
			...
			// 	Cache::set('mf_dcim_vnc_'.$param['id'], $cache, 30*60);
			// }
	 ...
	 * @title 重启VNC
	 * @desc 重启VNC
	 * @url /console/v1/mf_dcim/:id/vnc/restart
	 * @method  POST
	 * @author hh
	 * @version v1
	 * @param int id - 产品ID require
	 ...
	 * @title 退出救援模式
	 * @desc 退出救援模式
	 * @url /console/v1/mf_dcim/:id/rescue/exit
	 * @method  POST
	 * @author hh
	 * @version v1
	 * @param   int id - 产品ID require
	 ...
	 * @title 获取重装状态
	 * @desc 获取重装状态
	 * @url /console/v1/mf_dcim/:id/reinstall_status
	 * @method GET
	 * @author hh
	 * @version v1
	 * @param int id - 产品ID require
	...
	public function getReinstallStatus()
	{
		...
		try{
			$CloudLogic = new CloudLogic((int)$param['id']);
			$result = $CloudLogic->getReinstallStatus();
			return json($result);
		}catch(\Exception $e){
			return json(['status'=>400, 'msg'=>$e->getMessage()]);
		}
