...
const clientOperateVue = new Vue({
  ...
  methods: {
    ...
    autoPass() {
      let passLen = 9;
      if (
        this.cloudConfig.custom_rand_password_rule &&
        this.cloudConfig.default_password_length
      ) {
        passLen = this.cloudConfig.default_password_length - 3;
      }
      let pass =
        randomCoding(1) +
    ...
    getCycleList() {
      ...
      upgradePackagePrice(this.id, params)
        .then(async (res) => {
          if (res.data.status == 200) {
            let price = Number(res.data.data.price); // 当前产品的价格
            if (price < 0) {
            ...
            this.upParams.original_price =
              Number(price) + Number(res.data.data.discount);
            this.upParams.totalPrice =
              Number(price) + Number(res.data.data.discount);
            // 开启了等级优惠
            // if (this.isShowLevel) {
    ...
    upgradeSub() {
      ...
      if (
        ...
        this.ipv4DelArr.length === this.calcDelNum
      ) {
      } else {
