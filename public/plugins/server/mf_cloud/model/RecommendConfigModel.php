...
class RecommendConfigModel extends Model
{
     ...
     * @param   float param.on_demand_flow_price - 流量按需价格
     * @return  int status - 状态(200=成功,400=失败)
     * @return  string msg - 信息
     * @return  int data.id - 套餐ID
     */
    public function recommendConfigCreate($param): array
    {
        ...
        $param['on_demand_price'] = $param['on_demand_price'] ?? 0;
        $param['on_demand_flow_price'] = $param['on_demand_flow_price'] ?? 0;
     ...
     */
    public function recommendConfigList($param): array
    {
        ...
        if(isset($param['data_center_id']) && $param['data_center_id'] > 0){
            $where[] = ['rc.data_center_id', '=', $param['data_center_id']];
        }

        $list = $this
                ->alias('rc')
                ->field('rc.id,rc.name,rc.description,rc.order,rc.product_id,rc.data_center_id,rc.upgrade_range,rc.hidden,rc.upgrade_show,rc.ontrial,rc.ontrial_price,rc.ontrial_stock_control,rc.ontrial_qty,rc.on_demand_price,rc.on_demand_flow_price,l.bill_type')
                ->leftJoin('module_mf_cloud_line l', 'l.id=rc.line_id')
                ->where($where)
        ...
        if(!empty($list)){
            ...
            $firstDuration = $DurationModel->firstDuration($list[0]['product_id']);
            if(!empty($firstDuration)){
                ...
                foreach($list as $k=>$v){
                    ...
                    $list[$k]['on_demand_flow_price'] = amount_format($v['on_demand_flow_price'], 4);
                }
            }else{
                foreach($list as $k=>$v){
                    $list[$k]['price'] = '0.00';
     ...
     * @return  string msg - 信息
     */
    public function recommendConfigUpdate($param): array
    {
        ...
        $this->startTrans();
        try{
            $this->update($param, ['id'=>$recommendConfig['id']], ['name','description','order','data_center_id','line_id','cpu','memory','system_disk_size','system_disk_type','data_disk_size','data_disk_type','bw','flow','peak_defence','ip_num','gpu_num','ipv6_num','in_bw','traffic_type','due_not_free_gpu','ontrial','ontrial_price','ontrial_stock_control','ontrial_qty','on_demand_price','on_demand_flow_price']);
        ...
        $des = [
            ...
            'on_demand_flow_price' => lang_plugins('流量按需价格'),
     ...
     * @return  string msg - 信息
     */
    public function recommendConfigDelete($id): array
    {
     ...
     * @param   bool removeIpv6 - 排除IPv6套餐(VPC网络不能选择)
     * @param   bool isOndemand false 是否按需计费
     * @return  int list[].id - 套餐ID
     * @return  int list[].product_id - 商品ID
     * @return  string list[].name - 名称
     * @return  string list[].description - 描述
     ...
     * @return  int count - 总条数
     */
    public function getUpgradeRecommendConfig($current_recommend_config = [], $removeIpv6 = false, $isOndemand = false)
    {
        $data = [];
        $count = 0;
        ...
        if($current_recommend_config['upgrade_range'] == self::UPGRADE_DISABLE){
            ...
            $where = function($query) use ($current_recommend_config,$removeIpv6,$isOndemand) {
                $query->where('rc.product_id', '=', $current_recommend_config['product_id']);
                $query->where('rc.data_center_id', '=', $current_recommend_config['data_center_id']);
                $query->where('rc.id', '<>', $current_recommend_config['id']);
                $query->where('rc.upgrade_show', '=', 1);

                if($removeIpv6){
                    $query->where('rc.ipv6_num', '=', 0);
                }
                if($isOndemand){
            ...
            if(!empty($id)){
                $where = function($query) use ($current_recommend_config,$id,$removeIpv6,$isOndemand) {
                    $query->where('rc.product_id', '=', $current_recommend_config['product_id']);
                    $query->where('rc.data_center_id', '=', $current_recommend_config['data_center_id']);
                    $query->where('rc.id', 'IN', $id);
                    $query->where('rc.upgrade_show', '=', 1);
     ...
     */
    public function formatRecommendConfig($recommend_config): array
    {
        ...
        if(!empty($line)){
            ...
            if($line['bill_type'] == 'bw'){
                ...
                $optionFlow = $OptionModel->matchOption($productId, OptionModel::LINE_FLOW, $line['id'], $recommend_config['flow'], $recommend_config['bw']);
                if(!empty($optionFlow)){
