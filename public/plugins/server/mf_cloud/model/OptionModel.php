...
class OptionModel extends Model
{
    ...
    public function cpuIndex($id)
    {
        ...
        $data = [
            ...
            'on_demand_price' => $option['on_demand_price'],
        ];
        return $data;
    }
     ...
     * @return  string on_demand_price - 按需价格
     */
    public function lineFlowOnDemandIndex($id): array
    {
        $option = $this->optionIndex($id);
        if(empty($option) || $option['rel_type'] != OptionModel::LINE_FLOW_ON_DEMAND){
            return [];
        }
        $data = [
            'id' => $option['id'],
     ...
     */
    public function optionCreate($param): array
    {
        if(in_array($param['rel_type'], [OptionModel::CPU,OptionModel::MEMORY,OptionModel::SYSTEM_DISK,OptionModel::DATA_DISK])){
            ...
            $param['rel_id'] = 0;
        }else if(in_array($param['rel_type'], [OptionModel::LINE_BW,OptionModel::LINE_FLOW,OptionModel::LINE_DEFENCE,OptionModel::LINE_IP,OptionModel::LINE_IPV6,OptionModel::LINE_FLOW_ON_DEMAND])){
            ...
            // 流量按需只能添加一个
            if($param['rel_type'] == OptionModel::LINE_FLOW_ON_DEMAND){
                ...
                //         ->find();
                // if(!empty($exist)){
                //     return ['status'=>400, 'msg'=>lang_plugins('最多只能添加一个流量按需配置') ];
                // }
            }
        ...
        try{
            ...
            $insert = [
                ...
                'create_time'       => time(),
                'on_demand_price'   => $param['on_demand_price'] ?? '0.0000',
            ];
            ...
            }else if($param['rel_type'] == OptionModel::LINE_FLOW_ON_DEMAND){
                if($line['bill_type'] != 'flow'){
                    throw new \Exception(lang_plugins('mf_cloud_line_is_not_flow_cannot_add_flow_option'));
                }
                $insert['other_config'] = json_encode([
            ...
            if($type == 'radio'){
                ...
                }else if(in_array($param['rel_type'], [OptionModel::LINE_FLOW, OptionModel::LINE_FLOW_ON_DEMAND])){
                    $whereSame[] = ['other_config', 'LIKE', '%'.trim(str_replace('\\', '\\\\', json_encode(['out_bw'=>(int)$param['other_config']['out_bw'] ])), '{}').',%'];
                }
            ...
            if($param['rel_type'] == OptionModel::LINE_FLOW_ON_DEMAND){
                LineModel::where('id', $param['rel_id'])->update(['support_on_demand'=>1]);
            }
     ...
     */
    public function optionList($param, $field = null): array
    {
        ...
        $field .= ',product_id,on_demand_price';
        ...
        if(!empty($list)){
            ...
            $param['product_id'] = $param['product_id'] ?? $list[0]['product_id'];
            ...
            $firstDuration = $DurationModel->firstDuration($param['product_id']);

            if(!empty($firstDuration)){
                $PriceModel = new PriceModel();
                $durationPrice = $PriceModel->optionDurationPrice([
                    'duration_id'   => $firstDuration['id'],
                ...
                foreach($list as $k=>$v){
                    ...
                    if($v['type'] == 'step'){
                        ...
                        $list[$k]['price'] = $durationPrice[ $v['id'] ] ?? 0;
                        foreach($list as $kk=>$vv){
                            ...
                            if($v['min_value'] > $vv['max_value']){
                                ...
                                $list[$k]['price'] = bcadd($list[$k]['price'], bcmul($durationPrice[ $vv['id'] ] ?? 0, $vv['min_value']==0?$vv['max_value']:$vv['max_value']-$vv['min_value']+1));
                                $list[$k]['on_demand_price'] = bcadd($list[$k]['on_demand_price'], bcmul($vv['on_demand_price'], $vv['min_value']==0?$vv['max_value']:$vv['max_value']-$vv['min_value']+1, 4), 4);
                            }
                        }
                    }else if($v['type'] == 'total'){
                        $list[$k]['price'] = !empty($durationPrice[ $v['id'] ]) ? bcmul($durationPrice[$v['id']], $v['min_value']) : '0.00';
                        $list[$k]['on_demand_price'] = bcmul($v['on_demand_price'], $v['min_value'], 4);
                    }else{
                        $list[$k]['price'] = $durationPrice[$v['id']] ?? '0.00';
                        $list[$k]['on_demand_price'] = amount_format($v['on_demand_price'], 4);
                    }
                    // 为0时显示
                    if ($v['type'] != 'radio' && isset($v['min_value']) && $v['min_value'] == 0){
                        $list[$k]['price'] = '0.00';
                    }
                ...
                foreach($list as $k=>$v){
                    ...
                    if($v['type'] == 'step'){
                        ...
                        // $list[$k]['on_demand_price'] = $v['on_demand_price'];
                        foreach($list as $kk=>$vv){
                            // 范围内的阶梯
                            if($v['min_value'] > $vv['max_value']){
                                if($param['rel_type'] == OptionModel::SYSTEM_DISK || $param['rel_type'] == OptionModel::DATA_DISK){
    ...
    public function optionUpdate($param)
    {
        ...
        try{
            ...
            $update['on_demand_price'] = $param['on_demand_price'] ?? $option['on_demand_price'];
            ...
            if(!empty($priceArr) && $option['rel_type'] != OptionModel::LINE_FLOW_ON_DEMAND){
                $PriceModel = new PriceModel();
                $PriceModel->insertAll($priceArr);
            }
            $this->commit();
        ...
        $des = [
            ...
            'on_demand_price' => lang_plugins('按需价格'),
        ];
        ...
        $new = [
            ...
            'on_demand_price' => $param['on_demand_price'] ?? $option['on_demand_price'],
        ];
        ...
        }else if($option['rel_type'] == OptionModel::LINE_FLOW_ON_DEMAND){
            $des['in_bw'] = lang_plugins('mf_cloud_line_flow_in_bw');
            $des['out_bw'] = lang_plugins('mf_cloud_line_flow_out_bw');
            $des['traffic_type'] = lang_plugins('mf_cloud_line_flow_traffic_type');
    ...
    public function optionDelete($id, $rel_type = null)
    {
        ...
        try{
            ...
            // 删除完了线路部分配置,自动关闭开关
            if(in_array($option['rel_type'], [OptionModel::LINE_DEFENCE, OptionModel::LINE_IP,OptionModel::LINE_IPV6,OptionModel::LINE_FLOW_ON_DEMAND])){
                // 是否还有对应配置,没有自动关闭开关
                ...
                if(empty($other)){
                    if($option['rel_type'] == OptionModel::LINE_DEFENCE){
                        ...
                        LineModel::where('id', $option['rel_id'])->update(['support_on_demand'=>0]);
                    }
                }
    ...
    {
        $option = $this
                ->field('id,product_id,rel_type,rel_id,type,value,min_value,max_value,step,other_config,firewall_type,defence_rule_id,on_demand_price')
                ->find($id);
        if(empty($option)){
            return [];
        }
        ...
        $option['on_demand_price'] = amount_format($option['on_demand_price'], 4);
     ...
     * @return  OptionModel|null option - 配置模型实例
    ...
    public function optionDurationPrice($productId, $relType, $relId = 0, $value = 0, $diskType = NULL, $freeSize = NULL)
    {
        ...
        $option = $this->matchOption($productId, $relType, $relId, $value, $diskType);
        if(empty($option)){
            return ['match'=>false, 'option'=>$option, 'price'=>$data];
        }
        $optionId = $option['id'];
        $PriceModel = new PriceModel();
        ...
        if($type == 'radio'){
            $data = $PriceModel
                    ->field('duration_id,price')
                    ->where('product_id', $productId)
                    ->where('rel_type', PriceModel::REL_TYPE_OPTION)
                    ->where('rel_id', $optionId)
                    ->select()
                    ->toArray();
            ...
            if(!is_null($diskType)){
                if(in_array($relType, [OptionModel::DATA_DISK,OptionModel::SYSTEM_DISK])){
                    $diskType = (string)$diskType;
                    $wherePrice[] = ['o.other_config', 'LIKE', rtrim(str_replace('\\', '\\\\', json_encode(['disk_type'=>$diskType])), '}').'%' ];
                }else if(in_array($relType, [OptionModel::LINE_FLOW,OptionModel::LINE_FLOW_ON_DEMAND])){
                    $diskType = (int)$diskType;
                    $wherePrice[] = ['o.other_config', 'LIKE', '%'.trim(str_replace('\\', '\\\\', json_encode(['out_bw'=>$diskType])), '{}').',%'];
                }
            }
        ...
        return ['match'=>true, 'option'=>$option, 'price'=>$data];
    }
     ...
     * @param   int  $relId     - 关联ID(rel_type=2,3,4,5,8,9时是线路ID) require
     * @param   int  $value     - 当前值 require
     * @param   int|string durationId 0 周期ID(数字=周期ID,on_demand=按需)
     * @param   string diskType NULL 磁盘类型
     * @param   int|null freeSize null 免费磁盘大小(不是null时,会减掉免费部分的价格)
     * @param    $name
     * @return  array
     * @return  bool match - 是否有该配置(false=无,true=有)
     ...
     */
    public function matchOptionDurationPrice($productId, $relType, $relId = 0, $value = 0, $durationId = 0, $diskType = NULL, $freeSize = NULL): array
    {
        $option = $this->matchOption($productId, $relType, $relId, $value, $diskType);
        if(empty($option)){
            return ['match'=>false, 'option'=>$option, 'price'=>'0.00' ];
        }
        // 按需4位,其他2位
        $scale = $durationId == 'on_demand' ? 4 : 2;
        bcscale($scale);
        ...
        if($type == 'radio'){
            // 按需
            if($durationId == 'on_demand'){
                $price = $option['on_demand_price'];
            }else{
                $price = $PriceModel
                    ->where('product_id', $productId)
                    ->where('rel_type', PriceModel::REL_TYPE_OPTION)
            ...
            if($durationId == 'on_demand'){
                $priceArr = $this
                        ->alias('o')
                        ->field('o.min_value,o.max_value,o.on_demand_price price')
                        ->where($wherePrice)
                        ->order('o.min_value', 'asc')
                        ->select();
            }else{
                ...
                $priceArr = $this
                        ->alias('o')
                        ->field('o.min_value,o.max_value,p.rel_id,p.price')
                        ->where($wherePrice)
        ...
        return ['match'=>true, 'price'=>amount_format($price, $scale), 'option'=>$option ];
    }
     ...
     * 时间 2025-03-26
     * @title 匹配对应配置
     * @desc  匹配对应配置,成功返回对应配置模型实例,失败返回null
     * @author hh
     * @version v1
     * @param   int  productId - 商品ID require
     * @param   int  relType   - 配置类型(0=CPU配置,1=内存配置,2=线路带宽配置,3=线路流量配置,4=线路防护配置,5=线路附加IP,6=系统盘,7=数据盘,8=线路显卡,9=线路IPv6) require
     * @param   int  relId     - 关联ID(rel_type=2,3,4,5,8,9时是线路ID) require
     * @param   int  value     - 当前值 require
     * @param   string  diskType - 磁盘类型/流量线路出带宽大小
     * @return  OptionModel|null
     */
    public function matchOption($productId, $relType, $relId = 0, $value = 0, $diskType = NULL)
    {
        $whereOption = [];
        ...
        if($diskType !== null){
            if(in_array($relType, [OptionModel::DATA_DISK,OptionModel::SYSTEM_DISK])){
                ...
                $diskType = (int)$diskType;
                $whereOption[] = ['other_config', 'LIKE', '%'.trim(str_replace('\\', '\\\\', json_encode(['out_bw'=>$diskType])), '{}').',%'];
            }
        }

        $option = NULL;
        // 先获取价格配置方式
        $type = $this->where($whereOption)->value('type');
        if($type == 'radio'){
            $whereOption[] = ['value', '=', $value];

            $option = $this->where($whereOption)->find();
        }else if($type == 'step' || $type == 'total'){
