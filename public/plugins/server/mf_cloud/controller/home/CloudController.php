...
class CloudController
{
     ...
     * @return  string config.custom_rand_password_rule - 自定义随机密码位数(0=关闭,1=开启)
     * @return  string config.default_password_length - 默认密码长度
	...
	* @version v1
     ...
     * @param   string status - 产品状态(Unpaid=未付款,Pending=开通中,Active=已开通,Suspended=已暂停,Deleted=已删除,Grace=宽限,Keep=保留)
     ...
     * @return  string list[].status - 产品状态(Unpaid=未付款,Pending=开通中,Active=已开通,Suspended=已暂停,Deleted=已删除,Grace=宽限,Keep=保留)
	 ...
	 * @return  int list[].billing_cycle - 计费方式(free=免费,onetime=一次性,recurring_prepayment=包年包月,on_demand=按需)
   ...
   * @param   string client_operate_password - 操作密码,需要验证时传 
	...
	public function calBackupConfigPrice()
	{
		...
		if (!$BackupConfigValidate->scene('buy')->check($param)){
 		...
 		try{
			...
			$result = $CloudLogic->calConfigPrice($param);
	...
	public function createBackupConfigOrder()
	{
		...
		try{
			...
			$result = $CloudLogic->createBackupConfigOrder($param);
        }catch(\Exception $e){
			$result = ['status'=>400, 'msg'=>$e->getMessage()];
        }
        return json($result);
 ...
 //        $VpcNetworkModel = new VpcNetworkModel();
     ...
     * @return  string renew_price_difference - 续费差价
     * @return  string base_price - 基础价格
	 * @return  string on_demand_flow_price - 按需流量价格
	 * @return  int settle_time - 按需结算时间
	 */
	 ...
	 * @title 实例流量数据
	 * @desc  实例流量数据
	 * @url console/v1/mf_cloud/:id/flow_data
	 * @method  GET
	 * @author hh
	 * @version v1
	 * @param   int id - 产品ID require
	...
	public function flowData()
	{
		...
		try{
			...
			$data = $CloudLogic->flowData($param);
		}catch(\Exception $e){
			$data = [
				'list'	=> [],
				'unit'	=> 'B',
