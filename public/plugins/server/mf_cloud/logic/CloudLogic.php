            	...
            	if($res['status']==200){
            		...
            		if(!empty($cache)){
            			if($this->isClient){
			                ...
			                $result['data']['url'] = request()->domain().'/'.env('DIR_ADMIN', 'admin').'/v1/mf_cloud/'.$this->hostModel['id'].'/vnc';
			            }
			            // 生成一个临时token
    ...
    public function calDiskPrice(&$param)
    {
		...
		if($isOnDemand){
			...
			if(in_array('data_disk', $productOnDemand['keep_time_billing_item'])){
				$keepTimePriceDifference = $priceDifference;
			}

			$showPrice = $this->calOnDemadShowPrice($priceDifference, $discountPrice);
    ...
    public function calResizeDiskPrice($param)
    {
		...
		$orderItem = [];	// 要添加的用户等级子项
       	foreach($param['resize_data_disk'] as $k=>$v){
			...
			// 累计用户等级折扣金额（非按需计费）
            if(!empty($config) && $config['level_discount_data_disk_upgrade'] == 1){
                $discountPrice = bcadd($discountPrice, bcsub($diskNewPrice, $diskOldPrice, 4), 4);
            }
       	}
		if(empty($resizeDisk)){
		...
		if($isOnDemand){
			...
			if(!$isDownstream){
				...
				if(!empty($clientLevel) && $discountPrice > 0){
					// 应用价格因子
					if(!empty($duration['price_factor']) && $duration['price_factor'] != 1){
						$discountPrice = bcmul($discountPrice, $duration['price_factor'], 4);
					}
    	...
    	if($line['ip_enable'] == 1){
    		...
    		if($oldIpNum != $param['ip_num']){
		        ...
		        if(!empty($config) && $config['level_discount_ipv4_upgrade'] == 1){
		            $discountPrice = bcadd($discountPrice, bcsub($ipv4NewPrice, $ipv4OldPrice, 4), 4);
		        }
    	...
    	if($line['ipv6_enable'] == 1 && $configData['network_type'] == 'normal'){
    		...
    		if(isset($param['ipv6_num']) && $oldIpv6Num != $param['ipv6_num']){
		        ...
		        if(!empty($config) && $config['level_discount_ipv6_upgrade'] == 1){
		            $discountPrice = bcadd($discountPrice, bcsub($ipv6NewPrice, $ipv6OldPrice, 4), 4);
		        }
    ...
    public function calCommonConfigPrice($param)
    {
    	...
    	if($param['cpu'] != $configData['cpu']['value']){
            ...
            if(!empty($config) && $config['level_discount_cpu_upgrade'] == 1){
                $discountPrice = bcadd($discountPrice, bcsub($cpuNewPrice, $cpuOldPrice, 4), 4);
            }
        ...
        if($param['memory'] != $configData['memory']['value']){
            ...
            if(!empty($config) && $config['level_discount_memory_upgrade'] == 1){
                $discountPrice = bcadd($discountPrice, bcsub($memoryNewPrice, $memoryOldPrice, 4), 4);
            }
    	...
    	if(empty($line)){
    		...
    		if($line['bill_type'] == 'bw'){
                ...
                if(isset($param['bw']) && !empty($param['bw']) && $param['bw'] != $configData['bw']['value']){
                    ...
                    if(!empty($config) && $config['level_discount_bw_upgrade'] == 1){
                        $discountPrice = bcadd($discountPrice, bcsub($bwNewPrice, $bwOldPrice, 4), 4);
                    }
                ...
                if($flowChange || $bwChange){
					...
					if($isOnDemand){
						...
						// 累计用户等级折扣金额
						$discountPrice = bcadd($discountPrice, bcsub($flowNewPrice, $flowOldPrice, 4), 4);
            ...
            if($line['defence_enable'] == 1 && $line['sync_firewall_rule'] == 0 && isset($param['peak_defence']) && is_numeric($param['peak_defence']) && $param['peak_defence'] >= 0 && $param['peak_defence'] != ($configData['defence']['value'] ?? 0)){
                ...
                // 累计用户等级折扣金额
                $discountPrice = bcadd($discountPrice, bcsub($defenceNewPrice, $defenceOldPrice, 4), 4);
		...
		if($isOnDemand){
			...
			$realPriceDifference = 0;
			$price = 0;
			$renewPriceClientLevelDiscount = 0;
        ...
        $result = [
            ...
            'data'   => [
				...
				'renew_price'				=> $renewPrice,
				'renew_price_client_level_discount' => $renewPriceClientLevelDiscount ?? '0.0000',
				'on_demand_flow_price'		=> $onDemandFlowPrice ?? -1,
...
//        ]);
	 ...
	 * @author hh
	 * @version v1
	 * @param   float priceDifference - 升降级差价 require
	 * @param   float discountPrice - 用户等级折扣部分,就是升降级差价 require
	 * @return  string base_renew_price - 原续费价格
	 * @return  string renew_price - 实际续费价格(折扣后)
	 * @return  string renew_price_client_level_discount - 续费价格用户等级折扣
	 */
	public function calOnDemadShowPrice($priceDifference, $discountPrice): array
	{
		// 原续费价格
		$baseRenewPrice = bcadd($this->hostModel['base_renew_amount'], $priceDifference, 4);
		$baseRenewPrice = $baseRenewPrice >= 0 ? $baseRenewPrice : 0;
		$baseRenewPrice = amount_format($baseRenewPrice, 4);

		$renewPrice = bcadd($this->hostModel['renew_amount'], $priceDifference, 4);
		$renewPrice = $renewPrice >= 0 ? $renewPrice : 0;
		...
		foreach ($hookDiscountResults as $hookDiscountResult){
			if ($hookDiscountResult['status'] == 200){
				$renewPriceClientLevelDiscount = $hookDiscountResult['data']['discount'] ?? 0;
				$renewPrice = bcsub($renewPrice, $renewPriceClientLevelDiscount, 4);
			}
		}
		// }
		// 有折扣
		...
		return ['base_renew_price'=>$baseRenewPrice, 'renew_price'=>$renewPrice, 'renew_price_client_level_discount'=>$renewPriceClientLevelDiscount ];
	}
