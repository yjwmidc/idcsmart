  ...
  methods: {
    ...
    changeOpt(type) {
      switch (type) {
        ...
        case "quitRescue":
          return this.showQuitRescueDialog();
        case "reinstall":
    ...
    getIpPrice() {
      ...
      ipPrice({
        ...
        .then(async (res) => {
          ...
          const ipPrice = res.data.data.price * 1 - this.ipCodePrice * 1;
          this.ipMoney = ipPrice > 0 ? ipPrice.toFixed(2) : 0;
          // 按需费用
          if (this.isDemandFee) {
            const temp_price =
      ...
      this.orderTimer = setTimeout(() => {
        ...
        diskPrice(params)
          .then(async (res) => {
            ...
            this.moreDiskPrice =
              (price * 1000 - this.moreCodePrice * 1000) / 1000 > 0
                ? (price * 1000 - this.moreCodePrice * 1000) / 1000
                : 0;
            // 按需费用
            if (this.isDemandFee) {
              const temp_price =
        ...
        expanPrice(params)
          .then(async (res) => {
            ...
            this.expansionDiskPrice =
              (price * 1000 - this.expansionCodePrice * 1000) / 1000 > 0
                ? (price * 1000 - this.expansionCodePrice * 1000) / 1000
                : 0;
            // 按需费用
            if (this.isDemandFee) {
              const temp_price =
    ...
    async getBsPrice(bol = false) {
      try {
        ...
        this.bsData.money =
          (price * 1000 - this.bsData.codePrice * 1000) / 1000 > 0
            ? (price * 1000 - this.bsData.codePrice * 1000) / 1000
            : 0;
        // 按需费用
        if (this.isDemandFee) {
          const temp_price =
