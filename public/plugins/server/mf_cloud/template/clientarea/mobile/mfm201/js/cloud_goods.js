...
const app2 = Vue.createApp({
  ...
  data() {
    return {
      ...
      params: {
        ...
        billing_cycle: "recurring_prepayment", //on_demand recurring_prepayment
      },
      plan_way: 0,
      hover: false,
      ...
      productInfo: {},
      timeObj: {
        second: lang.mf_second,
        minute: lang.mf_minute,
        hour: lang.mf_hour,
      },
      renewPrice: 0,
      demandRatio: 10000,
      demandPrice: {
        renew_price_total: 0,
        renew_price: 0,
        renew_price_client_level_discount: 0,
        base_on_demand_flow_price: 0,
        on_demand_flow_price: 0,
        on_demand_flow_price_client_level_discount: 0,
      },
    };
  },
  mounted() {
  ...
  watch: {
    "params.bw"(val) {
      this.bwName = val === 0 ? lang.not_limited : val + "M";
    },
    "params.billing_cycle"(val) {
      this.billTypeName =
        val === "on_demand" ? lang.mf_demand_fee : lang.mf_month_year;
    },
    "params.image_id"(id) {
      if (id) {
  ...
  computed: {
    calcBillWayArr() {
      if (this.productInfo.pay_type === "on_demand") {
        return [{text: lang.mf_demand_fee, value: "on_demand"}];
      } else {
        return [
          {text: lang.mf_month_year, value: "recurring_prepayment"},
          {text: lang.mf_demand_fee, value: "on_demand"},
        ];
      }
    },
    calcFilterPrice() {
      return (money) => {
        if (isNaN(money) || money * 1 < 0) {
          const num =
            this.params.billing_cycle === "on_demand" ? "0.000" : "0.00";
          return num;
        } else {
          const point = this.params.billing_cycle === "on_demand" ? 4 : 2;
  ...
  methods: {
    ...
    // 配置数据
    async getConfig(bol = true, type) {
      try {
        // 获取商品的周期类型
        if (bol) {
          const proRes = await apiProductDetail({
            id: this.id,
          });
          this.productInfo = proRes.data.data.product;
        }
        const params = {
          id: this.id,
          billing_cycle:
            type ||
            (this.productInfo.pay_type === "on_demand"
              ? "on_demand"
        ...
        // 初始化数据
        let temp_data_center_id = "";
        if (!bol) {
          // 切换按需，备份之前的数据中心
          temp_data_center_id = this.params.data_center_id;
        }
        if (!this.isUpdate) {
          // 不是回填
          this.params = {
            data_center_id: temp_data_center_id,
            cpu: "",
            memory: 1,
          ...
          this.billTypeName =
            params.billing_cycle === "on_demand"
              ? lang.mf_demand_fee
              : lang.mf_month_year;
          this.qty = 1;
    ...
    handlerCustom(bol = false) {
      ...
      if (!this.isUpdate) {
        ...
        if (!this.params.line_id && bol) {
          const temp = this.dataList
            .filter((item) => item.id === this.country * 1)[0]
            ?.city.filter((item) => item.name === this.city)[0]
            ?.area.filter((item) => item.name === this.area_name);
          this.params.line_id = temp[0]?.line[0]?.id;
          this.getLineDetails(this.params.line_id);
        }
        if (bol) {
          this.getCycleList();
        }
    ...
    handleClick() {
      ...
      this.getConfig(false, this.params.billing_cycle);
    },
    // 选择区域
    changeArea(val) {
      const e = val[0].name;
    ...
    async changeConfig() {
      try {
        ...
        const {
          renew_price_total,
          renew_price,
          renew_price_client_level_discount,
          base_on_demand_flow_price,
          on_demand_flow_price,
          on_demand_flow_price_client_level_discount,
        } = res.data.data;
        // 按需价格
        this.demandPrice.renew_price_total = renew_price_total;
        this.demandPrice.renew_price = renew_price;
        this.demandPrice.renew_price_client_level_discount =
          renew_price_client_level_discount;
        this.demandPrice.base_on_demand_flow_price = base_on_demand_flow_price;
        this.demandPrice.on_demand_flow_price = on_demand_flow_price;
        this.demandPrice.on_demand_flow_price_client_level_discount =
          on_demand_flow_price_client_level_discount;
