...
class TpCaptchaLogic
{
    public function baseDescribe()
    {
        ...
        if(isset($config['code_set'])){
            if($config['code_set']==1){
                $config['code_set'] = '2345678';
            }else if($config['code_set']==2){
                $config['code_set'] = 'abcdefhijkmnpqrstuvwxyzABCDEFGHJKLMNPQRTUVWXY';
            }else if($config['code_set']==3){
                $config['code_set'] = '2345678abcdefhijkmnpqrstuvwxyzABCDEFGHJKLMNPQRTUVWXY';
            }else{
                $config['code_set'] = '2345678';
            }
        }
                            ...
                            $.ajax({
                                ...
                                success: function (result) {
                                    if (result.status === 200) {
                                        ...
                                        resolve({ success: true, value: inputValue, token: tokenTag })
                                    } else {
                                        resolve({ success: false, msg: result.msg })
                                    }
                                },
                                error: function (error) {
                                    resolve({ success: false, msg: error.msg })
                                }
                            })
                        })
                    }
