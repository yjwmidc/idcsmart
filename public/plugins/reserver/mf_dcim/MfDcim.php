...
class MfDcim
{
	...
	{
		return ['display_name'=>'DCIM代理(自定义配置)', 'version'=>'3.0.2'];
	}
				...
				if($result['status'] == 200){
                    ...
                    if ($result['status'] == 200){
                        ...
                        if (!empty($result['data']['customfields']['agent_use_coin'])){
                            $RouteLogic->curl('/console/v1/coin/pay',[
                                'auto'=>1,
                                'coin_coupon_ids'=>[],
    ...
    */
    ...
    public function clientArea()
    {
        if (use_mobile()){ // 手机端
            ...
            $mobileTheme = $homeHostThemeMobile['mf_dcim'] ?? 'default';
            $type = 'mobile';
            // 1、配置主题没有走默认的
            ...
            $clientareaTheme = $homeHostTheme['mf_dcim'] ?? 'default';
