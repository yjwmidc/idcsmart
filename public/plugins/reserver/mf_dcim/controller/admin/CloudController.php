...
class CloudController
{
	 ...
	 * @title 重启VNC
	 * @desc 重启VNC
	 * @url /admin/v1/remf_dcim/:id/vnc/restart
	 * @method  POST
	 * @author hh
	 * @version v1
	 * @param int id - 产品ID require
	...
	public function restartVnc()
	{
		...
		try{
			...
			if(!$RouteLogic->isUpstream){
				if(class_exists('\server\mf_dcim\controller\admin\CloudController')){
					return (new \server\mf_dcim\controller\admin\CloudController())->restartVnc();
				}else{
					$result = ['status'=>400, 'msg'=>lang_plugins('res_mf_dcim_error_act')];
				}
			}else{
