...
class MfFinance{
	...
	public function metaData(){
		return ['display_name'=>'魔方财务代理(自定义配置)', 'version'=>'3.0.0'];
	}
	...
	public function changePackage($params){
        ...
        if (!empty($supplier) && $supplier['type']=='default' && $upstreamProduct['res_module']=='mf_finance'){
            ...
            if ($result['status']==200){
                ...
                $result2 = $RouteLogic->curl('console/v1/credit', $creditData, 'POST');
                if ($result2['status']==200){
                    // 使用平台币支付
                    if (!empty($result2['data']['customfields']['agent_use_coin'])){
                        $RouteLogic->curl('console/v1/coin/pay',[
                            'auto'=>1,
                            'coin_coupon_ids'=>[],
                            'order_id' => $result['data']['id']??0,
                            'use' => 1,
                        ],'POST');
                    }
                }
