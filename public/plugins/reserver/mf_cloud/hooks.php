...
add_hook('fifteen_minute_cron', function(){
    ...
    echo '处理魔方云代理流量通知检查开始:',date('Y-m-d H:i:s'),PHP_EOL;

	$where = function($query){
		$query->where('up.res_module', '=', 'mf_cloud');
	...
	foreach($host as $v){
		...
		try{
			...
			$flowDetail = $RouteLogic->curl( sprintf('console/v1/remf_cloud/%d/flow', $RouteLogic->upstream_host_id), [], 'GET');
    ...
    echo '处理魔方云代理流量通知检查完成:',date('Y-m-d H:i:s'),PHP_EOL;
});