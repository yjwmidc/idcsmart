  ...
  KEY `admin_id` (`admin_id`)
	...
	public function upgrade()
	{
		...
		if(isset($plugin['version'])){
            ...
            if(version_compare('2.3.2', $plugin['version'], '>')){
                $sql[] = "INSERT INTO `idcsmart_plugin_hook`(`name`, `status`, `plugin`, `module`, `order`) VALUES ('home_client_index', 1, 'IdcsmartWithdraw', 'addon', 0);";
            }
		}
		foreach ($sql as $v){
			Db::execute($v);
    ...
    public function homeClientIndex($param)
    {
        $IdcsmartWithdrawModel = new IdcsmartWithdrawModel();
        return $IdcsmartWithdrawModel->homeClientIndex($param);
    }
