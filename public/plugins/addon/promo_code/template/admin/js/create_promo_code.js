      ...
      data () {
        return {
          ...
          formData: {
            ...
            loop: 0,
            cycle_limit: 0,
            on_demand_to_recurring_prepayment: 0,
            cycle: [],
            notes: "",
            products: [],
            need_products: [],
          ...
          rules: {
            ...
            percentVal: [
              { required: true, message: this.calcPlaceholder, type: 'error' },
              {
        ...
        async onSubmit ({ validateResult, firstError }) {
          if (validateResult === true) {
            try {
              ...
              params.start_time = parseInt(parseInt(moment(params.start_time).valueOf()) / 1000);
              if (params.end_time) {
                params.end_time = parseInt(parseInt(moment(params.end_time).valueOf()) / 1000);
              }
              if (this.optType === "add") {
                delete params.id;
              } else {
