...
class PromoCodeModel extends Model
{
    ...
    {
        $promoCode =  $this->field('id,code,type,value,client_type,client_level,start_time,end_time,max_times,single_user_once,upgrade,host_upgrade,renew,loop,cycle_limit,cycle,notes,on_demand_to_recurring_prepayment')
            ->where('delete_time',0)
            ->find($param['id']);
        if (empty($promoCode)){
            return (object)[];
                ...
                if(isset($param['host_id']) && !empty($param['host_id']) && $param['scene']=='new'){
                    ...
                    if(!empty($host) && $host['is_delete'] == 0){
                        ...
                        if($host['billing_cycle'] != 'on_demand'){
                            $update['renew_amount'] = $PromoCodeModel['loop']==1 ? (($host['renew_amount']-$resultRenew['data']['discount']??0)>0 ? ($host['renew_amount']-$resultRenew['data']['discount']??0) : 0) : $host['renew_amount'];
                        }
    ...
    public function afterOrderCreate($param)
    {
        ...
        $orderItems = $OrderItemModel->alias('oi')
            ...
            ->whereIn('oi.type',['host','renew','upgrade','change_billing_cycle']) # 仅新购/续费/升降级可使用优惠码
            ->select()
            ->toArray();
        if (empty($orderItems)){
            return false;
