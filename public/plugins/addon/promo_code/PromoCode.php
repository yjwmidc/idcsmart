  ...
  KEY `client_id` (`client_id`)
    ...
    public function applyPromoCode($param)
    {
        ...
        if (!empty($host) && $host['is_delete'] == 0){
            ...
            // 当变更计费方式
            if($param['scene'] == 'change_billing_cycle'){
                // 传了优惠码,这里需要计算续费折扣,使用新的优惠码计算
                if(!empty($param['promo_code'])){
                    $postData['promo_code'] = $param['promo_code'];
                }else if(!empty($postData['promo_code'])){
                    // 以前用过优惠码,当成续费场景来使用
                    $postData['scene'] = 'renew';
                }
            }
