...
class CertificationLogModel extends Model
{
        ...
        try{
            ...
            if (IdcsmartCertificationLogic::getDefaultConfig('certification_update_client_name')){
                if ($log['type']==1){
                    ...
                    $log['custom_fields_json'] = json_decode($log['custom_fields_json'],true) ?? [];
                    $update = [
                        'username' => $log['custom_fields_json']['custom_fields1'] ?? $log['company'],
                        'company' => $log['company'],
                        'update_time' => time()
                    ];
                }
    ...
    public function reject($param)
    {
        ...
        $reason = $param['reason'] ?? $log['auth_fail'];
     ...
     * @return int certification_show_certify_id - 是否显示认证ID
    ...
    public function certificationInfo($param)
    {
        ...
        $configuration = [
            ...
            'certification_show_certify_id' => $config['certification_show_certify_id']??0,
        ];
        ...
        if ($status==1){
            ...
            if ($config['certification_update_client_name']){
                if ($certificationLog['type']==1){
                    ...
                    $certificationLog['custom_fields_json'] = json_decode($certificationLog['custom_fields_json'],true) ?? [];
                    $update = [
                        'username' => $certificationLog['custom_fields_json']['custom_fields1'] ?? $certificationLog['company'],
                        'company' => $certificationLog['company'],
                        'update_time' => time()
                    ];
                }
                $ClientModel = new ClientModel();
