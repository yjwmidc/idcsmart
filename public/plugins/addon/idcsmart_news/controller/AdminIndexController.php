...
class AdminIndexController extends PluginAdminBaseController
{
     ...
     * @author wyh
     * @version v1
     * @url /admin/v1/news/:id/top
     * @method  PUT
     * @param int id - 新闻ID required
     * @param int is_top - 0取消置顶1置顶 required
     */
    public function topIdcsmartNews()
    {
        $param = $this->request->param();

        // 参数验证
        if (!$this->validate->scene('top')->check($param)){
            return json(['status' => 400 , 'msg' => lang_plugins($this->validate->getError())]);
        }
        ...
        // 置顶新闻
        $result = $IdcsmartNewsModel->topIdcsmartNews($param);
