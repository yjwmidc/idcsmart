...
// 手动处理重试
function apiRetry(params) {
  return Axios.post(`/host/retry`, params);
}

// 冻结余额
function apiFreezeCredit(params) {
  return Axios.post(`/client/${params.client_id}/credit/freeze`, params);
}

// 冻结记录
function apiFreezeRecord(params) {
  return Axios.get(`/client/${params.client_id}/credit/freeze`, {params});
}

// 解冻余额
function apiUnFreezeCredit(params) {
  return Axios.post(`/client/${params.client_id}/credit/unfreeze`, params);
}

// 用户可用平台币详情
function apiClientCoin(params) {
  return Axios.get(`/coin/client/coupon`, {params});
}

// 设置订单发票开关
function apiOrderInvoiceSwitch(params) {
  return Axios.put(`/invoice/order/switch`, params);
}
