  ...
  <com-config>
    ...
    <t-card class="list-card-container">
      <div class="common-header top">
        <div class="left">
          <t-button @click="createTemplate" class="add"
            v-permission="'auth_system_interface_sms_notice_sms_template_create_template'">{{lang.create_template}}</t-button>
          <t-button @click="batchSubmit" theme="default" class="add com-gray-btn"
            v-permission="'auth_system_interface_sms_notice_sms_template_batch_create_template'">{{lang.batch_submit}}</t-button>
          <t-button @click="batchReSubmit" theme="default" class="add com-gray-btn"
            v-permission="'auth_system_interface_sms_notice_sms_template_batch_create_template'">{{lang.batch_re_submit}}</t-button>
          <t-button theme="default" @click="back" class="add com-gray-btn">{{lang.back}}</t-button>
        </div>
      </div>
      <t-table row-key="id" :data="data" size="medium" @select-change="rehandleSelectChange"
        :selected-row-keys="selectedRowKeys" :columns="columns" :hover="hover" :loading="loading"
        ...
        <template #status="{row}">
          <t-tag theme="default" class="com-status" v-if="row.status===0" variant="light">{{lang.no_submit}}
          </t-tag>
          <t-tag theme="success" class="com-status" v-if="row.status===1" variant="light">{{lang.under_review}}
          </t-tag>
          <t-tag theme="primary" class="com-status" v-if="row.status===2" variant="light">{{lang.pass}}</t-tag>
        ...
        <template #op="{row}">
          <t-tooltip :content="lang.edit" :show-arrow="false" theme="light">
            <t-icon name="edit-1" class="common-look" @click="updateHandler(row)" 
              :class="{disable: row.status===1 || (formData.name === 'Aliyunnew' && row.status === 2)}"
              v-permission="'auth_system_interface_sms_notice_sms_template_update_template'">
            </t-icon>
          </t-tooltip>
          <t-tooltip :content="lang.test" :show-arrow="false" theme="light">
            <a class="common-look" @click="testHandler(row)"
              v-if="row.status===2 && $checkPermission('auth_system_interface_sms_notice_sms_template_test_template')">
          ...
          <t-tooltip :content="lang.delete" :show-arrow="false" theme="light">
            <t-icon name="delete" class="common-look" @click="deleteHandler(row)"
              v-permission="'auth_system_interface_sms_notice_sms_template_delete_template'">
            </t-icon>
          </t-tooltip>
        </template>
      </t-table>
    ...
    <t-dialog :visible.sync="visible" :header="addTip" :on-close="close" :footer="false" width="600"
      ...
      <t-form :rules="rules" :data="formData" ref="userDialog" @submit="onSubmit" v-if="visible">
        ...
        <!-- Aliyun 独有 -->
        <template v-if="formData.name === 'Aliyun' || formData.name === 'Aliyunnew'">
    ...
    <!-- 测试 -->
    <t-dialog :header="lang.sms_test" :visible.sync="statusVisble" :footer="false" :width="isEn ? 700 : 600"
      class="test-dialog">
      <t-form :rules="rules" :data="testForm" ref="userDialog" @submit="testSubmit">
        <t-form-item :label="lang.phone" name="phone">
          <t-select v-model="testForm.phone_code" filterable style="width: 150px" :placeholder="lang.phone_code"
            :disabled="isChina">
