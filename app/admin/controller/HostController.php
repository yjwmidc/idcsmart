...
class HostController extends AdminBaseController
{
     ...
     * @return string host.status - 状态Unpaid未付款Pending开通中Active已开通Suspended已暂停Deleted已删除Failed开通失败Grace宽限Keep保留
     ...
     * @return array status - 状态Unpaid未付款Pending开通中Active已开通Suspended已暂停Deleted已删除Failed开通失败Grace宽限Keep保留
     ...
     * @param  float keep_time_price - 保留期价格
     * @param  float on_demand_flow_price - 按需流量价格
     * @param  string on_demand_billing_cycle_unit - 出账周期单位(hour=每小时,day=每天,month=每月)
     * @param  int on_demand_billing_cycle_day - 出账周期号数，每月有效
     * @param  string on_demand_billing_cycle_point - 出账周期时间点(如00:00), 每天/每月生效
     * @param  int upgrade_renew_cal - 升降级时是否按原价处理续费金额：1是，0否默认
     */
