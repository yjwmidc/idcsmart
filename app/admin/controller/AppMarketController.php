...
class AppMarketController extends AdminBaseController
{
	...
	public function setToken(): Json
	{
		...
		$result = ['status'=>200, 'market_url'=>$this->market_url.'/console/v1/app_market/market/token_login?from='.request()->domain().'/'.env('DIR_ADMIN', 'admin').'&token='.$token.'&time='.time()];
		return json($result);
	}
     ...
     */
	public function checkToken(): Json
	{
     ...
     */
	public function getNewVersion(): Json
	{
        for($i=0;$i<3;$i++){
     ...
     */
	public function getTemplateVersion(): Json
	{
		$param = $this->request->param();
	...
	public function install(): Json
	{
		...
		$res = curl($this->market_url."/console/v1/app_market/market/download?id={$id}&from=".request()->domain().'/'.env('DIR_ADMIN', 'admin').'&token='.$token.'&time='.time(), [], 30, 'GET');
		if($res['http_code'] == 200){
     ...
     */
	private function unzip($filepath,$path): Json
    {
        $zip = new \ZipArchive();
     ...
     * @return mixed
     */
	private function curl_download($url, $file_name): Json
	{
	    $ch = curl_init($url);
	    //设置抓取的url
	    $dir = $file_name;
