...
class ViewController extends AdminBaseController
{
	...
	public function index()
    {
		...
		$data['template_catalog'] = env('DIR_ADMIN', 'admin');
		...
		View::config(['view_path' => '../public/'.env('DIR_ADMIN', 'admin').'/template/'.$data['themes'].'/']);
	...
	public function view($tplName, $data){
        View::config(['view_path' => '../public/'.env('DIR_ADMIN', 'admin').'/template/'.$data['themes'].'/', 'view_suffix' => 'php']);
		return View::fetch('/'.$tplName,$data);
    }
