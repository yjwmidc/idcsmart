...
class ProductController extends AdminBaseController
{
     ...
     * @return string list[].pay_type - 付款类型(免费free，一次onetime，周期先付recurring_prepayment,周期后付recurring_postpaid,按需计费on_demand,周期先付+按需计费recurring_prepayment_on_demand
     ...
     * @return int product.auto_renew_in_advance - 自动续费提前开关(0=关闭1=开启)
     * @return int product.auto_renew_in_advance_num - 自动续费提前时间数
     * @return string product.auto_renew_in_advance_unit - 自动续费提前时间单位(minute=分钟,hour=小时,day=天)
     ...
     * @param string pay_type recurring_prepayment 付款类型(免费free，一次onetime，周期先付recurring_prepayment(默认),周期后付recurring_postpaid,按需计费on_demand,周期先付+按需计费recurring_prepayment_on_demand required
     ...
     * @version v1
     * @param   int id - 商品ID require
     * @return  int product_id - 商品ID require
     * @return  string billing_cycle_unit - 出账周期单位(hour=每小时,day=每天,month=每月)
     * @return  int billing_cycle_day - 出账周期号数
     * @return  string billing_cycle_point - 出账周期时间点(如00:00)
     * @return  int duration_id - 周期ID
     * @return  string duration_ratio - 周期比例
     * @return  string min_credit - 购买时用户最低余额
     * @return  int min_usage_time - 最低使用时长
     * @return  string min_usage_time_unit - 最低使用时长单位(second=秒,minute=分,hour=小时)
     * @return  int upgrade_min_billing_time - 升降级最低计费时长
     * @return  string upgrade_min_billing_time_unit - 升降级最低计费时长单位(second=秒,minute=分,hour=小时)
     * @return  int grace_time - 宽限期
     * @return  string grace_time_unit - 宽限期单位(hour=小时,day=天)
     * @return  int keep_time - 保留期 require
     * @return  string keep_time_unit - 保留期单位(hour=小时,day=天)
     * @return  array keep_time_billing_item - 保留计费项目标识
     * @return  string initial_fee - 初装费
     * @return  int client_auto_delete - 允许用户设置自动释放(0=否,1=是)
     * @return  int on_demand_to_duration - 允许按需转包年包月(0=否,1=是)
     * @return  int duration_to_on_demand - 允许包年包月/试用转按需(0=否,1=是)
     * @return  int credit_limit_pay - 允许信用额支付(0=否,1=是)
     */
     ...
     * @version v1
     * @param   int id - 商品ID require
     * @param   string billing_cycle_unit - 出账周期单位(hour=每小时,day=每天,month=每月) require
     * @param   int billing_cycle_day - 出账周期号数 requireIf,billing_cycle_unit=month
     * @param   string billing_cycle_point - 出账周期时间点(如00:00) requireIf,billing_cycle_unit=day/month
     * @param   int duration_id - 周期ID
     * @param   float duration_ratio - 周期比例 requireIf,duration_id>0
     * @param   float min_credit - 购买时用户最低余额 require
     * @param   int min_usage_time - 最低使用时长 require
     * @param   string min_usage_time_unit - 最低使用时长单位(second=秒,minute=分,hour=小时) require
     * @param   int upgrade_min_billing_time - 升降级最低计费时长
     * @param   string upgrade_min_billing_time_unit - 升降级最低计费时长单位(second=秒,minute=分,hour=小时)
     * @param   int grace_time - 宽限期 require
     * @param   string grace_time_unit - 宽限期单位(hour=小时,day=天) require
     * @param   int keep_time - 保留期 require
     * @param   string keep_time_unit - 保留期单位(hour=小时,day=天) require
     * @param   array keep_time_billing_item - 保留计费项目标识
     * @param   float initial_fee - 初装费
     * @param   int client_auto_delete - 允许用户设置自动释放(0=否,1=是) require
     * @param   int on_demand_to_duration - 允许按需转包年包月(0=否,1=是) require
     * @param   int duration_to_on_demand - 允许包年包月/试用转按需(0=否,1=是) require
     * @param   int credit_limit_pay - 允许信用额支付(0=否,1=是) require
     */
