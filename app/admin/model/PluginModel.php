...
class PluginModel extends Model
{
        ...
        if ($pluginConfigInDb) {
            ...
            foreach ($plugin['config'] as $key => $value) {
                ...
                if (isset($pluginConfigInDb[$key])) {
                    ...
                    if($value['type']=='pull_sign' && !empty($plugin['config'][$key]['value'])){
                        $plugin['config'][$key]['type'] = 'text';
                    }
                }
            }
        }
    ...
    public function settingPost($param)
    {
        ...
        foreach ($plugin['config'] as $key => $value) {
            ...
            }else if (in_array($value['type'],['pull_sign'])){
                $res = $this->smsPullSign($param);
                if($res['status']==200){
                    $config[$key] = $res['data']['sign'];
                }else{
                    $config[$key] = '';
                    //return $res;
                }
    ...
    public function smsPullSign($param)
    {
        if (!isset($param['config']) || !is_array($param['config'])){
            return ['status'=>400,'msg'=>lang('param_error')];
        }
        ...
        $SmsTemplateModel = new \app\common\model\SmsTemplateModel();
        $res = $SmsTemplateModel->pullSign($param);
        if($res['status']==200){
            $PluginClass = new $class;
            ...
            foreach ($plugin['config'] as $key => $value) {
                if ($value['type']=='pull_sign') {
                    $pluginConfigInDb[$key] = $res['data']['sign'];
                }
            }

            $update = [];

            $update['config'] = json_encode($pluginConfigInDb);
            $update['update_time'] = time();
            ...
            return ['status'=>200,'msg'=>$res['msg'],'data'=>['sign'=>$res['data']['sign']]];
        }else{
            return $res;
        }
    }
    ...
    public function download($param)
    {
        ...
        $res = curl($this->market_url."/console/v1/idcsmart_business/plugin/".$id."/download?from=".request()->domain().'/'.env('DIR_ADMIN', 'admin').'&token='.$token.'&time='.time(), [], 30, 'GET');
        if($res['http_code'] == 200){
