...
class AccountController extends HomeBaseController
{
     ...
     * @return string account.customfield.pending_amount - 待审核的提现金额
     ...
     * @return string account.oauth[].img - 图标地址
     * @return string account.oauth[].img_unbound - 未绑定图标地址
     * @return int account.notice_open  - 是否接收短信、邮件通知，1是默认0否
     * @return string account.notice_method  - 通知方式：all所有，email邮件，sms短信
     * @return float account.total_credit - 账户余额
     * @return int account.credit_remind - 余额提醒：0关闭默认，1开启
     * @return float account.credit_remind_amount - 阈值
     */
     ...
     */
    public function codeUpdatePassword(): Json
    {
        $param = $this->request->param();
     ...
     */
    public function creditList(): Json
    {
        // 合并分页参数
     ...
     */
    public function updateOperatePassword(): Json
    {
        $param = $this->request->param();
        //参数验证
     ...
     * @author  hh
     * @version v1
     * @param string module - 模块标识(mf_cloud=魔方云,mf_dcim=DCIM) require
     * @return int warning_switch - 预警开关(0=关闭,1=开启)
     * @return int leave_percent - 预警百分比(0是关闭)
     */
    public function clientTrafficWarningIndex(): Json
    {
        $param = $this->request->param();
     ...
     * @version v1
     * @param string module - 模块标识(mf_cloud=魔方云,mf_dcim=DCIM) require
     * @param int warning_switch - 预警开关(0=关闭,1=开启) require
     * @param int leave_percent - 预警百分比(5,10,15,20,关闭传0) require
     */
    public function clientTrafficWarningUpdate(): Json
    {
        $param = $this->request->param();
     ...
     * @method  POST
     * @return int credit_remind - 余额提醒：0关闭默认，1开启
     * @return float credit_remind_amount - 阈值
     */
    public function creditRemind(): Json
    {
        $param = $this->request->param();
