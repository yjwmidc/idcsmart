...
-----END PUBLIC KEY-----";
    ...
    //模板继承文件读取
    private function themeConfig($file): array{
     ...
     */
    private function webSeoCustom($param): array
    {
     ...
     */
    private function webDataCustom($param): array
    {
        $data = [];
    ...
    {   
        ...
        if(in_array($homeTheme, ['mf101','mfm201'])){
            $info = configuration('systemlicenseinfo');
            if(!empty($info)){
                $code = explode('|zjmf|', base64_decode($info));
                $authkey = "-----BEGIN PUBLIC KEY-----
