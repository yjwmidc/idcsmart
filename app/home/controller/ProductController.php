...
class ProductController extends HomeBaseController
{
     ...
     */
    public function productGroupFirstList(): Json
    {
        $result = [
            'status' => 200,
     ...
     */
    public function productGroupSecondList(): Json
    {
     ...
     * @param array product_ids - 筛选哪些商品
     * @return array list - 商品列表
     * @return int list[].id - ID
     * @return string list[].name - 商品名
     * @return string list[].description - 描述
     * @return string list[].pay_type - 付款类型免费free,一次onetime,周期先付recurring_prepayment,周期后付recurring_postpaid,按需计费on_demand,周期先付+按需计费recurring_prepayment_on_demand
     ...
     * @return int product.pay_type - 付款类型(免费free，一次onetime，周期先付recurring_prepayment,周期后付recurring_postpaid,按需计费on_demand,周期先付+按需计费recurring_prepayment_on_demand
...
//    }
     ...
     */
    public function moduleCalculatePrice(): Json
    {
        $param = $this->request->param();
     ...
     */
    public function productStock(): Json
    {
        $param = $this->request->param();
     ...
     */
    public function orderPageSelfDefinedField(): Json
    {
        $param = $this->request->param();
