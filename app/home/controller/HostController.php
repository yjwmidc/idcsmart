...
class HostController extends HomeBaseController
{
     ...
     */
    public function clientHostList(): Json
    {
        // 合并分页参数
        $param = array_merge($this->request->param(), ['page' => $this->request->page, 'limit' => $this->request->limit, 'sort' => $this->request->sort]);
     ...
     */
    public function menuHostList(): Json
    {
     ...
     * @return int host.auto_release_time - 自动释放时间(0=未设置过)
     * @return string host.keep_time_price - 保留期价格
     * @return string host.on_demand_flow_price - 按需流量价格
     * @return string host.on_demand_billing_cycle_unit - 出账周期单位(hour=每小时,day=每天,month=每月)
     * @return int host.on_demand_billing_cycle_day - 出账周期号数
     * @return string host.on_demand_billing_cycle_point - 出账周期时间点(如00:00)
     * @return int host.change_billing_cycle_id - 是否申请了到期转按需(0=否,>0是,申请了不能执行续费/升降级操作)
     ...
     * @return int host.product_on_demand.client_auto_delete - 允许用户设置自动释放(0=否,1=是)
     * @return int host.product_on_demand.on_demand_to_duration - 允许按需转包年包月(0=否,1=是)
     * @return int host.product_on_demand.duration_to_on_demand - 允许包年包月/试用转按需(0=否,1=是)
     ...
     */
    public function clientArea(): Json
    {
        $param = $this->request->param();
     ...
     */
    public function updateHostNotes(): Json
    {
        // 接收参数
        $param = $this->request->param();
     ...
     * @return int count - 产品总数
     */
    public function clientHost(): Json
    {
        $param = $this->request->param();
     ...
     */
    public function suspendAccount(): Json
    {
        $param = $this->request->param();
     ...
     * @param   int id - 产品ID required
     */
    public function unsuspendAccount(): Json
    {
        $param = $this->request->param();
     ...
     */
    public function hostIpIndex(): Json
    {
        $param = $this->request->param();
     ...
     */
    public function hostSpecificInfo(): Json
    {
        $param = $this->request->param();
     ...
     * @method  PUT
     * @param   int id - 产品ID require
     * @param   int auto_release_time - 自动释放时间戳 require
     */
    public function updateHostAutoReleaseTime(): Json
    {
        // 接收参数
        $param = $this->request->param();
     ...
     * @return  string duration[].name_show - 周期显示名称
     */
    public function onDemandToRecurringPrepaymentDurationPrice(): Json
    {
        $param = $this->request->param();
     ...
     */
    public function onDemandToRecurringPrepayment(): Json
    {
        $param = $this->request->param();
     ...
     * @method  GET
     * @param   int id - 产品ID require
     * @return  string on_demand_price - 按需出账价格,可能已经折扣了
     * @return  string base_renew_price - 按需出账原价
     * @return  string keep_time_price - 保留期价格
     * @return  string billing_cycle_name - 周期名称
     */
    public function recurringPrepaymentToOnDemandDurationPrice(): Json
    {
        $param = $this->request->param();
     ...
     * @param   int id - 产品ID require
     */
    public function recurringPrepaymentToOnDemand(): Json
    {
        $param = $this->request->param();
     ...
     * @param   int id - 产品ID require
     */
    public function cancelRecurringPrepaymentToOnDemand(): Json
    {
        $param = $this->request->param();
