...
class SmsTemplateModel extends Model
{
     ...
     * @param int param.resubmit - 已通过模板重新提交0否1是
     */
    public function auditSmsTemplate($param)
    {
        ...
        if(isset($param['resubmit']) && $param['resubmit']==1){
            $sms_template = $this->field('id,template_id,type,title,content,notes,status')
                ->whereIn('id',$ids)
                ->where('sms_name',$param['name'])
                ->select()
    ...
    public function pullSign($param)
    {
        $data = [
            'name' => $param['name'],
            'config' => $param['config'],
        ];
        $sms_methods = $this->smsMethods('pullSign',$data);
        if($sms_methods['status'] == 'success'){
            return ['status'=>200, 'msg'=>lang('pull_sms_sign_success'), 'data' => ['sign' => $sms_methods['sign']]];//签名拉取成功
        }else{
            return ['status'=>400, 'msg'=>lang('pull_sms_sign_error').' : '.$sms_methods['msg']];//签名拉取失败
        }
    }
	//短信接口调用
	private function smsMethods($cmd,$param){
		...
		}else if(strpos($cmd,"pullSign")!==false){
            $data = [
                'config' => $param['config']?:[],
            ];
        }
