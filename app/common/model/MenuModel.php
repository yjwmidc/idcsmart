...
class MenuModel extends Model
{
    ...
    protected $schema = [
        ...
        'create_time'       => 'int',
        'second_reminder'   => 'int',
        'show_quick_order'  => 'int',
        'quick_order_url'   => 'string',
    ];
    ...
    public function getHomeMenu()
    {
        ...
        $menus = $this->field('id,menu_type type,name,language,url,second_reminder,icon,nav_id,parent_id,module,product_id,res_module,show_quick_order,quick_order_url')
            ->where('type', 'home')
            ->order('order','asc')
            ...
            ->toArray();
        if(empty($menus)){
            $menus = NavModel::field('id,name,url,icon,id nav_id,parent_id,module,show_quick_order,quick_order_url')
                ->where('type', 'home')
                ->order('order','asc')
                ->select()
                ->toArray();
     ...
     * @param int menu[].show_quick_order - 是否展示快捷订购
     * @param string menu[].quick_order_url - 快捷订购地址
     * @param array menu[].child - 子菜单 required
        ...
        try {
            ...
            foreach ($param['menu'] as $key => $value) {
                $menu = $this->create([
                    ...
                    'order'             => $order,
                    'create_time'       => time(),
                    'show_quick_order'  => $value['show_quick_order']??0,
                    'quick_order_url'   => $value['quick_order_url']??'',
                ]);
                $order++;
                if(isset($value['child'])){
    ...
    public function saveChild($type = '', $parentId = 0, $child = [], $order = 0)
    {
        foreach ($child as $k => $v) {
            $menu = $this->create([
                ...
                'order'             => $order,
                'create_time'       => time(),
                'show_quick_order'  => $v['show_quick_order']??0,
                'quick_order_url'   => $v['quick_order_url']??'',
            ]);
            $order++;
            if(isset($v['child'])){
    ...
    public function homeMenu(){
        ...
        if(!empty($clientId)){
            ...
            $menus = $this->alias('m')
                ->field('m.id,m.name,m.language,m.url,m.second_reminder,m.icon,m.parent_id,n.url nav_url,m.menu_type,n.plugin,m.show_quick_order,m.quick_order_url')
                ->leftjoin('nav n', 'n.id=m.nav_id')
                ->where('m.type', 'home')
                ->order('m.order','asc')
                ->select()
    ...
    public function customHomeMenuIcon($param)
    {
        try {
            if (empty($param['icon'])){
                throw new \Exception(lang_plugins('param_error'));
            }
            $UploadLogic = new UploadLogic(WEB_ROOT . 'upload/common/iconfont/');
            $file = $UploadLogic->moveTo($param['icon']);
            if (isset($file['error'])){
                throw new \Exception($file['error']);
            }
            $filename = explode('^',$file)[1]??$file;
        }catch (\Exception $e){
            return ['status' => 400, 'msg' => $e->getMessage()];
        }
        return ['status' => 200, 'msg' => lang('success_message'),'data'=>[
            'show_name' => $filename,
            'icon_uri' => '/upload/common/iconfont/' . $file
        ]];
    }
