        ...
        $where = function (Query $query) use ($param, $app, $selectDataRange, $self){
            ...
            if($app=='home'){
                ...
                if(isset($param['scene']) && $param['scene'] == 'security_group'){
                    $query->whereIn('s.module', ['mf_cloud','common_cloud','cloudpods','huawei_cloud','huawei_rds','ctyun_ecs','ctyun_rds'])
            ...
            if(!empty($param['keywords'])){
                $query->where('h.id|p.name|h.name|c.username|c.email|c.phone|h.first_payment_amount|hi.dedicate_ip|hi.assign_ip|h.base_info', 'like', "%{$param['keywords']}%");
            }
