...
class ConfigurationModel extends Model
{
    ...
    public function systemList()
    {
        ...
        foreach($configuration as $v){
            if(in_array($v['setting'], $this->config['system'])){
                if($v['setting'] == 'lang_home_open' || $v['setting'] == 'maintenance_mode' || $v['setting'] == 'client_start_id_value' || $v['setting'] == 'order_start_id_value' || $v['setting']=='home_show_deleted_host' || $v['setting']=='clientarea_logo_url_blank' || $v['setting'] == 'global_list_limit'){
    ...
    public function loginList()
    {
        ...
        foreach($configuration as $v){
            if(in_array($v['setting'], $this->config['login'])){
                if($v['setting']=='email_suffix' || $v['setting']=='admin_login_ip_whitelist'){
                    $data[$v['setting']] = (string)$v['value'];
    ...
    public function loginUpdate($param)
    {
        foreach($param as $k=>$v){
            if($k=='email_suffix' || $k=='admin_login_ip_whitelist'){
                $param[$k] = (string)$v;
        ...
        foreach($loginList as $k=>$v){
            ...
            if(in_array($k, ['email_suffix','admin_password_or_verify_code_retry_times','admin_frozen_time','admin_login_expire_time','home_login_expire_time','admin_login_ip_whitelist'])){
                $lang_old = $v;
                $lang_new = $param[$k];
        ...
        foreach($configuration as $v){
            if(in_array($v['setting'], $this->config['currency'])){
                if($v['setting'] == 'recharge_open' || $v['setting'] == 'recharge_min' || $v['setting'] == 'recharge_max'){
                    $data[$v['setting']] = (float)$v['value'];
                }else if($v['setting'] == 'recharge_notice' || $v['setting'] == 'balance_notice_show'){
                    $data[$v['setting']] = (int)$v['value'];
                }else{
                    $data[$v['setting']] = (string)$v['value'];
                }
    ...
    public function currencyUpdate($param)
    {
        ...
        foreach($desc as $k=>$v){
            $lang = '"'.lang("configuration_log_{$k}").'"';
            if($k=='recharge_open' || $k == 'recharge_notice' || $k == 'balance_notice_show'){
                $lang_old = lang("configuration_log_switch_{$systemList[$k]}");
                $lang_new = lang("configuration_log_switch_{$v}");
            }else{
                $lang_old = $systemList[$k];
        ...
        try {
            foreach($this->config['currency'] as $v){
                if($v == 'recharge_min'){
                    ...
                    $param[$v] = round($param[$v],2);
                }else if($v == 'recharge_open' || $v == 'recharge_notice' || $v == 'balance_notice_show'){
    ...
    public function cronList()
    {
        ...
        // 使用 php think 命令格式
        $data['cron_shell'] = 'php ' . root_path() . 'think cron';
        ...
        $data['cron_task_shell'] = 'php ' . root_path() . 'think task';

        // 按需出账任务队列命令及状态
        $onDemandCronEndTime = configuration('on_demand_cron_end_time');
        // 开始时间
        ...
        $data['cron_on_demand_cron_shell'] = 'php ' . root_path() . 'think on_demand_cron';
    ...
    public function themeList()
    {
        ...
        $adminThemeList = get_files(IDCSMART_ROOT . 'public/'. env('DIR_ADMIN', 'admin') .'/template');
        foreach ($adminThemeList as $key => $value) {
            $data['admin_theme_list'][] = ['name' => $value, 'img' => $domain . '/'. env('DIR_ADMIN', 'admin') .'/template/'.$value.'/theme.jpg'];
        }
...
-----END RSA PRIVATE KEY-----';
            ...
            $url = $domain . '/' . env('DIR_ADMIN', 'admin');
