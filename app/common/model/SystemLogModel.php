...
class SystemLogModel extends Model
{
     ...
     * @param string param.keywords - 关键字,搜索:详情,IP
    ...
    public function systemLogList($param)
    {
        ...
        $where = function (Query $query) use($param, $app) {
            if(!empty($param['keywords'])){
                $query->where('description|ip', 'like', "%{$param['keywords']}%");
            }
            ...
            if (!empty($param['begin_time'])){
                $query->where('create_time', '>=', $param['begin_time']);
            }
            if (!empty($param['end_time'])){
                $query->where('create_time', '<=', $param['end_time']);
            }
            if (!empty($param['admin_name'])){
                $query->where('user_name', 'like', "%{$param['admin_name']}%");
            }
        };
