...
//                }
    ...
    public function indexProduct($id)
    {
        ...
        pay_type,auto_setup,type,rel_id,product_id,price,cycle,renew_rule,auto_renew_in_advance,auto_renew_in_advance_num,auto_renew_in_advance_unit,show_base_info,product_group_id,pay_ontrial')
            ->find($id);
        ...
        try {
            ...
            for ($i=1; $i<=$param['qty']; $i++) {
                $host = HostModel::create([
                    ...
                    'active_time' => $time,
                    'due_time' => !in_array($param['host_billing_cycle'], ['on_demand','onetime']) ? $time : 0,
                    'create_time' => $time,
...
//                    }
                ...
                if (isset($result['data']['discount'])){
                    ...
                    if(!empty($result['data']['renew_price_client_level_discount'])){
                        $result['data']['renew_price_client_level_discount'] = bcmul($result['data']['renew_price_client_level_discount'],$qty,4);
                    }
                    
                    $baseRenewPrice = bcmul($baseRenewPrice,$qty,4);
                }else{
