...
class OrderTmpModel extends Model
{
    ...
    public function pay($param)
    {
        ...
        # 订单归属错误
        if (!$this->isAdmin && $order->client_id != get_client_id()){
    ...
    public function credit($param)
    {
        ...
        if (!$use){
            ...
            // 返回自定义字段
            $hookResult = hook('order_create_return_customfield',['order_id'=>$id]);
            $customfields = [];
            foreach ($hookResult as $value){
                $customfields = array_merge($customfields,$value);
            }

            return ['status'=>200,'msg'=>lang('success_message'),'data'=>['id'=>$id,'customfields'=>$customfields]];
        }
    }
