...
class ResModuleLogic
{
        ...
        if ($res['status']==200){
        	...
        	elseif($res['data']['supplier']['type']=='finance'){ // 对接老财务
	            ...
	            if (!empty($upstreamInfo['token'])){
	                ...
	                $token = md5(rand_str(16).time().$id);
	                $url = !empty($tmp = configuration('clientarea_url'))?$tmp:configuration('website_url'); //request()->domain() . request()->rootUrl(); 服务器定时任务走cli模式,获取的是本地localhost
	                $upstreamHost->save([
	                    'upstream_info' => json_encode(['token'=>$token,'url'=>$url])
	                ]);
	            }
                ...
                try {
                    ...
                    if ($res['status']==200){
                        if (isset($res['data']['order_id']) && $res['data']['order_id']){ # 已在上游下单,但未支付
                            // 使用平台币支付
                            if (!empty($res['data']['customfields']['agent_use_coin'])){
                                idcsmart_api_curl($apiId,'/console/v1/coin/pay',[
                                    'auto'=>1,
                                    'coin_coupon_ids'=>[],
	...
	public function cartCalculatePrice($ProductModel, $params = [], $qty=1, $scene="cal_price",$create=false,$cartSettle=false)
	{
        ...
        elseif ($supplier['type']=='finance'){
            ...
            $getTotalData = [
                ...
                'configoption' => $params['configoption']??'',
                'customfield' => $params['self_defined_field']??[],
                'price_basis'=>$this->upstreamProduct['price_basis']??'agent',
            ];
            ...
            if ($res['status']==200){
                // 计算汇率
                if(!empty($res['products']['type']['type']) && $res['products']['type']['type']==1){
                    $res['products']['sale_total'] = ($res['products']['sale_total'] ?? 0) * $supplier['rate'];
                }else{
                    $res['products']['sale_total'] = bcsub(($res['products']['sale_total'] ?? 0),$res['products']['type']['bates']??0,2) * $supplier['rate'];
                }
            ...
            $res = idcsmart_api_curl($apiId,"/console/v1/product/{$upstreamProductId}/config_option?is_downstream=1", ['config_options'=>$params, 'qty'=>$qty,'price_basis'=>$this->upstreamProduct['price_basis']??'agent'],30,'POST');
	        if ($res['status']==200){
                // 子产品
                if (!empty($res['data']['sub_host']) && is_array($res['data']['sub_host'])){
                    foreach ($res['data']['sub_host'] as &$v){
                        foreach ($v['preview'] as &$v1){
                            $v1['price'] = bcsub($v1['price'],$v['discount']??0,2);
                ...
                if ($this->upstreamProduct['renew_profit_type']==1){
                    $renewProfit = bcadd(0, $this->upstreamProduct['renew_profit_percent'], 2);
                    $res['data']['base_renew_price'] = bcadd($res['data']['renew_price']/$qty,$renewProfit,2)>0?bcmul(bcadd($res['data']['renew_price']/$qty,$renewProfit,2),$qty,2):0;
                    $res['data']['renew_price'] = bcadd($res['data']['renew_price'],$renewProfit,2)>0?bcadd($res['data']['renew_price'],$renewProfit,2):0;
                }else{
                    $renewProfitAndBasePercent = bcadd(1,$this->upstreamProduct['renew_profit_percent']/100,4);
                    $res['data']['base_renew_price'] = bcmul($res['data']['renew_price'],$renewProfitAndBasePercent,2)>0?bcmul($res['data']['renew_price'],$renewProfitAndBasePercent,2):0;
                    $res['data']['renew_price'] = bcmul($res['data']['renew_price'],$renewProfitAndBasePercent,2)>0?bcmul($res['data']['renew_price'],$renewProfitAndBasePercent,2):0;
                }
                $fixedProfit = $this->upstreamProduct['profit_percent'];
                $description = '';
        ...
        if ($res['status']==200 && isset($param['is_downstream']) && $param['is_downstream']==1 && $priceAgent){
	...
	public function durationPrice(HostModel $HostModel)
	{
        ...
        if ($supplier['type']=='whmcs'){
            ...
            $priceBasis = $this->upstreamProduct['price_basis']??'agent';
            $res = idcsmart_api_curl($apiId,"/console/v1/host/{$upstreamHost['upstream_host_id']}/renew",['price_basis'=>$priceBasis],30,'GET');
                ...
                foreach ($res['data']['host'] as $item){
                    ...
                    if ($this->upstreamProduct['renew_profit_type']==1){
                        ...
                        if ($priceBasis=='agent'){
                            ...
                            $item['base_price'] = bcadd($item['base_price'], $item['profit'], 2)>0?bcadd($item['base_price'], $item['profit'], 2):$formatZero;
                        }
                        ...
                        if ($priceBasis=='agent'){
                            ...
                            $item['base_price'] = bcmul($item['base_price'], bcadd($this->upstreamProduct['renew_profit_percent']/100,1,4), 2)>0?bcmul($item['base_price'], bcadd($this->upstreamProduct['renew_profit_percent']/100,1,4), 2):$formatZero;
                        }
                    }
                    $result['data'][] = $item;
                }
