...
{
    public $upgrade_log = "https://api.yjwmidc.com/api/upgrade/idcsmart.html";//更新服务器地址,存放升级版本记录
    //原地址为https://license.soft13.idcsmart.com/upgrade/rc/upgrade.php 这个地址为官方更新地址,为了防止官方更新后覆盖修改后的源码遂更换
    public $upload_dir = IDCSMART_ROOT.'public/upgrade/';//升级包目录
    ...
    public function getAuth()
    {
        if(mt_rand(0, 100)%2==1){
            $res = get_system_license();
        }else{
            $res = $this->keep_pass();
        }
        if($res){
            return ['status' => 200, 'msg' => lang('get_system_license_success')];
        }else{
            return ['status' => 400, 'msg' => lang('get_system_license_failed')];
        }
    }
