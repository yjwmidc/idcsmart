...
class ModuleLogic
{
	 ...
	 * @return  int status - 状态(200=成功,400=失败)
	 * @return  string msg - 信息
	 * @return  string data.price - 价格,按需时=初装费
	 * @return  string data.renew_price - 续费价格/按需价格
	 ...
	 * @return  string data.host_billing_cycle - 当商品是费用类型是多个时,必须返回产品计费周期
	 * @return  float data.keep_time_price - 按需保留期价格
	 * @return  float data.on_demand_flow_price - 按需流量价格
	 */
	 ...
	 * @version v1
	 * @param   ProductModel ProductModel - 商品模型 require
     * @return  string data[].name - 配置项标识
     * @return  string data[].value - 配置项名称
	 */
	public function billingItem(ProductModel $ProductModel): array
	{
		$data = [];
		$module = $ProductModel->getModule();
		if($ImportModule = $this->importModule($module)){
			if(method_exists($ImportModule, 'billingItem')){
				$data = call_user_func([$ImportModule, 'billingItem'], ['product'=>$ProductModel]);
			}
		}
		return $data;
	}
	 ...
	 * @param HostModel HostModel - 产品模型 require
	 * @param array  params - 自定义参数 require
	 * @param string params.old_billing_cycle - 原计费方式 require
	 * @param string params.new_billing_cycle - 新计费方式 require
	 * @param int params.duration_id - 周期ID(按需转包年包月有)
	 */
	public function changeBillingCycle(HostModel $HostModel, $params): void
	{
		$module = $HostModel->getModule();
		if($ImportModule = $this->importModule($module)){
			if(method_exists($ImportModule, 'changeBillingCycle')){
				// 获取模块通用参数
				$moduleParams = $HostModel->getModuleParams();
				$moduleParams['custom'] = $params;
				
				call_user_func([$ImportModule, 'changeBillingCycle'], $moduleParams);
			}
		}
		// 不需要返回
	}
	 ...
	 * @return  array
	 */
	public function currentConfig(HostModel $HostModel): array
	{
		$res = [];
		$module = $HostModel->getModule();
		if($ImportModule = $this->importModule($module)){
			if(method_exists($ImportModule, 'currentConfig')){
				// 获取模块通用参数
				$moduleParams = $HostModel->getModuleParams();
				$res = call_user_func([$ImportModule, 'currentConfig'], $moduleParams);
			}
		}
		return $res;
	}
...
//        $module = $ProductModel->getModule();
     ...
     * @param   string param.template_dir - 模板路径(clientarea=会员中心,cart=购物车) require
     * @param   string param.file - 主题下文件名,如product_detail.html require
     * @return  array
     * @return  string template - 模板路径
     */
    public function moduleDefaultView($param)
    {
    	$module = $param['module'];
    	$templateDir = $param['template_dir'];
    	$file = $param['file'];

    	// 模块路径
    	$moduleDir = WEB_ROOT . 'plugins/server/'.$module.'/';
    	if (use_mobile()){ // 手机端
    		if($templateDir == 'cart'){
            ...
            if(in_array($mobileTheme, ['mf101','mfm201'])){
                $info = configuration('systemlicenseinfo');
                if(!empty($info)){
...
-----END PUBLIC KEY-----";
            ...
            $template = "template/{$templateDir}/mobile/{$mobileTheme}/{$file}";
        	...
        	// pc端
            if($templateDir == 'cart'){
                $clientareaTheme = configuration('cart_theme');
            }else{
            	$homeHostTheme = configuration('home_host_theme');
            	$homeHostTheme = json_decode($homeHostTheme ?? '{}', true) ?: [];
                $clientareaTheme = $homeHostTheme[ $module ] ?? 'default';
            }
            $template = "template/{$templateDir}/pc/{$clientareaTheme}/{$file}";
            if (!file_exists($moduleDir . $template)){
