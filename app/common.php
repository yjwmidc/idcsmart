...
function create_jwt($info, $expire = 7200, $is_admin = false)
{
    ...
    $authCode = env('AUTHCODE'); // 先获取 AUTHCODE
...
{
	if($app == 'admin') $app = env('DIR_ADMIN', 'admin');
...
function lang($name = '', $param = [])
{
 ...
 //    }
	...
	// $langAdmin = include WEB_ROOT.'/'.env('DIR_ADMIN', 'admin').'/language/'. $defaultLang .'.php';
...
function idcsmart_password($pw, $authCode = '')
{
    ...
    //如果不存在或者为空 false
    //把字符串转换成bool值
    if ((bool)env('IS_ZKEYS', false)) { 
        //兼容zkeys迁移密码
        // Zkeys 逻辑中，$authCode 通常是作为参数传入的，如果为空则不使用
...
function idcsmart_addon_url($url, $vars = [], $is_admin = false)
{
    ...
    if ($is_admin){
        $new = '/'. env('DIR_ADMIN', 'admin') . '/addon?' . http_build_query($params);
    }else{
...
function check_admin_enforce_safe_method_redirect()
{
    ...
    return ['redirect'=>$redirect, 'url'=>request()->domain() . '/' . env('DIR_ADMIN', 'admin') . '/security_center.htm'];
}
