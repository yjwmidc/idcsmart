...
class  AppInit
{
    public function handle()
    {
        ...
        # 模板控制器路由(需要登录才能访问)
        Route::any(env('DIR_ADMIN', 'admin').'/v1/template/[:_plugin]/[:_controller]/[:_action]', "\\app\\event\\controller\\TemplateController@index")
            ->allowCrossDomain([
                'Access-Control-Allow-Origin'        => $origin,
                'Access-Control-Allow-Credentials'   => 'true',
                'Access-Control-Max-Age'             => 600,
        ...
        Route::any(env('DIR_ADMIN', 'admin').'/addon', "\\app\\event\\controller\\AddonController@index")
        ...
        Route::any(env('DIR_ADMIN', 'admin').'/module/[:module]/[:controller]/[:method]', "\\app\\event\\controller\\ModuleHomeController@index")
...
//        }
        ...
        if(stripos(ltrim($requestStr, '/'), env('DIR_ADMIN', 'admin').'/v1/plugin/')===0 && in_array(substr($requestStr, strrpos($requestStr, '/')), ['/download', '/upgrade'])){
        }else if(stripos(ltrim($requestStr, '/'), env('DIR_ADMIN', 'admin').'/v1/upstream/product')===0 && in_array($requestMethod, ['POST', 'PUT'])){
